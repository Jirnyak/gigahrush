...
Второй проход того же дня (по фидбеку владельца):

- **Доска рисуется у ПРИНЯВШЕГО приглашение.** `openTabletopGameFor(npcId)` сверял id оппонента с хостовым креслом `npc` из снапшота — у акцептора это разные номера, доска не рисовалась, вместо неё текст-заглушка («что-то открылось, но играть нельзя»). Теперь сидящий за кооп-столом оппонент — достаточное доказательство своей партии; точная сверка осталась для игр против NPC. Одна правка в `systems/tabletop.ts` закрывает все восемь игр.
- **ОБМЕН заработал впервые.** Три дефекта: (1) `proposeCoopSession` отвергал `stake <= 0` («Ставку не покрыть»), а бартер объявляет `stake: () => 0` замыслом — обмен не открывался никогда; отказ снят, нулевая ставка легальна и для нищих партий. (2) Confirm-флаг существовал только в протоколе — ни одна клавиша его не слала; теперь ↑ подтверждает/снимает (`upNav` в `handleCoopBarterInput`, payload-`confirm` оставлен). (3) У бартера не было отрисовки вовсе — добавлен `render/barter_ui.ts` (две корзины, сумка с курсором, журнал, подсказка клавиш), подключён в интерфейс-вкладку `npc_ui`.
- **E работает в меню пира.** Онлайн-блок пира съедал фронт `input.interact` даже при открытом меню — E-взять в ящике не срабатывал; теперь клавиша при открытом меню принадлежит меню.
- **Копия удалённого ящика переживает пересборку карты.** Копия живёт только в производной `containerById`, которую чистит любой `rebuildContainerMap()` — меню закрывалось «само»; кадр пира возвращает копию на место из модульной переменной. Хост логирует результат каждого take/put (`ок/ОТКАЗ`, чем разрешился resolve) в `[netdbg]`.
- **Набор всех восьми настольных игр в «Учебном шкафчике вылазки»** (комната Ольги Дмитриевны): без предметов кооп-партию было не собрать, пока набор не выпадет из мира.

Третий проход 2026-09-03 (кооп-столы фундаментально + мир для гостя):

- **Главный корень мёртвых столов:** `closeNpcInteractionInterface` закрывает ВСЕ настольные игры, а `openCoopInterfaceLocally` звал его СРАЗУ ПОСЛЕ того, как `openCoopSession` разложил партию — хост сам убивал свежие шахматы, обе стороны навсегда видели текст-заглушку. Чистка предыдущей NPC-партии перенесена ДО раскладки (в `hostResolveCoopReply` и `onOnlineCoopOpen`), из `openCoopInterfaceLocally` вызов удалён.
- **Кресло без меню = выход из-за стола.** Путей закрыть интерфейс много (ЛКМ-крестик, ПКМ, тап), и каждый пропущенный оставлял игрока сидящим за невидимым столом — замороженным навсегда. `reconcileCoopSeat()` каждый кадр: меню закрыто, а `coopSeatOf(player.id)` жив → пир шлёт `coop_leave`, хост сдаёт своё кресло.
- **Страховка вью:** пока стол жив, хост раз в секунду переотправляет `coop_view` — потерянное сообщение больше не оставляет кресло без доски.
- **Блокираторы у гостя.** Поле `world.pathBlockers` — производное от features и в снапшот не пакуется; сетевой unpack его не пересобирал (в отличие от restore-путей `floor_memory`), и гость ходил сквозь мебель и блокираторы. Добавлен `rebuildPathBlockersFromWorldObjects` после распаковки снапшота.
- **Мировые интеракции гостя: раковина, туалет, контентные механики, аномалии, ремонт гермодвери.** У `activateNormalPriorityInteractionForLook` появился режим `worldOnly` (без транспорта, дверей, контейнеров и хост-оверлеев), обёрнутый в `activateWorldInteractionForActor`; хост зовёт его хвостом обслуживания E пира. Сообщения интеракции на время вызова перехватываются и уезжают гостю существующим `coop_note` — а не в лог хоста.
Четвёртый проход 2026-09-03 (по фидбеку живого теста «стало намного лучше»):

- **`/netdebug` в чате (N)** — рантайм-переключатель сетевого лога (лог и тумблер переехали в `systems/online_client.ts`). Консоль/localStorage подводили: на встраивающих площадках консоль смотрит не в тот фрейм.
- **Самосбор и гости не сосуществуют** (решение владельца): при живых гостях таймер самосбора замирает; `/host` во время самосбора даёт отказ; вход гостя в комнату с активным самосбором отклоняется `server_error: samosbor_active` (гость видит «вход закрыт» и отключается); жертва инвазии при самосборе хост не поднимает — метка истекает. `version_mismatch` заодно получил человеческий текст и дисконнект.
- **Цвет фигур — абсолютный** (шахматы, шашки): армия предложившего золотая, принявшего серая НА ОБОИХ экранах; раньше каждый видел свои золотыми и «оба играли белыми». Позиция по-прежнему зеркалится (своя армия снизу). Го/нарды уже были абсолютными. Замок обновлён в `tests/chess.test.ts`.
- **Бросок предмета обрезается первой стеной** (`dropItem` + пир-ветка drop): слепые «3 клетки вперёд» хоронили предмет в бетоне — так «пропадали» вещи, выброшенные лицом к стене или стоящему рядом напарнику.
- **Кровь у гостя**: `spawnBloodHit` на хосте реле-хуком шлёт fx `hit {a: угол, m: gore}` (кап общий, 24/тик), гость проигрывает те же брызги/отметины. Один хук — по образцу `publishWeaponNoise`; не добавлять pushNetFx в отдельные боевые пути.

- **Экранный контент гостя: верстак и компьютеры.** Хост НЕ открывает оверлеи у себя — он называет цель, экран рисует гость. Верстак: `openCraftMenu`-колбэк world-only цепочки шлёт `craft_open {mode, station}`, гость открывает своё крафт-меню; пул материалов и изученные рецепты — ГОСТЕВЫЕ (живут в его `state.crafting`, не хостовом), локальный крафт — предсказание, а предмет выдаёт/забирает хост по интентам `craft {recipeId}` / `craft_break {slot, defId}` (relaxed trust: только выход существующего рецепта, произвольный предмет не сминтить; разборка слот-валидируется и снимает надетое `reconcileEquippedAfterLoss`). Компьютеры: реестр терминалов в снапшот не пакуется, поэтому хост шлёт `overlay_open {overlay:'computer', idx, defId}`, гость строит терминал литералом и открывает оверлей — содержимое целиком в data. Остальные экраны (игровые автоматы, НЕТ-взлом, скоростной лифт, картостанция) гостю по-прежнему молчат: у них деньги/мир/сеть завязаны на хостовые системы, отдельная итерация.
...
- **Инвазии `/invade` (добавлено 2026-08-10)**: Dark-Souls-style вторжение БЕЗ явного `/host` у жертвы. Инвейдер пишет `/invade` в НЕТ-СФЕРЕ → `POST /api/net/invade` помечает случайную живую сессию (`net_sessions.invaded_by/invaded_at`, TTL 120 с, исключаются мёртвые/gameOver/уже помеченные/сам инвейдер — фильтр по `progress_json`). Жертва узнаёт о метке из ответа очередного heartbeat `/hello` (`invasion: {by, nickname}`) и МОЛЧА поднимает хост (`startOnlineHost` + немедленный heartbeat с `hostingRoomId`); гость в чужой комнате хостом не становится — метка истекает. Инвейдер поллит `/invade` каждые 5 с (до 36 попыток) и по `status:'ready'` подключается `joinOnlinePeer(roomId, invader=true)`; `peer_join` несёт флаг `invader`. Хост даёт инвейдеру `Faction.WILD` (враждебен его NPC и монстрам через обычную матрицу отношений) плюс личную враждебность `playerRelation=-100`; личный канал в `isHostile` симметричен (ветка «player→personally-hostile NPC» в `factions.ts`), поэтому пара хост↔инвейдер остаётся взаимно враждебной, даже если хост в своём прохождении задружился с дикими и поднял матрицу WILD↔PLAYER выше порога и спавнит ДАЛЕКО от хоста — в клетке случайного живого NPC/монстра на дистанции ≥24 (гарантированно играбельное место); при смене этажа/пост-самосборном ститче инвейдер снова размещается далеко, а не у хоста. Обе стороны получают HUD-предупреждение (`consumeNetSphereNotices` дренится в `main.ts`). Смерть/эвакуация/дисконнект инвейдера — стандартные правила визита (труп с лутом у хоста, домой лифтом живым). Миграция D1: `cloudflare/d1/net_sphere_v3.sql`.
- Тесты: `tests/online_protocol.test.ts` (санитизация интентов/экспорта/trade_deal, эхо-reconciliation, клеточные патчи, интерполяция, container/npc dirty-push, slot bookkeeping); `tests/net-sphere.test.ts` (полный цикл матчмейкинга `/invade`: метка → сигнал в hello → hosting_room → ready; фильтры мёртвых/своей сессии).

Известные ограничения v2 (кандидаты следующей итерации): квесты/контракты/кастомные NPC-опции для пира; карма/экономические события сделок пира не публикуются (только материальный обмен); цены пира считаются по его локальной эконом-копии (relaxed-trust, допустимо по Trust Correction). Ограничения инвазий: жертва слышит heartbeat раз в 30 с — вторжение подключается до ~40 с (по-даркосусльному медленно, это ок); инвейдер попадает в комнату жертвы, куда могут `/join` и обычные гости; специальной награды за убийство хоста нет (труп инвейдера — обычный лут).
