...
- **Тесты генерации.** `npm run test:generation` — 4 падения из 1059 (было 168, затем 29) и он по-прежнему не входит ни в один гейт. Замерено 2026-08-15 прогоном на коммите `a294402c` и на текущем дереве: `craft-recipe-sources` ×2, `hell-genfix-087` (ждёт решения владельца), `hladon-anomaly`. Разбор по классам — в `tests.md`. Ловушка при сверке: «ordinary story floor craft station placement» падает только в полном прогоне и проходит в одиночку — это давняя нестабильность, а не свежая регрессия. Общее правило прежнее: устаревший счётчик переводится в инвариант, качественно неверное значение (ноль населения, недостижимая комната) двигать порогом нельзя.
