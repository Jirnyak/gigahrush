...
- **Тесты генерации.** `npm run test:generation` — 29 падений (было 168) и он не входит ни в один гейт. Прежде чем чинить падение, определи класс: устаревший счётчик переводится в инвариант, качественно неверное значение (ноль населения, недостижимая комната) — это находка в коде, порог двигать нельзя.
...
- **Верхний пояс маршрута (|z| ≥ 44).** `baseNpcTarget` глушит там обычное население, а `floorRunZAllowsNpcs()` его разрешает — два источника истины расходятся, решение за владельцем.
