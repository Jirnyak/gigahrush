...
- **Регистрация контента — на импорте (частично закрыто 2026-08-15).** Ребро `systems/samosbor → gen/floor_manifest` срезано, крупнейший рантайм-цикл 293 → 106, генератор приходит инъекцией, точка сборки контента — `src/content.ts`. Кому нужен реестр — импортирует её сам, включая тесты. Осталось: 439 вызовов `register*` живут на верхнем уровне модулей, и порядок их выполнения значим. Ленивый реестр этажей поэтому невозможен и на цикл ничего не даёт — замерено, те же 106. Подробности — первая строка `problems.md`.
