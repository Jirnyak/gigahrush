...
- **Тесты генерации.** `npm run test:generation` — 29 падений (было 168) и он не входит ни в один гейт. Все 29 разобраны по классам в `tests.md` («Разбор 29 падений матрицы»): 1 файл вообще не импортируется и скрывает свой счёт, 8 правятся в тесте, 18 — находки в коде, 1 — спор о дизайне. Не начинай с нуля: возьми класс оттуда. Общее правило прежнее — устаревший счётчик переводится в инвариант, качественно неверное значение (ноль населения, недостижимая комната) двигать порогом нельзя.
- **Резервные личности A-Life.** Числовой `plotNpcId` — позиционный индекс регистрации пакетов; переданный снаружи, он подменяет личность. Связывать резерв нужно строковым `id` вида `npc:<пакет>`. `tests/demos.test.ts` красный именно поэтому.
