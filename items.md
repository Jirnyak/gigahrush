# Система предметов, оружия, ресурсов и производства

> Центральный документ предметной системы.
>
> Роль: описывает items, tools, weapons, PSI clots, loot, resources, crafting/production, food, water, value, use effects, procedural sprites and economy hooks. Связан с `economics.md`, `balance.md`, `alife.md`, `fight.md` and `quests.md`.

Актуально на 2026-05-30. Этот файл описывает уже работающую предметную систему, а не список будущих item-кандидатов. Архивные item-манифесты и оркестратор перенесены в `../gatbage/`.

## Слой данных

Базовая единица - `ItemDef` в `src/data/items.ts`: `id`, русское `name`, `ItemType`, `desc`, `spawnRooms`, `spawnW`, `value`, опциональные `tags`, `stack`, `durability` и `use`. Сейчас registry содержит 446 item id.

`ItemType` намеренно грубый: `FOOD`, `DRINK`, `MEDICINE`, `WEAPON`, `TOOL`, `KEY`, `NOTE`, `MISC`, `AMMO`. Новые подтипы обычно выражаются tags, use-handler, weapon stats, resource mapping или production recipe. Новый enum нужен только если существующие каналы не могут выразить поведение без частных костылей.

`ITEM_TAGS` - основной язык сквозной интеграции. Через tags предмет становится документом, контрабандой, sampleware, repair input, bait, cleanup tool, ammo family, resource member, production input, counterplay item или route clue. Логика должна читать ids/tags, а не русские display names.

## Единый Источник Истины: Универсальный Процедурный Лут

Система `src/systems/procedural_loot.ts` является универсальным генератором лута, который опирается на реестр `ItemDef` (`ITEMS` в `src/data/items.ts`) как на Единый Источник Истины. 

Эта система устраняет необходимость в жестком хардкоде инвентарей (например, таблиц для каждого NPC или контейнера отдельно) и опирается на параметры самих предметов:
- `value`: Ценность предмета сопоставляется с бюджетом лута (который зависит от уровня сущности, глубины/опасности этажа).
- `ItemType` и `tags`: Определяют предпочтения фракций, классов (огнестрел для ликвидаторов, медикаменты для ученых, и т.д.).
- `spawnW`: Базовый вес появления предмета.

**ЗАКОН: таблица лута универсальна и агностична.** Она не знает и не должна знать,
что за предмет через неё проходит: патрон, еда, ключ или ствол попадают в неё на
общих правах, а редкость выражается **ценой и ресурсом**, а не запретом на спавн.
`spawnCount` (`data/items.ts`) сам делает дорогое редким: дешёвое ложится пачкой,
дорогое поштучно.

Обратный закон в проекте уже был и обошёлся дорого. До 2026-08-29 у **всех
семнадцати** видов патронов стоял `spawnW: 0`, и это было заперто тестами как
замысел («патроны идут из плановых источников»). Следствие: `buildLootPool`
отбрасывает нулевой вес первой строкой, поэтому патрон не мог попасть **ни в один**
процедурный ящик, карман или на прилавок — весь боезапас на этажах был расставлен
вручную. Множители `ammoMult: 3` и `4` в профилях фракций и теги ящиков
`'ammo'`/`'weapon'` при этом были написаны и умножали ноль. Замер: **0.9 патрона на
стрелка** во всём мире базы ликвидаторов при 442 опустошениях за десять минут, и
единственным настоящим источником была досыпка боеприпаса из воздуха
(`trySimulateNpcAmmoRestock`) — то есть запрет на общий лут не создал дефицит, а
породил предмет без происхождения.

**Не восстанавливать `spawnW: 0` как «замысел».** Ноль означает ровно одно: у
предмета есть ЯВНЫЙ источник и он в общий лут не идёт (ключи маршрута, сюжетные
вещи). Для целого класса предметов ноль — запрет, а не редкость. Замки нового
закона — `tests/data-ids.test.ts` («ammo enters the generic loot table like any
other item») и восемь поштучных тестов патронов.

**Планы интеграции:**
Система уже используется для генерации стартового инвентаря NPC (A-Life loadouts) и автоматической выдачи боеприпасов для их оружия.
В дальнейшем эта же система будет интегрирована для:
1. **Генерации лута в контейнерах:** Заполнение ящиков, сейфов и тайников на основе уровня этажа и фракции-владельца.
2. **Торгового инвентаря:** Генерация ассортимента торговцев с учетом их фракции, редкости и экономики (динамическое ценообразование на основе базового `value`).

Рантайм-инвентари имеют универсальную вместимость в 64 слота (`MAX_INVENTORY_SLOTS`), а количество предметов, генерируемых в контейнерах через `seedInventory` (`src/systems/containers.ts`), масштабируется процедурно в зависимости от глубины (номера этажа `Z`), обеспечивая больше добычи на глубоких опасных этажах. Вместимость у всего одна: поле `capacitySlots` снято целиком 2026-09-12 — из `WorldContainer`, из `ContainerDef` и из 130 контентных пакетов. Оно и не было вместимостью: класть игрок мог до общих 64, а числом 5..12 ограничивался только генератор, то есть отвечало оно на вопрос «сколько КЛАСТЬ». На деле связывала ценность (`proceduralValueCap`), а не слоты — замерено: слотовый потолок упирался у 1 ящика из 68 на жилом, после снятия этаж не сдвинулся ни на предмет, а по формуле процедурных этажей разошлись 4 сочетания вид×опасность из 110. Локальные параметры «сколько класть» теперь называются `fillSlots`.

## Инвентарь и использование

Runtime-предметы хранятся как compact stacks: `defId`, `count`, опциональное `data`. Стекуемость выводится из типа и `stack`; оружие и инструменты обычно занимают отдельный слот, durable tools расходуют `durability`. `src/systems/inventory.ts` владеет категориями, equip/use/drop/read/check actions, ammo consumption, tool durability, use-events и player-facing сообщениями.

World item drops подбираются через тот же inventory transfer path. Browser-local настройка автоподбора включена по умолчанию; если игрок выключает ее в `U` -> настройки интерфейса, item drops больше не берутся при проходе рядом и становятся `E`-целью с названием, описанием и ручным pickup prompt.

Использование предмета должно давать решение, а не только эффект:

- еда/вода закрывает needs, но может портить HP, статус или маршрутный риск;
- медицина лечит, снимает статус или меняет tradeoff;
- документы открывают доступ, создают audit risk, forged/legal fork или evidence handoff;
- инструменты чинят, чистят, светят, метят, герметизируют, вскрывают или дают counterplay;
- образцы и ПСИ-сгустки связывают НИИ, контрабанду, риск вскрытия, ресурс `slime_samples`/`psi` и события.

Важные действия публикуют `publishEvent()` с компактными tags/data. Горячий runtime не должен парсить JSON или сканировать весь мир ради предмета.

## Оружие

Физическое оружие живет в `src/data/weapons.ts`, ПСИ-сгустки - в `src/data/psi.ts`, объединенный read-only registry экспортируется из `src/data/catalog.ts` как `WEAPON_STATS`. Сейчас есть 71 физических weapon stat entries и 18 PSI entries.

Каждое `ItemType.WEAPON` должно иметь executable stats или быть явно небоевым предметом другого типа. Физические stats задают damage, range, speed, durability, ranged/ammo, projectile speed, pellets/spread, projectile type, AoE, beam flags и sound id. Role tiers дают HUD/readability язык: emergency melee, industrial tool, reach, heavy, control, sidearm, rifle, corridor shotgun, ammo burn, grenade, rare energy, fuel clear, PSI.

ПСИ-сгустки используют тот же executable weapon contract, но экипируются через `tool` slot, платят `psiCost` и могут иметь `psiEffect`. Они остаются предметами и ресурсом, а не отдельной магической подсистемой вне инвентаря.

## Броня: лестница из восьми ступеней (2026-08-27)

Броня — **покупка, а не находка**. До перецены самый дорогой комплект игры стоил дешевле самого
дешёвого приличного ствола, а четыре комплекта из пяти влезали в `lootValueCap` средней полосы и
валялись в сейфах как расходник.

Цены выведены из полос экономики (`ECONOMY_MONEY_BANDS`, `economics.md` §5) и не содержат
собственных чисел:

| ступень | цена, ₽ | вид | чем она является |
| --- | ---: | --- | --- |
| Лёгкая броня | 12 000 | `CLOTH` | начало: понемногу от всего |
| Комплект ОЗК | 16 000 | `CLOTH` | специалист: **БИО 70** |
| Костюм ТОК-200 | 18 000 | `CLOTH` | специалист: **ОГОНЬ 70** |
| Средняя броня | 20 000 | `PLATE` | патрульная баллистика |
| Ряса Культиста | 100 000 | `CLOTH` | специалист: ПСИ 75 |
| Тяжёлая броня | 140 000 | `PLATE` | глубокая баллистика |
| Броня Ликвидатора | 200 000 | `PLATE` | штурм, замкнутый контур |
| Комплект СЗК-9 | 500 000 | `PLATE` | универсал без дыр |

Верх лестницы стоит ровно столько же, сколько верх оружейной (гравилуч, 500 000 ₽): полная защита
и лучшее оружие — покупки одного порядка.

**Правило колонки БИО.** У НЕГЕРМЕТИЧНОГО комплекта БИО равен его же ОГНЮ: и пламя, и кислота идут
по швам, и держит их одна и та же толщина (лёгкая 5/5, средняя 10/10, тяжёлая 20/20). Исключений
два, и оба объяснены материалом:

- **ряса культиста** — ОГНЯ у неё нет вовсе, смоляная пропитка горит охотнее сухой ткани, а жидкое
  та же пропитка отталкивает (БИО 10, много ниже её настоящей темы — ПСИ 75);
- **броня Ликвидатора** — единственный герметичный комплект из семи, её БИО выведен не из толщины,
  а из замкнутого контура: половина глубины специалиста (70/2 = 35).

Отдельно заперт честный ноль: **ТОК-200 кислоту не держит намеренно** (`resistances[BIO]`
не объявлен). Асбестовое полотно держит пламя и не держит ничего больше — это и есть то «и всё»,
ради которого узкая специализация заведена; ноль заперт своей строкой теста, чтобы «забыли
объявить» и «объявили ноль намеренно» не путались.

**Форма строки, а не сумма.** Три ступени различимы замыслом: начало даёт понемногу минимум от
четырёх колонок и ни одной глубже половины; специалист глубже всего в СВОЕЙ колонке и минимум
вчетверо глубже любой другой; универсал объявляет все шесть, и самая слабая его колонка не ниже
того числа, которым лестница уже называет тяжёлую защиту. При этом специалист остаётся глубже
универсала в своей колонке — иначе покупка за 500 000 отменяет выбор, ради которого специализация
и заведена.

**Достижимость.** У каждой ступени есть россыпь или прилавок, и это проверяется той же дверью, что
у игры (`generateNpcTradeItems`). СЗК-9 в лут не попадает никогда (`spawnW: 0`, комнат появления
нет): его единственный путь — прилавок завхоза, и витрину видно задолго до денег на неё.
`spawnRooms` у брони стал АДРЕСОМ «чья это одежда»: ОЗК живёт в медпункте и на складе, ТОК-200 —
в цеху, боевые комплекты — у службы.

**Кислотный фартук намеренно НЕ получил резистов** (`protective_apron`, 95 ₽). Любая вещь с
`resistances` становится бронёй по `itemEquipSlot`, а комплект за 109 ₽ со спредом по карману уже
на старте — и правило «броня начинается не раньше среднего маршрута» ломается. Фартук остаётся
торговым вещдоком поста НИИ; химзащита — отдельная вещь средней ступени. Решение заперто
`tests/armor-ladder-reach.test.ts`.

Замки: `tests/armor-price-ladder.test.ts` (монотонность, «не находка», по карману своей полосе,
броня доходит до людей), `tests/armor-ladder-reach.test.ts` (россыпь или прилавок у каждой
ступени), `tests/armor-matrix-parity.test.ts` (колонка БИО закрыта, форма строк).

## Ресурсы, крафт и производство

`src/data/resources.ts` связывает предметы с экономическими ресурсами: вода, еда, медицина, металл, патроны, инструменты, бумага, топливо, электроника, ПСИ, образцы слизи, контрабанда, документы, industrial slurry, грибной субстрат, желемыш и трудочасы. Сейчас есть 17 resource defs.

Игровой крафт отделен от макроэкономических `ResourceDef`. `src/data/craft_materials.ts` задает фиксированный вектор из 9 craft-компонентов: mechanics, electronics, consumables, bio, chemical, metal, cybernetics, psimatter, metamatter. `src/data/item_composition.ts` хранит composition для каждого предмета, а `src/data/craft_recipes.ts` строит item recipes поверх этих vectors. Это definition-level данные; runtime item stack не хранит состав в `Item.data`.

Рецепты становятся известны через default список и источники в `src/data/craft_recipe_sources.ts`: blueprint items, notes, quests, terminals, NPC/floor sources and recipe billboards. Станки открываются через interactive definitions `craft_lathe`, `disassembly_workbench`, `craft_lab_bench` and `recipe_billboard`; сборка и разбор используют player crafting state, а не factory production queues.

`src/data/factories.ts` описывает production surfaces: factories, recipes, inputs, inputItems, outputs, access, cycleSec, bad batches and repair outputs. Сейчас есть 12 factory defs и 42 recipes. Factories остаются floor/economy production path with cadence and access rules; крафт остается personal item/material path.

Рецепт должен иметь достижимые inputs, полезный output, access decision и bounded cadence. Bad batch допустим как factory rule, если у него есть repair path и событие; он не должен превращаться в скрытый таймер без игрокового решения.

## Достижимость

Предмет считается shipped только если игрок может встретить или использовать его нормальным путем:

- room loot через `spawnRooms`/`spawnW`;
- procedural NPC loadouts через `src/systems/procedural_loot.ts` (зависит от `value`, типов, фракционных тегов и множителей);
- container pool, faction/owner/locked/secret access;
- NPC trade, contract, quest, route floor, samosbor aftermath;
- factory output, craft recipe/source или repair input;
- one-per-run unique source с явным route/debug proof;
- use-handler, weapon stats, document gate, counterplay или event consequence.

`spawnW: 0` не делает предмет запрещенным, но требует явного источника. Мертвые registry entries без reachability не должны добавляться.

### Именованные схроны

Авторская сцена вокруг предмета — не рукописная функция в генераторе, а строка в реестре `src/data/authored_caches.ts`. Условия существования схрона (глубина, опасность, геометрия, большинство, тип или префикс комнаты) лежат данными; содержимое выводится из предметов, объявивших **дом тегом**, равным `id` схрона. Генератор процедурных этажей не знает ни одного предмета по имени.

Практический смысл: новое оружие получает свой тайник одной строкой в `items.ts` — дописать тег. Количество считается одним выражением от опасности этажа с потолком по стаку предмета, поэтому таблиц количеств тоже нет.

Чего делать нельзя: гейтить существование сцены жребием `spec.lootBiasIds`. Так было раньше у награды ликвидаторов — авторский схрон появлялся, только если случайная пятёрка лута вытянула его ствол (0.63% этажей вместо 3.47% по контексту).

### Дневник покойного

`npc_diary` — один `ItemDef` на всех, а личность лежит в экземпляре: `Item.data` несёт `plotNpcId`, `title` и текст записки. Это единственный сейчас предмет с персональным именем экземпляра, и механизм общий: `itemInstanceName()` даёт `data.title` приоритет над `def.name` там, где для длинного имени есть место — детали выбранного предмета в инвентаре, в торговле и в контейнере. Любая будущая авторская записка может носить своё имя тем же путём.

Тип `ItemType.NOTE` выбран не для красоты: он бесплатно даёт действие «прочесть» с печатью `data.text` (`src/systems/inventory.ts`) и защиту дропа от вытеснения по FIFO (`src/systems/entity_limits.ts`). Разные дневники не слипаются в один слот — `canStackData` требует пустого `data` с обеих сторон.

Достижимость: падает с любого сюжетного NPC при смерти и воплощается на месте гибели, когда цепочка в покойного упёрлась. Правила — `quests.md`, «След покойного»; код — `src/systems/plot_trace.ts`.

## Добавление предмета

Минимальный порядок:

1. Добавить `ItemDef` с lowercase snake_case id и русскими `name`/`desc`.
2. Добавить tags, если предмет участвует в ресурсах, документах, контрабанде, sampleware, counterplay или производстве.
3. Для оружия добавить `WeaponStats`/role tier или PSI stat.
4. Привязать ресурс в `resources.ts`, если предмет влияет на scarcity/economy.
5. Дать reachability через rooms, containers, factory, contract, NPC, route content or aftermath.
6. Добавить use-handler/event/test только если поведение не data-only.

Запрещены сотни near-duplicate guns, новый item type ради вкуса, renderer-owned item state, per-frame global gas/slime/fluid scans, save shape changes для data-only предметов и content-specific branches in `main.ts`, `core/world.ts`, `render/webgl.ts`.
