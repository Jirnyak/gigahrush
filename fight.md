...
## Тип урона и матрица брони (отгружено 2026-08-27)

Раньше «чем ударили» и «чем защищён» были тремя разными языками. Теперь язык один: **тип урона —
колонка, вид защиты — строка, множитель — их пересечение.**

**`DamageType` (`src/core/types.ts`) — шесть членов:** `KINETIC`, `BUCKSHOT`, `ENERGY`, `FIRE`,
`PSI`, `BIO`. `BIO` добавлен этой правкой и покрывает кислоту, споры, слизь, гниль и газ ОДНИМ
членом, а не слоем химии поверх боя: до него слизневик с кислотной плетью бил ту же кинетику, что
и кувалда. Имя выбрано по ИСТОЧНИКУ (плоть, гриб, жижа), а не по эффекту — «токсин» не покрывает
споры, «химия» не покрывает слизь. Ненамеренный удар считается кинетикой; иначе броня молча
перестаёт работать.

**`ArmorType` (там же) — вид защиты, а не предмет и не тварь:** `NONE`, `CLOTH`, `PLATE`,
`CONCRETE`, `WET_HIDE`, `WIRING`, `LIVE_NET`. Условие (упор в стену, мокрый грунт, живая сеть,
целые плиты) решает, какая броня сейчас активна; один и тот же вид носят и предмет, и тварь.
Перечислено только то, что реально есть в игре, — ни одной записи под будущее.

**Матрица — `src/data/armor_matrix.ts`**, вход `armorMultiplier(armorType, damageType, impact)`.
Закон заполнения общий на всю таблицу: **колонка `KINETIC` в каждой строке оставлена такой, какой
была до наполнения.** Кинетика — то, чем в игре бьют почти всегда, старые плоские числа
настраивались против неё, и сдвинуть их значило бы менять уровень защиты под видом
избирательности. Остальные пять колонок разведены вокруг кинетики по физике материала: сталь
держит вещество и течёт по стыкам (`BIO 0.86`), бетон принимает плеск на себя (`BIO 0.62`), но
скол от луча летит в того, кто за ним (`ENERGY 0.82`); мокрая шкура гасит огонь (`FIRE 0.20`) и
проводит ток (`ENERGY 1.06`) — это её собственный контрплей, парный к живой сети Червия.

**ЗАКОН ПСИ: материальная броня не мешает пси** — `PSI = 1` у `PLATE`, `CONCRETE` и `WET_HIDE`.
Это прямое следствие того, что пси-дефазинг проводит СКВОЗЬ стены: если разум проходит сквозь
несущую панель, он тем более проходит сквозь то, что от неё откололи. Исключений два, и оба не
материал: живая сеть Червия (распределённый разум, `PSI 0.70`) и ряса культиста, пропитанная
специально — её резист живёт процентами предмета.

**Ось импульса `ArmorImpact` (`weak` / `heavy` / `tool` / `final`) ВЫВОДИТСЯ формулой, а не пишется
руками.** Она ортогональна типу и не сводится к нему: кувалда кинетическая, но плиту срывает, а
панические пули — нет; сегодня её различает одна броня — бронеплиты. Прежние три числа плиты
(0.28 → 0.68 → 0.92) оказались долями РАЗРЫВА до единицы: 0.72 сжимается в 0.32 и в 0.08, то есть
ровно в `4/9` и в `1/9`. Поэтому новых ручек в файле нет — обе доли выведены из старой тройки, а
не подобраны, и `weak` осталась исходной строкой. Формула работает и на уязвимости: строка больше
единицы так же стягивается к единице, а не разгоняется за неё.

**Носимая броня матрицу в бою НЕ читает.** Её проценты живут в `ItemDef.resistances` и вычитаются
из ста в `calculateDamage` — проценты И ЕСТЬ её строка, второй слой просто считал бы резист дважды.
`armorType` предмет всё равно объявляет: по нему работает ось удара и по нему предмет узнают
правила материала, у которых процентов нет.

**У тварей появился свой тип урона:** `MonsterDef.damageType`, объявлен у **31 вида из 73** (12 BIO,
12 PSI, 7 ENERGY). Молчание означает кинетику, и объявлять её вслух ЗАПРЕЩЕНО тестом — иначе
умолчание перестаёт быть умолчанием.

**Три ключа классификации сведены к одному.** Снят список `CHERVIE_ENERGY_WEAPONS` — «что считается
энергооружием» больше не пишется вторым перечнем. `ProjType.FLAME` теперь ЗНАЧИТ огонь через
`PROJ_TYPE_DAMAGE_TYPES` (`data/weapons.ts`; ствол без явного `damageType` берёт его от снаряда,
иначе от ролевого тира, иначе кинетика). Уязвимость растений к огню переехала в
`MonsterDef.damageFloor` — «этот тип гарантированно снимает такую долю максимума» — и работает
теперь **от любой руки**, а не только на пути снаряда игрока: Борщевик 0.44, Кровяной цвет 0.38,
Туманная акула 1.

## Среда входит в ту же дверь (отгружено 2026-08-27)

Кислота, споры, пар, обвал, разряд и пси-протокол больше не снимают здоровье сами. Тонкий вход
`damageActorByEnvironment` (`src/systems/actor_damage.ts`) называет то, что у среды одинаково
всегда — автора нет, толкать нечем, источник `environment`, — а тип, броню, толчок, смерть и добычу
делает то же ядро, что и удар чужой рукой. Сейчас через него проходят **33 вызова в 27 файлах**
`systems/` и `gen/`, каждый с назначенным типом.

Три правила, все снятые с дефекта:

- **Тип обязателен, умолчания нет.** «Не назвал» здесь значит «броня молча не сработала» — ровно
  та поломка, ради которой среду и вели к двери. ОЗК за 16 000 ₽ с био-защитой 70 не мешал кислоте
  ничем, пока кислота не знала, что она БИО.
- **Объявленный клеткой урон работает как ПОТОЛОК:** броня умеет только уменьшать. Конвейер умеет
  и увеличивать (`damageFloor`, строка матрицы больше единицы), но это про оружие в руках — там за
  ударом стоит выбор и патрон. Клетка тикает четырежды в секунду; без потолка Туманная акула
  умирала бы от первого такта парового сброса.
- **Среда бьёт всех, а не только игрока.** `tickHazardSubject` применял урон под условием «это
  игрок»: жилец на кислоте, в паре, на тепловой линии и под излучением только залипал и здоровья
  не терял. Вся средовая лестница брони существовала ровно для одного актора из двух тысяч. Замер
  после правки — смертность людей на бане 47.8 → 49.1 %, на коллекторах и поясе в шуме: у самой
  злой клетки в игре 2.2 урона в секунду, а первый же прошедший такт ставит `AIGoal.FLEE`.

Голод и жажда осознанно оставлены мимо двери: у них нет ни автора, ни типа, ни брони — это не удар,
а исход. Контракт целиком — `architecture.md`, «Damage Door Contract».

## Пять веток «игрок особенный» сняты (2026-08-27)

Закон «игрок — просто NPC» дошёл до боя. Каждая ветка проверена по коду, каждая доставалась
ТОЛЬКО игроку:

- **обратная тяга огнемёта** мерила расстояние ОТ ИГРОКА, а не от стрелка, поэтому тварь с
  огнемётом дышала своим пламенем безнаказанно; теперь обжигается любой стрелок вплотную к вспышке
  (ОГОНЬ через дверь), а личная строка в журнале остаётся игроку — журнал это его HUD;
- **давление самосбора вне рабочей гермы** шло мимо всякой брони; теперь ПСИ, и ряса культиста с
  пси-защитой 75 режет его вчетверо;
- **колокол Истотита** — тем же ПСИ и той же дверью;
- **запасной путь ПСИ** (`psiHit`) удалён ЦЕЛИКОМ вместе с причиной, по которой существовал: он
  разрывал цикл импортов `damage → psi`, из-за которого дверь приходила инъекцией
  `setPsiDamageSink` и считала броню не так, как настоящая. Флаг фазы уехал в лист `psi_state.ts`,
  цикла нет, инъекции нет, ронять урон молча стало нечему;
- **поезд** симметризован ВВЕРХ: было `isPlayerEntity(victim) ? 38 : 260`, стало 260 всем, кулдаун
  переезда общий и тратит его только ВЫЖИВШИЙ — иначе состав давил бы по одному из толпы.
  Симметризация вниз отвергнута замером: при 38 всем состав перестаёт давить кого-либо вообще,
  то есть механика исчезает, а спецслучай игрока просто становится законом.

Замок — `tests/player-privilege-damage-door.test.ts`, проверен на регрессии.
