...
- **оглушение пишет ОДИН писатель** — `applyHitStagger` (`systems/combat.ts`): оба поля
  (`staggerTimer` на сущности и его зеркало в `ai`), порог по доле HP, рефрактерный период
  длиной в сам стаггер. Отдача зовёт ту же дверь и считает силу толчка той же формулой.
  Ближний бой берёт только половину «боль»: толчка у него нет ни у игрока, ни у пира.
  До 2026-09-12 ответов было три — канон, плоские `0.15` в одно поле у руки пира и ничего
  у руки игрока;
- воюют ЛЮДИ, а не только нашивки: `isHostile` спрашивает сперва личное ребро графа Демоса к человеку, затем личное отношение к его стороне, и лишь потом глобальную матрицу (решение владельца 2026-09-12, отменяет обратное правило 2026-08-23). Помимо боя вражда меняет поведение — отказ помочь, избегание — и разряжается разборкой один на один (`systems/npc_feud.ts`), см. `manifest.md`. Монстры не дерутся с монстрами по умолчанию, потому что это одна экология;
