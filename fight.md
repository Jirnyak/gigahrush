...
- воюют фракции и личная вражда между людьми: `isHostile` после матрицы фракций читает направленное ребро социального графа Демоса (`isDemosPersonalEnemy`), и игрок в нём такой же человек, как остальные. Монстры не дерутся с монстрами по умолчанию, потому что это одна экология (личное ребро им и недоступно: у монстра нет `alifeId`);
