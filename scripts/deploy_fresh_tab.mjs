...
async function main() {
  console.log("=== DEPLOYING TO FRESH TAB ===");

  // 1. Find or create edit tab
  const res = await fetch("http://127.0.0.1:9222/json");
  const tabs = await res.json();
  let editTab = tabs.find(t => t.url && t.url.includes("itch.io/game/edit/4587160"));
  
  if (!editTab) {
    console.log("Creating new tab...");
    const createRes = await fetch("http://127.0.0.1:9222/json/new?https://itch.io/game/edit/4587160", { method: 'PUT' });
    editTab = await createRes.json();
    await new Promise(r => setTimeout(r, 4000));
  }

  console.log("Using tab:", editTab.id, editTab.webSocketDebuggerUrl);
  ...
  console.log("Connected to fresh tab WebSocket!");
  ...
  for (let i = 0; i < 10; i++) {
    try {
      const check = await send("Runtime.evaluate", {
        expression: "document.querySelector('form.edit_game_form') !== null",
        returnByValue: true
      });
