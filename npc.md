...
**Доставка на этаж — общая, а не забота модуля.** В конце генерации любого дизайн-этажа
`generateDesignFloor` вызывает `deliverFloorNpcPackages`: он берёт из реестра всех, кто объявил
этот этаж домом, и ставит тех, кого модуль не поставил сам. Модуль по-прежнему волен разместить
своего человека точно — в свою комнату, в свою позу, рядом со своей сценой, — и почти всегда так и
делает; общий шаг лишь не даёт забытой строчке спавна выкинуть персонажа из игры. `event_only`
пропускается: такой человек приходит событием, а не этажом. Комнату для добора подсказывает
`spawnRoomAlias` анкеты (ищется по тегам и имени комнаты), иначе берётся любая.

Следствие, которое надо держать в голове: **`homeFloorKey` теперь не подпись, а адрес доставки.**
Ошибиться в нём — значит поставить человека не туда, а не «нигде».

Безлюдный маршрутный этаж (`floorRunZAllowsNpcs` = нет) отменяет доставку целиком: там нет никого,
включая авторских, и личность под такое место заводить не надо. Поручение, которое некому выдать,
объявляет шаг цепочки без `giverId` — он появляется сам, как только закрыт предыдущий, и закрывается
делом. Так сделан финал в Пустоте.

**Объявлять этаж обязательно, и он должен совпадать с местом спавна.** `homeFloorKey` — это ключ
бакета в плане населения A-Life: по нему личность резервируется и по нему же проверяется
`floorKeyAllowsNpcs`. Регистрация без этажа не остаётся пустой, а молча уезжает в дефолт
`design:living` — так 160 пакетов из 473 объявляли жилой этаж, а жили на министерстве и в
коллекторах (закрыто 2026-08-16). Поэтому модуль контента зовёт `registerFloorSideQuest(KEY, ...)`
или передаёт `homeFloorKey` в `registerAuthoredNpc`, а `KEY` берёт из `designNpcFloorKey('<этаж>')`
константой на модуль. Механический замок — `tests/npc-home-floor.test.ts`: он генерирует все
дизайн-этажи и сверяет объявленный дом с фактическим этажом каждой сущности.
