...
- **Обнажилось ранее замаскированное:** пропажа обязательного `runSeed` в фикстурах перебивалась ошибкой про лишнее `baseFloor`. Закрыто там, где попадало в опасный срез; остальные три — класс частичных фикстур, и под него нужен тип-помощник в `tests/`, а не поля россыпью.
