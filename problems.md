...
| --- | --- | --- |
| Регистрация контента держится на нарушении слоёв | `systems/samosbor.ts` импортирует `generateFloor` из `gen/floor_manifest`, и это ребро делает две вещи сразу. Первая известна: оно замыкает рантайм-цикл на 293 файла, и убрать только его достаточно, чтобы цикл упал до 106 — остальные шесть обратных рёбер на цикл не влияют вовсе. Вторая скрыта: через него транзитивно грузятся 63 генератора этажей, которые побочным эффектом импорта регистрируют контент. Импорт одного самосбора даёт 464 пакета NPC; без ребра — 101. Замерено 2026-08-15: срез ребра убирает 261 модуль (252 из `gen/`) из транзитивного замыкания **421 тестового файла из 604**. Падают при этом только два (`demos-social`, `wrong-door`), остальные 419 проходят на урезанном реестре, и ручаться за то, что они всё ещё проверяют задуманное, нельзя. Правка откачена намеренно. | Регистрация контента становится явной, а не побочным эффектом транзитивного импорта: тест или точка входа, которой нужен реестр, загружает его сама. Смежное условие — числовые `plotNpcId` перестают быть позиционными индексами регистрации (см. пункт про резервные личности), иначе состав импортов продолжит двигать личности. После этого ребро `systems/samosbor → gen/floor_manifest` режется инъекцией генератора, и `npm run check:invariants` показывает крупнейший рантайм-цикл 106 вместо 293. |
