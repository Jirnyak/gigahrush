...
| **Речевые «либо-либо», требующие решения владельца (замерено 2026-08-24)** | Пять авторских механизмов дописаны до конца, но не подключены ни одной строкой, и каждый — это ВЫРЕЗ КОНТЕНТА, а не чистка: агент не вправе решать за автора. (1) `registerVoidSpiritTollChamber` (`systems/void_protocols.ts:123`) не зовёт ни один генератор Пустоты → протокол `spirit_toll` со всеми `obey/pay/break/reroute` (≈250 строк) недостижим. (2) `registerSamosborLocalShelter` (`systems/samosbor_hooks.ts:46`) не зовут нигде → семь циклов по локальным убежищам в `samosbor.ts` крутятся по пустому массиву, а `AGENTS.md` числит этот файл действующей точкой расширения. (3) `grantArenaChampionRewards` (`systems/arena_rewards.ts:9`) без вызывающих → Золотой Кубок (`arena_gold_trophy`) не выдаётся никогда, предмет без источника. (4) `publishItemTradeEvent` (`systems/inventory.ts:824`) без вызывающих → вместе с ним не работает единственное место, где считается конфискация говняка ликвидатором. (5) `noteObservedEventFact` / `formatRecentWitnessFactLine` (`systems/npc_memory.ts`) — канал «я видел» дописан вместе с TTL и лимитом, но `push` не случается ни разу, и реплика свидетеля всегда `undefined`. | По каждому пункту нужен выбор автора: подключить одной строкой или снять вместе с ветками. Промежуточное состояние дороже обоих: следующий агент читает механизм как работающий. |
| **Две страховки перестройки мира не подключены** | `pruneVolatileContainersForRebuild` (`systems/containers.ts:380`) и `pruneRouteCuesForVolatileRebuild` (`systems/route_cues.ts:378`) написаны как чистка временного состояния при перестройке этажа — снять ящики и маршрутные метки, оказавшиеся в новой геометрии не на месте, — но не вызываются НИ ОТКУДА (по одной ссылке на всё дерево, только объявление). Обе принимают `(world, z)` и возвращают число снятого, то есть автор их писал под конкретную точку вызова: стежку самосбора либо стрим этажа пиру. Пока их не звали, временные ящики и метки переживают перестройку в геометрии, которой уже нет. Это не мертвечина на снос: в отличие от снятых отладочных сводок, здесь есть наблюдаемое последствие. | Решение владельца: подключить обе в путь перестройки мира (там же, где `markWorldReplaced`, — он на пути всех шести замен) или снять как отменённый замысел. Проверяется замером: после стежки самосбора ни один контейнер и ни одна маршрутная метка не остаются на клетке, ставшей бетоном. |
...
| **Подсказки загрузки — вторая таблица читаемости монстров** | `MONSTER_TIPS` (`data/tips.ts:168`) — 66 записей с cue/tactic/mistake/loot по РУССКОМУ отображаемому имени, при том что те же поля живут в `MonsterEcologyDef` на 73 вида, и `monsters.md` фиксирует правило «читаемость берётся из экологии, копий нет». Копия уже разошлась: замерено 2026-08-24 — **17 имён подсказок не совпадают ни с одним видом** (расхождение по регистру и ё/е: «Дикий мертвяк» против «Дикий Мертвяк», «Споровый ковёр» против «Ковер», «Обживальщик» против «Комнатный обживальщик»), и **24 вида остались без подсказки вовсе**. Функционального сбоя сегодня нет — подсказка просто печатается текстом, — поэтому это не правится молча: перевод 66 авторских формулировок на экологию меняет текст, который видит игрок на каждой загрузке. | `addMonsterTips()` читает `MONSTER_ECOLOGY` (cue → `cue`, tactic/mistake → `counterplay`/`rule`, loot → `lootHint`, имя — из `MONSTERS`), таблица удаляется. Проверяется тем, что подсказка есть у каждого вида и ни одно имя не набрано руками. |
