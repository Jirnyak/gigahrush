...
### 4-Anchor Spanning Forest (Поиск пути на тороидальной топологии) [ВЫПОЛНЕНО]

**В чем проблема:**
Алгоритм поиска пути (`buildBakedTreePath`) использовал одно запеченное остовное дерево (Spanning Tree), построенное BFS-обходом от случайного корня, чтобы за `O(1)` в реальном времени находить путь через Lowest Common Ancestor (LCA). 
Так как игровой мир — это бесшовный тор (Torus), края карты закольцованы. Волны единственного BFS-дерева огибают тор и встречаются на противоположной стороне, образуя топологический "шов". Если две соседние клетки (например, `y=1023` и `y=0`) находились по разные стороны этого шва, алгоритм возвращал огромную прямую линию — путь шел через всю карту в обход к корню дерева, вместо того чтобы сделать 1 шаг. 

**Почему не A* / Локальный BFS:**
Запуск A* или BFS в реальном времени строго запрещен "Железным Законом" (`optimization.md`), так как сложность `O(W^2)` при массовых проверках сотен NPC обрушит FPS.

**Элегантное решение (4-корневой лес):**
- Вместо одного дерева, при загрузке карты запекаются **4 независимых остовных дерева** из противоположных квадрантов тора: `(0,0)`, `(512,0)`, `(0,512)`, `(512,512)`.
- Математически гарантировано, что швы не накладываются. Если для одного дерева клетки разделены швом, для другого они лежат прямо у корня.
- При запросе пути алгоритм за `0.001 мс` проверяет длину LCA-пути во всех 4-х деревьях и выбирает дерево с **минимальной длиной**.
- Итог: идеальный кратчайший путь через любую границу тора без единой аллокации или BFS-поиска в реальном времени. Массив `_navComponent` остался один (т.к. связность не зависит от корня), что сэкономило 16 МБ памяти.
- Итог: идеальный кратчайший путь через любую границу тора без единой аллокации или BFS-поиска в реальном времени. Массив `_navComponent` остался один (т.к. связность не зависит от корня), что сэкономило 16 МБ памяти.
