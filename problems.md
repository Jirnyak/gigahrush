...
| Верхний пояс маршрута без населения | `baseNpcTarget` глушит обычное население при `Math.abs(route.z) >= 44` (крыша 50, наружный микрорайон 48, чердак 46), хотя маршрутный контракт `floorRunZAllowsNpcs()` людей там разрешает — два источника истины расходятся. Для чердака это уже принято как замысел (`chthonic-attic.test.ts` ассертит `ambient === 0`), но «улицы, домики, тишина» наружного микрорайона с нулём жителей противоречат собственной роли этажа. Побочно: на этом поясе низкая авторская опасность не переселяет людей, а прореживает этаж (это задокументировано в коде как намеренное). | Решение владельца: либо пояс объявляется NPC-free явно (и `floorRunZAllowsNpcs` приводится к тому же), либо этажи с социальной ролью получают явный `npcTarget` (как уже сделал `radon_exchange`). |
| Точные счётчики в тестах генерации | Часть хвоста `test:generation` всё ещё пиннит микросостояние траектории ГПСЧ (точные равенства комнат/населения). Такие ассерты падают от любого контент-дрейфа и ничего не охраняют, зато прячут настоящие регрессии в шуме. Владелец одобрил перевод в инварианты; переведена большая часть, остаток — в оставшихся 29 падениях. | Ассерты выражают контракт: границы масштаба, законы сохранения (бюджет актёров, реестр дверей, связность от спавна), непустоту и монотонности; точные равенства остаются только там, где число само является контрактом. |
