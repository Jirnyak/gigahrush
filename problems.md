...
| --- | --- | --- |
| Регистрация контента — побочный эффект импорта | **Ребро срезано 2026-08-15, цикл 293 → 106; осталась сама привычка.** `systems/samosbor.ts` больше не импортирует `gen/floor_manifest`: генератор приходит инъекцией (`setSamosborFloorGenerator`), тип `FloorGeneration` идёт `import type` и при сборке стирается, точку сборки контента держит `src/content.ts`. Слоты сюжетных личностей заморожены списком `src/data/npc_plot_ids.ts` — числовой `plotNpcId` перестал быть индексом регистрации, и `getPlotNpcCount()` больше не дышит вместе с составом импортов (473 всегда, было 101..464). Остаточная проблема: контент по-прежнему регистрируется НА ИМПОРТЕ, а не по вызову. 439 вызовов `register*` живут на верхнем уровне модулей, и порядок их выполнения значим — он задаёт порядок `NPC_PACKAGES` и `SIDE_QUESTS`, а через него состав плана населения A-Life, поэтому очерёдность пришлось выписать в `content.ts` руками. **Замерено 2026-08-15, во что это обходится:** ленивый реестр этажей (43 пакета `gen/*` из 78 тянет ТОЛЬКО `design_floors/manifest`) убрал бы из стартового замыкания **288 модулей из 685** — но вместе с ними перестали бы срабатывать **317 вызовов `register*` в 101 файле**, а плану населения A-Life эти личности нужны на старте прогона. Из 101 файла **71 отделяется дёшево** (182 вызова: файл тянет не больше 12 лишних модулей и не тянет геометрию своего этажа), **30 запутаны** (135 вызовов). Второй, независимый блокиратор: генерация этажа синхронна насквозь — `scheduleLoading` в `main.ts` принимает синхронный колбэк, и `updateSamosbor` зовёт генератор прямо в кадре, — а ленивый реестр требует `import()`, то есть асинхронной генерации. | Порядок работ, не одна правка: (1) 71 дешёвый модуль контента переезжает под явный список `content.ts`, генератор этажа перестаёт быть условием регистрации; (2) 30 запутанных распутываются — объявление отдельно от геометрии; (3) генерация этажа становится асинхронной через шлюз загрузки; (4) только после этого реестр этажей делается ленивым. Проверяемо на каждом шаге: срез рёбер `design_floors/manifest` теряет всё меньше вызовов `register*`, счёт снимается скриптом. |
| Домашний этаж пакета NPC врёт у трети реестра | Замерено 2026-08-15 прогоном всех 51 дизайн-этажа и сверкой с `placement.homeFloorKey`: **160 пакетов из 473 объявляют не тот этаж, на котором спавнятся**. 156 из них объявляют `design:living` просто потому, что это значение по умолчанию в `normalizeNpcPackageInput`, а модуль контента звал `registerSideQuest` без этажа вместо `registerFloorSideQuest`. Крупнейшие группы: квартиры 53, коллекторы 49, министерство 31, ад 5. Ещё четыре — настоящие инверсии (`ag43_seva_cartographer` объявлен коллекторами, живёт в жилом; `batushka` наоборот; `hell → podad`; `black_market_88 → living`). Это не косметика: `src/data/alife_population_plan.ts:267` берёт `placement.homeFloorKey` как ключ бакета, а строка 252 считает по нему же `floorKeyAllowsNpcs`. Значит A-Life резервирует эти 160 личностей в бакете жилого этажа, а генератор ставит их на министерстве, в квартирах и в коллекторах — распределение населения по этажам перекошено на треть сюжетного пула, и Демос показывает этим людям неверное место. Отдельно: пять авторских пакетов с `presence: 'anchor'` не появляются НИ НА ОДНОМ дизайн-этаже — `liquidator_armorer`, `liquidator_medic`, `liquidator_quartermaster` (у них есть отдельные близнецы `liq_*`, которые спавнятся), `void_warning`, `olevia_kiber`. | Объявленный `homeFloorKey` совпадает с этажом, на котором пакет реально спавнится, у всех, кроме явно объяснённых исключений; проверяется прогоном генераторов, а не глазами. Правка механическая — модуль зовёт `registerFloorSideQuest(<этаж>, ...)` вместо `registerSideQuest(...)`, — но она двигает раздачу населения A-Life у 160 личностей, поэтому идёт отдельным заходом с проверкой численности по этажам, а не попутно. Пять неспавнящихся якорей разбираются отдельно: у них либо нет вызова спавна, либо их вытеснили близнецы. |
