...
| Тороидальный поиск пути (Spanning Tree LCA Flaw) | Алгоритм 4-корневого леса (`4-Anchor Spanning Forest`), использующий LCA для поиска пути в реальном времени, фундаментально сломан для открытых пространств. Остовное дерево (BFS) всегда разрывает циклы графа (образует невидимые "швы" / cuts). Если монстр и цель стоят по разные стороны такого шва, LCA-путь идет до самого корня дерева и обратно, пересекая границу тора. Сдвиг корней в 4 деревьях не спасает, так как в широких комнатах (как AI Arena) швы всех 4 деревьев могут пересекаться, и маршрут сходит с ума. Монстр доходит до цели только благодаря String Pulling (Line-of-Sight), но сам `ai.path` содержит мусор до `(0,0)`. | Полный отказ от LCA Spanning Tree для локальной навигации. Переход на Flow Fields / локальный кэшированный A* / Local Avoidance, чтобы маршрут не зависел от глобальных разрывов графа. |
...
### 4-Anchor Spanning Forest (Фундаментальная математическая ошибка) [ПЕРЕНЕСЕНО В ДОЛГ]

**Исторический контекст:**
Алгоритм поиска пути (`buildBakedTreePath`) использовал одно запеченное остовное дерево (Spanning Tree), построенное BFS-обходом, чтобы за `O(1)` в реальном времени находить путь через Lowest Common Ancestor (LCA). Так как игровой мир — это тор, края закольцованы, и волны BFS-дерева огибают тор, образуя топологический "шов". Если две соседние клетки находились по разные стороны этого шва, алгоритм возвращал огромную прямую линию в обход к корню дерева, пересекая весь тор.

**Попытка решения (4-корневой лес):**
При загрузке карты запекались 4 дерева из противоположных квадрантов тора: `(0,0)`, `(512,0)`, `(0,512)`, `(512,512)`. Ожидалось, что швы не наложатся друг на друга.

**Почему это провалилось (Текущий баг с уходом пути на край экрана):**
В открытых пространствах (например, в тестовой арене или широких залах) "швы" от корней расходятся во все стороны как лучи. Если стартовая и конечная точки находятся на противоположных сторонах такого луча (с точки зрения обхода графа), их наименьшим общим предком становится сам корень дерева!
В итоге для двух соседних точек на открытом пространстве (или снаружи/внутри спирали) **ВСЕ 4 дерева могут иметь разрыв графа между ними**. Путь `Start -> LCA -> End` идет через корень `(0,0)`, рисуя линию через весь экран к границе тора. Монстры в игре визуально не сходят с ума только потому, что их спасает алгоритм "сглаживания" пути (String Pulling по прямой видимости) в `followPath`, который просто "отрезает" этот мусорный хвост. Но сам `ai.path` генерируется абсолютно неверным.

**Вывод:**
LCA Pathfinding по остовному дереву **неприменим** для графов с циклами (и открытых 2D-сеток), так как никакое разумное количество смещенных деревьев не покроет все возможные "швы" графа. Решение должно быть пересмотрено (см. `Активные проблемы`).
...
2. **LCA Path Flapping:** При преследовании игрока (`HUNT`), алгоритм раз в 2 секунды полностью затирал `ai.path` и пересчитывал `buildBakedTreePath` до текущей клетки игрока. Если игрок не двигался, но двигался монстр, изменение дистанций до 4-х корней тороидального леса приводило к тому, что на одном шаге самым коротким оказывалось дерево 0 (путь обходит препятствие слева), а на следующем — дерево 1 (путь обходит препятствие справа). Путь постоянно менялся на противоположный.

**Элегантное решение:**
1. **Hysteresis в `tryAssignPathToCell`:** Добавлена проверка: если текущая субклетка цели совпадает с уже назначенной (`ai.tx, ai.ty`) и активный путь всё ещё валиден (`ai.path.length > 0`), алгоритм мгновенно возвращает `'same'` и не вызывает дорогой `buildBakedTreePath`, сохраняя выбранное дерево обхода.
...
### String-pulling Corner Clipping / Float Radius Conflict (Застревание на углах) [ВЫПОЛНЕНО]
