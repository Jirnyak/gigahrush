...
| Капли крови привязаны к камере, а не к миру | `renderParticlesGL` (`render/webgl.ts`) объявляет параметр высоты камеры как `_camHeight` и не использует его: экранная высота капли считается формулой `halfH + SCR_H/(tyf*2) − p.z*SCR_H/tyf`, где `0.5` — захардкоженная высота глаза. Ошибка по вертикали равна `(0.5 − высота камеры)·H/ty` и ходит вместе с покачиванием, приседанием и дыханием кадра. У живности этот же дефект чинён 2026-08-22 (`critters_pass.ts` получил `uCamHeight`); капли остались, потому что живут 0.2 секунды и промах на них не читается — но он там же. | Проход частиц получает `camHeight` и считает экранную высоту той же арифметикой, что спрайты (`footY` в `webgl.ts`), пол раскастера и теперь живность. Проверяется тем, что капля, брызнувшая на пол, не ездит по экрану при приседании. |
...
| Личная вражда доводит до смертельного боя | `isHostile` (`systems/factions.ts`) после матрицы фракций читает личный канал графа Демоса, и два жителя с отношением ниже −64 открывают огонь, хотя их фракции дружественны. Замерено на живом графе из 400 личностей: 86 враждебных рёбер из 2070, из них 26 между ДРУЖЕСТВЕННЫМИ фракциями. На коллекторах за 90 секунд без игрока и без монстров гибнет 136 NPC. Своих по фракции генерация врагами больше не делает (закрыто 2026-08-21), остаётся межфракционная утечка. | Решить, достаточно ли личной неприязни для смертельного боя. Варианты: отдельный, гораздо более низкий порог «кровной вражды»; либо запрет личному каналу перебивать дружественную связь фракций. Закрытие — прогон этажа без игрока не выедает население. |
