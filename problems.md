...
| A-Life: резервные личности подменяются числовым id | В bucket-форме `createPrefilledAlifeState` явно переданный числовой `plotNpcId` ПОДМЕНЯЕТ личность резерва: это индекс регистрации пакетов, зависящий от того, какие модули успели импортироваться, поэтому запрошенный Гордон Фримен становился Марко Лоло. Без `plotNpcId` конвейер берёт первую зарегистрированную личность вместо запрошенной по строковому `id` вида `npc:<пакет>`. Системность: числовые id пакетов — позиционные, а не стабильные, и любой новый импорт сдвигает их. | `buildAlifeStateFromPopulationPlan` связывает резерв строго по строковому `id`; числовой `plotNpcId` выводится из него и не принимается снаружи как ключ; `tests/demos.test.ts` зелёный без ручного подбора чисел. |
