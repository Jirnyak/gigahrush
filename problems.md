...
**Смежная находка, НЕ закрыта (2026-08-26).** В `tests/procedural-floors.test.ts` живут 19 фикстур с полем `baseFloor`, которого в `ProceduralFloorSpec` больше нет, и в 12 местах этой строкой (`'kvartiry'`, `'ministry'`, `'maintenance'`) выставляется `state.currentZ`, объявленный как `number`. Компилятор их не показал в срезе неверных вызовов только потому, что внутри `Partial<GameState>` это `TS2322`, а не `TS2345`. Это ровно та подмена строкового ключа числовым `z`, от которой в `AGENTS.md` есть отдельное предупреждение. Проверено на одном месте: вырожденным тест там не был (`normalizeFloorRunState` берёт `currentZ` из `floorRun`, а не из `state`), но полагаться на это нельзя. Критерий закрытия: ни одной строки в поле `currentZ` по всему `tests/`, проверяется `TS2322` в том же конфиге. Частичные фикстуры при этом трогать НЕ надо — под них заводится тип-помощник вида `Partial<Entity> & {id, x, y}` в `tests/`, а не пятьдесят полей в каждом объекте. |
