...
| Blind Z-coordinate refactoring regressions | При массовой замене `floor` на `z` (и истреблении `FloorLevel`/Z-координат) автозамены и ручные правки оставили множество поломок: в тестах (`samosbor-shelter.test.ts`, `sprites-floors.test.ts`, `hermodoor_borer.test.ts`) в параметры `z: number` передаются строки `'ministry'`, `'maintenance'`, `'living'`, `'hell'`, `'void'`, из-за чего проверка нечетности Z (`'maintenance' % 2 !== 0` → `NaN !== 0` → `true`) вызывает процедурную генерацию с `z = NaN` и краш `dangerBias` на `undefined`. В `src/` остались костыли вида `@ts-ignore` над `baseFloor: 100` (`floor_instances.ts`) и `themeClass` (`design_floors.ts`), а также некорректные замены ключей в реестрах (например `fractal_floor` → `fractal_z`). Так как `tests/` не включена в `tsconfig.json`, `npm run typecheck` не ловил передачу строк вместо чисел. | Все вызовы функций/тестов с параметром `z` или `currentZ` принимают строгие числовые Z-координаты; ошибочные `@ts-ignore` и `baseFloor`/`themeClass` артефакты очищены; `npm run test:generation` и `npm run test:unit` проходят без `TypeError` и `NaN`. |
