...
**Остаток класса (открыт).** По переписи в `AIState` ещё 16 видовых кластеров. Ценные (не сносить, а выселить состояние в `WeakMap`): Лишенный `light*` — работает на реальных данных мира и даёт настоящий контрплей. Требуют решения автора: Мухожук `parasite*` — снос осиротит два замороженных сюжетных слота и карантинный квест. Заблокированы чужими зонами: Туманник `fogOffset*` (читает `render/webgl.ts`), Слизевик `slime*` (упирается в одну строку литерала `ai` в `src/gen/maintenance/safe_slimevik_den.ts:101`), Безэхий `deadEcho*`. Плюс виды со своими блоками в общем файле без полей ядра: Жорная тварь (335 строк), Слепоглаз (252), Трескотник (230), Клинковый элитник (227), Зелёный пёс (212).

**Мёртвые тексты после сноса.** В `src/data/rumors.ts` живы слухи `ecology_lozhnyy_dukh_door`, `monster_glubinnaya_ten_second_beat`, `ecology_glubinnaya_ten_afterimage`, в `monster_ecology.ts` — cue второго темпа Глубинной Тени. Они описывают поведение, которого больше нет.
