...
| --- | --- | --- |
| Регистрация контента — побочный эффект импорта | **Ребро срезано 2026-08-15, цикл 293 → 106; осталась сама привычка.** `systems/samosbor.ts` больше не импортирует `gen/floor_manifest`: генератор приходит инъекцией (`setSamosborFloorGenerator`), тип `FloorGeneration` идёт `import type` и при сборке стирается, точку сборки контента держит `src/content.ts`. Слоты сюжетных личностей заморожены списком `src/data/npc_plot_ids.ts` — числовой `plotNpcId` перестал быть индексом регистрации, и `getPlotNpcCount()` больше не дышит вместе с составом импортов (473 всегда, было 101..464). Остаточная проблема: контент по-прежнему регистрируется НА ИМПОРТЕ, а не по вызову. Из-за этого нельзя сделать реестр этажей ленивым — отложив загрузку генератора, отложишь и объявление его NPC и квестов, а плану населения A-Life они нужны на старте прогона. Из-за этого же 439 вызовов `register*` живут на верхнем уровне модулей, а порядок их выполнения значим: он задаёт порядок `NPC_PACKAGES` и `SIDE_QUESTS` и через него состав плана населения, поэтому очерёдность пришлось выписать в `content.ts` руками. | Регистрация становится вызовом, а не импортом: модуль экспортирует `registerXContent()`, а собирающий её список — данные, а не граф импортов. Проверяемо тем, что реестр этажей можно сделать ленивым, не потеряв ни одного пакета NPC в плане населения, и что `content.ts` перестаёт зависеть от порядка своих строк. |
