...
export function isHostile(attacker: Entity, target: Entity): boolean {
  ...
  /* Решают ЛИЧНЫЕ отношения, и у них два канала — к человеку и к его стороне.
   *
   * Личное к ЧЕЛОВЕКУ сильнее: ненавидящий конкретного человека враждебен ему,
   * даже когда их фракции в мире. Это решение владельца от 2026-09-12, и оно
   * ОТМЕНЯЕТ прежнее «личная неприязнь боевой целью не делает» (2026-08-23).
   * Цена прежнего решения была в том, что вражда между людьми ничего не стоила
   * жизни; цена нового замерена — см. `problems.md`.
   *
   * Личное к ФРАКЦИИ идёт следом и тоже направленное: житель может считать
   * врагом ликвидатора, которому сам врагом не кажется. Глобальная матрица —
   * последняя, только когда личной ячейки нет вовсе. */
  if (isPersonalFeudEnemy(attacker, target)) return true;
