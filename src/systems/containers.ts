 ...
 *
 * Улики спрашиваются ОБЩЕЙ меткой `evidence` — решение владельца 2026-09-12
 * расширило набор с 13 названных бумаг до правила. Ловушка этого расширения
 * была замерена: семь из тринадцати названных бумаг `evidence` не носили, и
 * «просто спросить общую метку» потеряло бы их; метка им дописана. Саботаж и
 * заслон остались своими наборами — общей метки с тем же смыслом у них нет.
 *
 * Спрашивать метку обязательно через `itemIdHasTag`: метки живут в ДВУХ местах
 * (`ITEM_TAGS` и поле `tags` определения), и чтение одного даёт пустой список. */
function isEvidenceItem(defId: string): boolean {
  return itemIdHasTag(defId, 'evidence');
}
