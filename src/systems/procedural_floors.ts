...
import { LiftDirection, MonsterKind, type GameState } from '../core/types';
import {
  FLOOR_ANOMALIES,
  FLOOR_GEOMETRIES,
...
function floorRunEntryKey(entry: Pick<FloorRunEntry, 'z' | 'designFloorId' | 'spec'>): string {
  ...
  return floorKeyForDesign(String(entry.z));
}
...
export function isCurrentStoryFloor(state: GameState, z: number): boolean {
...
export function floorRunEntryKindLabel(entry: FloorRunEntry): string {
  ...
  if (entry.designFloorId) return 'маршрут';
  return 'архивный этаж';
}
...
export function summarizeFloorRun(state: GameState): string[] {
  ...
  if (entry.designFloorId) out.push(`route=${entry.designFloorId}`);
  return out;
}
