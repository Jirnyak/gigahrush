 ...
 * по потолку текущей клетки, отступая на тот же зазор, каким камера обходит
 * стену. Под открытым небом ограничивать нечем.
 *
 * Отступ раньше равнялся амплитуде покачивания (0.15 = 0.54 м). Этого хватало,
 * чтобы «дыхание» кадра не пробило плиту, но зазором это не было: кадр шёл
 * вплотную под перекрытием и терял обзор. На Перевалке весь дальний проезд по
 * грузовым авеню (потолок 2.0, заказ такта тоже 2.0) вёлся в полуметре от плиты.
 * Камера — тело с радиусом, а не точка, и по вертикали этот радиус тот же, что
 * по горизонтали: единицы совпадают буквально, клетка и единица высоты — обе
 * 3.6 м. Дыхание меньше зазора, поэтому инвариант «покачивание не пробивает
 * плиту» держится по-прежнему.
 *
 ...
 * около 0.66, то есть примерно два с половиной метра над полом.
 */
function ceilingLimitedHeight(world: World, x: number, y: number, wanted: number): number {
  ...
  const ceiling = cellCeilingHeight(world, cx, cy) - CAMERA_WALL_CLEARANCE;
