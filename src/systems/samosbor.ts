...
export function getSamosborWarningSnapshot(state?: GameState): SamosborWarningSnapshot | null {
  ...
  return {
    z: samosborWarning.z,
    floorName: floorDisplayNameForZ(samosborWarning.z),
...
function ensureSamosborWarning(
  ...
  const variant = getActiveSamosborVariant() ?? chooseSamosborVariant();
...
export function updateSamosbor(
   ...
   * оказаться в новом бетоне — тогда поводок поста держит говорящего в стене,
   * а камера уходит в вынужденный прямой ход.
   *
...
function applyPendingSamosborAftermath(
  ...
  knownSamosborTime = state.time;
  const defs = getSamosborAftermathBeats(pending.variant.def.id);
  const target = 1;
...
export function getSamosborDebugLines(): string[] {
  ...
  for (const def of getSamosborAftermathBeats(last ?? 'classic')) {
    const rt = aftermathRuntime.get(def.id);
...
function captureZone(
  ...
  const markHellMeat = world.hasMeatWalls &&
