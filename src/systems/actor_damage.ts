 ...
 *
 * Флаг снимается ВСЕМ, включая игрока, и порядок «сначала флаг, потом
 * обработчик» здесь не деталь, а контракт: ПСИ-щит внутри обработчика поднимает
 * `alive` обратно (`absorbPsiShieldDamage`), поэтому поглощённый удар не
 * оставляет за собой мёртвого игрока, а непоглощённый — живого.
 *
 * Раньше игрок здесь пропускался, и его смерть объявлял каждый бьющий сам. Пути
 * без своего `killEntity` не могли убить его вовсе: состав на рельсах — ЕДИНСТВЕННЫЙ
 * средовой урон с `lethal: true` — снимал 260 и оставлял игрока живым на нуле,
 * а `damageEntity` по `victim.alive` не ставил кулдаун и молол его каждый кадр.
 *
 * Здоровье прижимается к нулю здесь же: перелёт ниже нуля печатался в HUD и
 * удорожал щит на величину оверкилла, и два пути из трёх гасили его руками.
 */
export function finishActorDeath(target: Entity, attacker: Entity | undefined, input: ActorDamageInput): void {
  killEntity(target);
  if ((target.hp ?? 0) < 0) target.hp = 0;
