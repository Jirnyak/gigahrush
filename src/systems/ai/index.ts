 ...
 *
 * Границы захвата — про то, чтобы не отменять чужой начатый ход, а не про вид
 * актора:
 *
 ...
 *     ходока. Замерено на жилом этаже (сид 20881, `tmp/goto_probe.ts`): курс
 *     `GOTO` при этом уцелевал, а заказанная точка подменялась стратегической
 *     целью драйва, и человек уходил от места назначения — за тридцать секунд
 *     на 9 клеток ДАЛЬШЕ, чем стоял вначале. Отказ общий на человека и тварь:
 *     исполнитель у приказа один, и видовой развилки у него нет. Тот же отказ,
 *     по тому же признаку и по той же причине, стоит и в `runActorTactic`
 *     (`ai/tactics.ts`) — это один список границ захвата, а не два.
 *
...
export function updateAI(world: World, entities: Entity[], dt: number, time: number, msgs: Msg[], playerId: number, clock: GameClock, samosborActive: boolean, nextId: { v: number }, currentZ?: number, state?: GameState): void {
  ...
  try {
    for (const e of entityIndex.ai) {
      ...
      /* Хозяин ушёл в чужое тело — это остаётся БЕЗВОЛЬНЫМ. Не думает, не идёт,
       * не защищается, и убить его могут, пока хозяина нет. Один закон на всех:
       * тело игрока во вселении лежит ровно так же, как тело NPC. Проверка
       * O(1) по зеркальному полю — искать хозяина перебором этажа нельзя. */
      if (e.psiAway !== undefined) {
        aiStats.skipped++;
        continue;
      }
