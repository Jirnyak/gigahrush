...
export type MonsterAIFlag =
  ...
  | 'webSpitter'
  | 'flying'
  /* Проходит сквозь материю: и мебель, и стены, и взгляд. Один флаг на всё
   * бесплотное — духи ходят сквозь стены, и делить это надвое нечего. */
  | 'noclip'
  | 'wetLineShot'
  | 'packHowl'
  | 'noiseFear'
  ...
  | 'sided'
  /* Свойства видов, которые до 2026-09-09 ядро гейтило ИМЕНЕМ вида в двух
   * `switch`-ах множителей хода и урона. Флаг здесь не украшение: без него
   * новый монстр не может взять готовую механику. */
  /** Свет мешает: на свету медленнее и слабее, в темноте быстрее и сильнее. */
  | 'lightShy'
  /** Толпа и дверное горло усиливают удар. */
  | 'crowdPressure'
  /** Бьёт сильнее, стоя в своей «убойной клетке» или когда в ней цель. */
  | 'killCellPressure'
  /** Бьёт сильнее в спину и заметно слабее в лицо. */
  | 'backstab'
  /** Мокрая клетка делает собственный выстрел опаснее для самого стрелка. */
  | 'wetShotRisk'
  /** Копит давление за долгий бой в одной комнате и срывается тяжёлым ударом. */
  | 'roomPressure'
  /** Первый взгляд на цель печатает строку читаемости и событие. */
  | 'firstSightCue'
  /** Броня закалки копит стаггер и сбрасывает его своим тактом. */
  | 'temperedArmor'
  /** Ведёт хор: держит подчинённых и поёт такт вокруг себя. */
  | 'choirLead'
  /** Источник приплода: порождает и держит потомство капом. */
  | 'broodSource';
