...
export const DEF: MonsterDef = {
  ...
  /* Строка семьи замаха. Длина замаха здесь БАЗОВАЯ (тёмная цель): при жёстком
   * световом захвате вид укорачивает её сам, это его собственное свойство. */
  windup: {
    windupSec: 0.95,
    range: 17,
    minRange: 0.9,
    color: '#fd6',
    tag: 'lampoglaz',
    warningLine: 'Лампоглаз держит световую линию. Желтый гул кончится зеленым выстрелом.',
    windupLine: 'Лампоглаз собирает зеленую точку по светлой полосе. Срывайте линию.',
    interruptLine: 'Зеленый захват Лампоглаза сорвался о геометрию.',
    counterplay: 'darkness_or_line_break',
  },
};

/* ── Световой захват: числа вида живут у вида ─────────────────────
 *
 * Пять констант стояли в теле общего боевого AI и назывались по виду — та же
 * недопись, из-за которой числа второго удара Слепоглаза жили не у Слепоглаза.
 * Свет для Лампоглаза не обстановка, а прицел: две ступени света дают две
 * длины замаха и два множителя урона. */

/** Свет цели, с которого захват вообще возможен. */
export const LIGHT_LOCK = 0.24;
/** Свет цели, с которого захват становится точным: лампа в упор. */
export const HARD_LIGHT_LOCK = 0.52;
/** Замах при точном захвате. Базовый (тёмная цель) — в строке `windup`. */
export const HARD_LOCK_WINDUP_SEC = 0.58;
/** Множители урона выстрела на двух ступенях захвата. */
export const LOCK_DMG_MULT = 1.28;
export const HARD_LOCK_DMG_MULT = 1.72;
