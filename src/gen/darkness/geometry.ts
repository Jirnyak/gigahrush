...
export type DarknessQuestChoice =
  | 'spend_light'
  | 'save_light'
  | 'preserve_name'
  | 'leave_name'
  ...
  | 'long_route'
  | 'carry_trace';
export type DarknessTopologyDecision =
  | DarknessQuestChoice
  | 'listen'
  | 'follow_protocol'
  | 'flee'
...
export interface DarknessRoomSpec {
  key: string;
  type: RoomType;
  x: number;
  y: number;
...
export interface DarknessSupportRoomSpec {
  type: RoomType;
  name: string;
  dx: number;
  dy: number;
...
export interface DarknessHqCompoundSpec {
...
export interface DarknessStationSpec {
  cx: number;
  cy: number;
  name: string;
  fog: number;
...
export const DARKNESS_STATIONS: readonly DarknessStationSpec[] = [
...
export const ROOM_SPECS: readonly DarknessRoomSpec[] = [
  {
    key: 'entry',
...
export const LIGHT_GRAPH_EDGE_SPECS: readonly {
  fromKey: string;
  toKey: string;
  lightCost: number;
...
export const SOUND_PATH_SPECS: readonly {
  id: string;
  fromKey: string;
  toKey: string;
  cueId: string;
...
export const RADON_SIGHT_SPECS: readonly {
  id: string;
  fromKey: string;
  toKey: string;
  fog: number;
...
export const QUESTS: readonly DarknessQuestDef[] = [
  {
...
export const darknessStateByWorld = new WeakMap<World, DarknessFloorState>();

export interface DarknessBfsScratch {
  visited: Uint8Array;
  queue: Int32Array;
  depth: Uint16Array;
  parent: Int32Array;
...
export interface DarknessRadonBuildResult {
...
export function centerX(room: Room): number {
  return worldWrap(room.x + (room.w >> 1));
}

export function centerY(room: Room): number {
  return worldWrap(room.y + (room.h >> 1));
}

export function worldWrap(v: number): number {
  return ((v % W) + W) % W;
}

export function darknessRoomSpecByKey(key: string): DarknessRoomSpec {
  const spec = ROOM_SPECS.find(item => item.key === key);
  if (!spec) throw new Error(`Unknown darkness room key: ${key}`);
  return spec;
}

export function makeDarknessBfsScratch(): DarknessBfsScratch {
  const n = W * W;
...
export function resetDarknessBfsScratch(scratch: DarknessBfsScratch): void {
...
export function darknessGraphWalkable(world: World, ci: number): boolean {
  const cell = world.cells[ci];
  return cell === Cell.FLOOR || cell === Cell.WATER || cell === Cell.DOOR;
}

export function darknessRevealRadius(spec: DarknessRoomSpec): number {
...
export function buildDarknessLightGraphEdges(roomsByKey: Map<string, Room>): DarknessLightGraphEdge[] {
...
export function buildDarknessRevealShell(
  world: World,
  room: Room,
...
export function findDarknessPathCells(
  world: World,
  fromRoom: Room,
  toRoom: Room,
  scratch: DarknessBfsScratch,
...
export function averageFogForCells(world: World, cells: readonly number[]): number {
...
export function buildDarknessSoundPath(
  world: World,
  roomsByKey: Map<string, Room>,
...
export function buildDarknessRadonSightCorridor(
  world: World,
  roomsByKey: Map<string, Room>,
...
export function buildDarknessTopologyPlan(
  world: World,
  roomsByKey: Map<string, Room>,
  labels: readonly DarknessRoomLabel[],
...
export function placeRoomLights(world: World, room: Room, lamps: readonly [number, number, Feature][] | undefined): void {
  if (!lamps) return;
...
export function setFloorFeature(world: World, x: number, y: number, feature: Feature): void {
  const ci = world.idx(x, y);
  if (world.cells[ci] === Cell.FLOOR) world.features[ci] = feature;
}

export function setInteriorWall(world: World, room: Room, dx: number, dy: number, wallTex = Tex.VOID_WALL): void {
  const ci = world.idx(room.x + dx, room.y + dy);
...
export function decorateDarknessRoom(world: World, room: Room, key: string): void {
  if (key === 'generator') {
...
export function connectRoomCenters(world: World, a: Room, b: Room): void {
  carveCorridor(world, centerX(a), centerY(a), centerX(b), centerY(b));
}

export function carveDarknessDisc(
  world: World,
  cx: number,
  cy: number,
  r: number,
...
export function carveDarknessPathCells(
  world: World,
  ax: number,
  ay: number,
  bx: number,
...
export function carveDarknessPath(world: World, a: Room, b: Room, radius: number, fog: number, floorTex = Tex.F_VOID): void {
  carveDarknessPathCells(world, centerX(a), centerY(a), centerX(b), centerY(b), radius, fog, floorTex);
}

export function softenFogDisc(world: World, cx: number, cy: number, r: number, fog: number): void {
  const r2 = r * r;
  for (let dy = -r; dy <= r; dy++) {
    for (let dx = -r; dx <= r; dx++) {
      if (dx * dx + dy * dy > r2) continue;
...
export function markLightIsland(world: World, room: Room, radius: number, fog: number): void {
  softenFogDisc(world, centerX(room), centerY(room), radius, fog);
}

export function addDeadLampRow(world: World, a: Room, b: Room): void {
  const ax = centerX(a);
  const ay = centerY(a);
...
export function applyLightRouteGeometry(world: World, roomsByKey: Map<string, Room>): void {
...
export function setDoorStates(world: World): void {
...
export function paintCorridors(world: World): void {
  for (let i = 0; i < W * W; i++) {
...
export function addLift(world: World, x: number, y: number, direction: LiftDirection): void {
  const ci = world.idx(x, y);
...
export function darknessMonsterAngle(id: number, x: number, y: number): number {
...
export function buildRooms(world: World): { roomsByKey: Map<string, Room>; labels: DarknessRoomLabel[] } {
  ...
  expandDarknessRouteGeometry(world, [], rngFn);
...
export function placeContent(world: World, entities: Entity[], nextId: { v: number }, roomsByKey: Map<string, Room>): void {
  const entry = roomsByKey.get('entry')!;
  const lamp = roomsByKey.get('lamp')!;
  const generator = roomsByKey.get('generator')!;
  const name = roomsByKey.get('name')!;
...
export function applyDarknessZones(world: World): void {
  generateZones(world);
...
export function initialState(labels: DarknessRoomLabel[], topology: DarknessTopologyPlan): DarknessFloorState {
  return {
...
export function darknessRoomByKey(world: World, key: string): Room | null {
  const idx = ROOM_SPECS.findIndex(spec => spec.key === key);
  return idx >= 0 ? world.rooms[idx] ?? null : null;
}

export function carveLightPocket(world: World, cx: number, cy: number, radius: number, fog: number, lamp: Feature): void {
...
export function canStampDarknessExpandedRoom(world: World, x: number, y: number, w: number, h: number): boolean {
...
export function stampDarknessExpandedRoom(
  world: World,
  type: RoomType,
  x: number,
  y: number,
...
export function paintDarknessRoomOwner(world: World, room: Room, owner: TerritoryOwner): void {
...
export function decorateDarknessSupportRoom(world: World, room: Room, feature: Feature): void {
...
export function hardenDarknessHqRoom(world: World, room: Room, owner: TerritoryOwner): void {
...
export function reinforceDarknessAuthoredHqTerritory(world: World): void {
  ...
  for (const room of world.rooms) {
    if (room.name) roomMap.set(room.name, room);
  }

  for (const compound of DARKNESS_HQ_COMPOUNDS) {
    const hq = roomMap.get(compound.hqName);
    if (!hq) continue;
    ...
    for (const support of compound.support) {
      const room = roomMap.get(support.name);
      if (!room) continue;
...
export function buildDarknessHqCompound(world: World, spec: DarknessHqCompoundSpec): Room | null {
...
export function buildDarknessStationBlock(world: World, spec: DarknessStationSpec, ordinal: number, rng: () => number): Room | null {
...
export function widenDarknessRoute(world: World, a: Room, b: Room, radius: number, fog: number): void {
...
export function retouchDarknessExpansion(world: World): void {
  for (let i = 0; i < W * W; i++) {
...
export function expandDarknessMidMicroLayer(world: World, rng: () => number): void {
