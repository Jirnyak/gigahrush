...
const WITNESS: PlotNpcDef = {
  ...
  talkLines: [
    'Там двор. Только солнца нет, и окна соседей стоят спиной.',
    'Не закрывай сразу. Я почти вспомнила, как туда выйти.',
    'Белое не светит. Оно просто забирает цвет с рук.',
    'Если оттащишь меня - я буду злиться. Если не оттащишь - меня потом не будет кому злиться.',
  ],
  talkLinesPost: [
    'Я стояла у окна и думала, что меня зовут по имени. Теперь не помню, как меня зовут снаружи.',
    'Если кто спросит, окна не было. Но песок под ногтями есть.',
    'Заклейте белое без меня. Я больше не держу его взглядом.',
  ],
...
registerSideQuest(WITNESS_ID, WITNESS, [
  {
    ...
    type: QuestType.TALK,
    desc: 'Лида Белооконная: «Оттащи меня от белого окна. Если я скажу, что там двор, не отвечай.»',
  ...
  },
  {
    ...
    desc: 'Лида Белооконная: «Белый обход справа от окна ведет короче обычного коридора. Проверь его и не называй это выходом.»',
    ...
    targetHint: 'Жилая зона: белый проход рядом с комнатой окна, где песок лежит не у стены, а у наружи.',
    eventTargetName: 'Игрок прошел белый обход Веретара и оставил свидетеля спорить с окном.',
    eventSeverity: 4,
    eventPrivacy: 'local',
