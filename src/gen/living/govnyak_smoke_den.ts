...
const TROFIM: PlotNpcDef = {
  ...
  talkLines: [
    'Тут не отдыхают. Тут платят за пять минут, когда стены звучат дальше.',
...
function spawnNpc(
  ...
  const def = plotNpcId === 'ag97_trofim_dymar' ? TROFIM
    : plotNpcId === 'ag97_pavel_podzalog' ? PAVEL
      : plotNpcId === 'ag97_klava_witness' ? KLAVA
        : plotNpcId === 'ag97_markov_report' ? MARKOV
          : RITA;
  const x = world.wrap(room.x + dx);
  const y = world.wrap(room.y + dy);
  const npc: Entity = {
