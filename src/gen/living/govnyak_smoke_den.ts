...
const QUEST_OUTCOMES: Record<string, DenOutcome> = {
  ag97_buy_cash: {
    ...
    itemId: 'govnyak_roll',
    itemCount: 3,
    economyDeltas: [{ resourceId: 'contraband', count: -3, z: number.LIVING }],
  },
  ag97_settle_debt: {
    ...
    itemCount: 1,
    economyDeltas: [
      { resourceId: 'contraband', count: -1, z: number.LIVING },
      { resourceId: 'documents', count: 1, z: number.LIVING },
    ],
  },
  ag97_refuse_credit: {
    ...
    severity: 4,
    privacy: 'local',
    economyDeltas: [{ resourceId: 'contraband', count: -1, z: number.LIVING }],
  },
  ag97_report_den: {
    ...
    itemCount: 1,
    economyDeltas: [
      { resourceId: 'contraband', count: -5, z: number.LIVING },
      { resourceId: 'documents', count: 1, z: number.MINISTRY },
    ],
  },
  ag97_turn_dealer_science: {
    ...
    itemCount: 1,
    economyDeltas: [
      { resourceId: 'contraband', count: -4, z: number.LIVING },
      { resourceId: 'slime_samples', count: 1, z: number.LIVING },
    ],
  },
};
...
function economyDeltaSummary(outcome: DenOutcome): string[] {
  ...
  for (const delta of outcome.economyDeltas) {
    out.push(`${delta.z}:${delta.resourceId}${delta.count >= 0 ? '+' : ''}${delta.count}`);
  }
  return out;
}
...
function applyDenEconomy(state: GameState, outcome: DenOutcome, event: WorldEvent): string[] {
  ...
  for (const delta of outcome.economyDeltas) {
    if (!changeResourceStock(state, delta.resourceId, delta.count, delta.z, {
      zoneId: event.zoneId,
      roomId: event.roomId,
      reason: `govnyak_den_${outcome.outcome}`,
      tags: [MODULE_TAG, 'govnyak', 'contraband', outcome.outcome],
    })) continue;
    applied.push(`${delta.z}:${delta.resourceId}${delta.count >= 0 ? '+' : ''}${delta.count}`);
  }
  return applied;
}
...
function handleGovnyakOutcome(state: GameState, event: WorldEvent): void {
  ...
  const economyDeltas = changeResourceStock(state, 'contraband', -1, number.LIVING, {
    zoneId: event.zoneId,
    roomId: event.roomId,
    reason: 'govnyak_den_theft',
    tags: [MODULE_TAG, 'govnyak', 'contraband', 'theft'],
  }) ? [`${number.LIVING}:contraband-1`] : [];
  publishEvent(state, {
    type: 'faction_relation_changed',
    z: number.LIVING,
    zoneId: event.zoneId,
