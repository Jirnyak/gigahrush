...
const NPC_DEFS: Record<string, PlotNpcDef> = {
  [VICTIM_ID]: {
    ...
    talkLines: [
      'Тише. Там белее, чем лампа.',
      'Если смотреть сбоку, оно почти не зовёт.',
      'Я сейчас отойду. Сейчас. Только дослушаю стену.',
      'Рука холодная не моя. Моя бы давно ушла.',
    ],
    talkLinesPost: [
      'Меня отвели от пятна. Теперь тишина не такая ровная.',
  ...
  [NEIGHBOR_ID]: {
    ...
    talkLines: [
      'Тоня стоит у белого следа и отвечает не мне.',
    ...
    talkLinesPost: [
      ...
      'Спасибо. Белое молчит хуже крика.',
    ],
  },
  ...
  [LIQUIDATOR_ID]: {
    ...
    talkLines: [
      'Дверь должна закрыться до того, как комната убедит ещё кого-нибудь.',
    ...
    talkLinesPost: [
      'Шов принял герметик. Комната всё ещё зовёт, но теперь через дверь.',
  ...
  [SCIENTIST_ID]: {
    ...
    talkLines: [
      'Белый остаток не берут пальцами. Даже если он делает вид, что это ваша идея.',
    ...
    talkLinesPost: [
      'Образец закрыт. Он не стучит, только ждёт, когда о нём вспомнят.',
      'Я отнесу пробу Якову. В отчёте напишу меньше, чем видел.',
    ],
  },
  ...
  [WATCHER_ID]: {
    ...
    talkLines: [
      'Можно ничего не делать. Домком любит такие решения: они уже похожи на акт.',
    ...
    talkLinesPost: [
      'Акт принят. Тоня теперь строка в журнале и шорох за дверью.',
      'Белые пятна любят бумагу. На ней легче стоять.',
    ],
  },
};
...
registerSideQuest(NEIGHBOR_ID, NPC_DEFS[NEIGHBOR_ID], [
  {
    ...
    type: QuestType.TALK,
    desc: 'Даша У Порога: «Подойди к Тоне и выведи её от белого остатка. Говори тихо, не смотри в пятно дольше вдоха.»',
...
registerSideQuest(LIQUIDATOR_ID, NPC_DEFS[LIQUIDATOR_ID], [
  {
    ...
    desc: 'Клим Пломба: «Один тюбик герметика - и я закрою белую комнату по шву. Без герметика это просто дверь с мнением.»',
