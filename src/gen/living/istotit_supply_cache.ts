...
const NPC_DEFS: Record<string, PlotNpcDef> = {
  [AGAFA_ID]: {
    ...
    talkLines: [
      ...
      'Во время Истотита люди становятся набожными быстрее, чем честными.',
    ],
    talkLinesPost: [
      'Воду разнесли. Теперь хоть один спор начнется после отбоя, а не перед ним.',
      'Свечу держи низко. Золотой свет и так слишком хорошо считает лица.',
    ],
  },
  [SAVVA_ID]: {
    name: 'Савва Дверной',
    ...
    talkLines: [
      'Я у двери стою не за веру. Чтобы ящик не вынесли вместе с дверью.',
      'Сборка за стеной уже нюхала воск. Отгони ее - люди поверят, что запас еще общий.',
      'Когда хор идет по трубе, охрана - это не поза. Это кто первым слышит когти.',
    ],
    talkLinesPost: [
  ...
  [MARKEL_ID]: {
    ...
    talkLines: [
      'Общий запас без ведомости - это будущая драка с иконой на стене.',
  ...
  [LIDA_ID]: {
    ...
    talkLines: [
      ...
      'Сургуч принесешь, я подпишу огарок как вклад. Смешно, пока дверь не слушает.',
      'Я не торгую благодатью. Я меняю расходник на расходник, пока все делают вид, что это обряд.',
    ],
    talkLinesPost: [
      'Сургуч пошел на пломбу. Свечу держи, но не обещай ею никого спасать.',
      'Бартер честнее молитвы, когда у молитвы есть очередь.',
    ],
  },
};
...
registerSideQuest(AGAFA_ID, NPC_DEFS[AGAFA_ID], [{
  ...
  desc: 'Агафья Свечная: «Принеси две бутылки воды в общий свечной запас. Раздадим у двери тем, кого хор застал без фляги.»',
...
registerSideQuest(LIDA_ID, NPC_DEFS[LIDA_ID], [{
  ...
  giverNpcId: LIDA_ID,
  type: QuestType.FETCH,
  desc: 'Лида Восковая: «Один сургуч - одна истотитная свеча и кружка чая. Без святости, по обмену.»',
