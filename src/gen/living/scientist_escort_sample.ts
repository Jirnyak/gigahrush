...
const NPC_DEFS: Record<string, PlotNpcDef> = {
  [SCIENTIST_ID]: {
    ...
    talkLines: [
      ...
      'Если меня срежут по дороге, маршрут закрывайте. Образец без свидетеля быстро становится легендой.',
    ],
    talkLinesPost: [
      'Доверие в НИИ измеряют не словами, а тем, кто вернулся с пробой и не продал её за угол.',
      'Фальшивку я отличу не сразу. Это, к сожалению, тоже научный результат.',
      'Пробная всё ещё дышит через щель. Быстрее берите контейнер и не стойте под лампой.',
    ],
  },
  [CHECKPOINT_ID]: {
    ...
    talkLinesPost: [
      ...
      'Слышишь скрежет? Это маршрут проверяет подпись.',
    ],
...
registerSideQuest(SCIENTIST_ID, NPC_DEFS[SCIENTIST_ID], [
  {
    ...
    type: QuestType.TALK,
    desc: 'Ира Пробиркина: «Доведите меня до пропускного решения: поговорите с Павлом у пробной. Если маршрут зашумит, сначала защитите живых, потом пробирки.»',
