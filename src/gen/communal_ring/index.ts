import {
  expandCommunalRing,
  tuneCommunalRingZone,
  labelCommunalRingPopulationRooms,
...
export function generateCommunalRingDesignFloor(seed = RING_SEED): FloorGeneration {
  return withSeededRandom(seed, () => {
    ...
    const generation: FloorGeneration = {
      ...
      onAfterTerritory: (w) => {
        reinforceCommunalRingAuthoredHqTerritory(w);
        for (const zone of w.zones) tuneCommunalRingZone(w, zone, 3);
      }
    };

    const route = designFloorById('communal_ring');
    if (route) {
      const rngFn = seededRandom(hashSeed(`design-full:${route.id}:${route.z}`, route.z));
      expandCommunalRing(world, rngFn, {
        floorTex: Tex.F_LINO,
        wallTex: Tex.PANEL,
        radius: 2,
        fog: 0,
