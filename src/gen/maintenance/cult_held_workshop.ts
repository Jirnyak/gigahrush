...
const WORKSHOP_ROOM = 'Культовая мастерская: станок дверных комплектов';
const OUTPUT_ROOM = 'Клетка готовых дверей культа';
...
const MECHANIC_DEF: PlotNpcDef = {
  ...
  talkLines: [
    'Клава Реверс. Это не алтарь, это станок. Он делает дверные комплекты, если ему не читать молитвы в ремень.',
    'Культисты держат выходной ящик. Рабочие делают вид, что чинят, чтобы их не положили под пресс.',
    'Две шестерни в подачу - и я запущу линию так, что даже Омилян назовет это знаком.',
  ],
  talkLinesPost: [
    'Станок снова режет металл, не людей. Уже прогресс.',
    'Если услышишь ровный стук - это не культ. Это смена вспомнила профессию.',
  ],
};
...
const FOREMAN_DEF: PlotNpcDef = {
  ...
  talkLines: [
    'Станок слушает Чернобога лучше мастера: железо отдает форму, человек - страх.',
    'Три серых брикета в ящик - и получишь комплект двери без крови. Это не торговля, это порядок.',
    'Не трогай готовые комплекты без обряда. Ворованный металл громко вспоминает владельца.',
  ],
  talkLinesPost: [
    'Десятина принята. Станок будет делать вид, что ты свой.',
...
const SABOTEUR_DEF: PlotNpcDef = {
  ...
  talkLines: [
    'Петя Нулевая Фаза. Я не против станков. Я против станков, которые платят культу.',
    'Два предохранителя в обратную цепь - и ремень встанет без стрельбы. Рабочие переживут, культ потеряет смену.',
    'Потом можно украсть, можно уйти. Главное - не стой рядом, когда молитва встретится с фазой.',
  ],
  talkLinesPost: [
...
registerSideQuest(MECHANIC_ID, MECHANIC_DEF, [
  {
    ...
    desc: 'Клава Реверс: «Две шестерни в подачу. Запустим станок как цех, а не как культовую пасть.»',
    targetItem: 'gear',
    targetCount: 2,
    rewardItem: 'door_kit',
    rewardCount: 1,
...
registerSideQuest(FOREMAN_ID, FOREMAN_DEF, [
  {
    ...
    type: QuestType.FETCH,
    desc: 'Омилян Черноремень: «Три серых брикета как десятина станка. Получишь дверной комплект и проход без очереди к ремню.»',
...
registerSideQuest(SABOTEUR_ID, SABOTEUR_DEF, [
  {
    ...
    type: QuestType.FETCH,
    desc: 'Петя Нулевая Фаза: «Два предохранителя в обратную цепь. Станок встанет, культ останется с молитвой и пустым ремнем.»',
    targetItem: 'fuse',
    targetCount: 2,
...
const QUEST_OUTCOMES: Record<string, QuestOutcome> = {
  [REPAIR_QUEST]: {
    ...
    message: 'Культовая мастерская снова дает дверные комплекты смене.',
  ...
  [TRIBUTE_QUEST]: {
    kind: 'tribute',
    targetName: 'Десятина мастерской уплачена без боя.',
    message: 'Культ пропустил тебя к партии дверных комплектов за брикеты.',
    type: 'room_produced_items',
    severity: 3,
  ...
  [SABOTAGE_QUEST]: {
    ...
    message: 'Ремень мастерской встал; культ потерял выпуск, но цех тоже замолчал.',
