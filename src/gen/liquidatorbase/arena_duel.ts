 ...
 *
 * Он ОБЩИЙ, а не ролевой, и это не мелочь. Ролевой поводок читается в обоих
 * режимах — и на посту, и после `release`, — то есть заменил бы бойцам ещё и
 * пост, а вместе с ним и всю выдержку до прихода камеры: держит их до сигнала
 * именно короткий пост. Общий читается только у отпущенных, а отпущены здесь
 * ровно двое.
 *
 ...
 * пятьдесят секунд из шестидесяти. Теперь поводок отпущенного не трогает ни
 * цели, ни памяти, ни скана и не переписывает курс каждым кадром
 * (`holdCastNearAnchor`), поэтому задетый в бою он бой не разваливает.
 */
 ...
 *
 * СНИЗУ его держит выдержка до сигнала. На посту человека отпускает на
 * `SCENE_POST_LEASH` (полторы клетки), то есть двое сходятся на три клетки
 * ближе объявленного, а арматура достаёт на две с половиной вместе с радиусом
 * удара. Зазор в шесть клеток (`FIGHTER_GAP = 3`) укладывался в это впритык и
 * на части сидов не удерживал: место ищется расходящейся спиралью, и разброс
 * сдвигал бойцов друг к другу — на сиде 20881 они встали в четырёх клетках и
 * успевали разменяться до прихода камеры.
 *
...
registerFloorScene({
  ...
  maxSeconds: 180,
  /* Поводок читается только у ОТПУЩЕННЫХ (`holdCastNearAnchor`), а отпущены
