...
export interface Site extends SiteSeed {
  id: number;
  name: string;
  originX: number;
  originY: number;
...
export interface RidgeCandidate {
  a: number;
  b: number;
  x: number;
  y: number;
...
export interface QuarantineCellStats {
  mid: number;
  micro: number;
  microDoors: number;
}

export interface MicroVoronoiCellData {
  parentSiteId: number;
  serial: number;
  role: SiteRole;
  faction: ZoneFaction;
...
export type MicroVoronoiSite = VoronoiRoomSite<MicroVoronoiCellData>;
...
export const SEED = hashSeed(VORONOI_QUARANTINE_ROUTE_ID);
export const CX = W >> 1;
export const CY = W >> 1;
export const CORE_MIN = 0;
export const CORE_MAX = W - 1;
export const LLOYD_PASSES = 2;
export const LLOYD_STEP = 8;
export const OWNER_NONE = -1;
export const GENERATED_CELL_RING_COUNT = 4;
export const MICRO_ROOM_TARGET_MIN = 12_288;
export const MICRO_SITE_SAMPLE_ATTEMPTS = 32;
export const MICRO_EXTRA_DOOR_RATIO = 0.42;

export const SITE_SEEDS: readonly SiteSeed[] = [
...
export const FACTION_HQ_SPECS: readonly FactionHqSpec[] = [
...
export const layouts = new WeakMap<World, VoronoiQuarantineLayout>();
...
export function buildSites(world: World, seed: number): Site[] {
...
export function generatedRoleForSite(role: SiteRole, ring: number): SiteRole {
...
export function generatedFactionForSite(site: Site, ring: number): ZoneFaction {
...
export function roomTypeForGeneratedRole(role: SiteRole, faction: ZoneFaction): RoomType {
...
export function wallTexForGeneratedRole(role: SiteRole, faction: ZoneFaction): Tex {
...
export function floorTexForGeneratedRole(role: SiteRole, faction: ZoneFaction): Tex {
...
export function generatedSiteName(parent: Site, id: number, ring: number, role: SiteRole, faction: ZoneFaction): string {
...
export function quarantineRoomType(site: Site, serial: number, scale: 'mid' | 'micro'): RoomType {
...
export function hasAuthoredVoronoiTerritory(world: World): boolean {
...
export function assignLaguerreCells(world: World, sites: readonly Site[]): Int16Array {
...
export function collectRidgeCandidates(
  world: World,
  owner: Int16Array,
  sites: readonly Site[],
...
export function placeRidgeDoors(
  world: World,
  sites: readonly Site[],
...
export function buildRoomsFromOwners(world: World, sites: readonly Site[], owner: Int16Array): void {
...
export function placeFactionMiniHqs(world: World, sites: readonly Site[], owner: Int16Array, seed: number): void {
...
export function placeQuarantineMidMicroRooms(world: World, sites: readonly Site[], owner: Int16Array, seed: number): QuarantineCellStats {
...
export function buildMicroVoronoiSites(world: World, sites: readonly Site[], owner: Int16Array, seed: number): MicroVoronoiSite[] {
...
export function placeMicroVoronoiSitesForParent(
  world: World,
  owner: Int16Array,
  parent: Site,
  bounds: Bounds,
...
export function findNearbyParentFloorCell(
  world: World,
  owner: Int16Array,
  parentSiteId: number,
  x: number,
...
export function makeMicroVoronoiSite(parent: Site, x: number, y: number, serial: number, id: number, seed: number): MicroVoronoiSite {
...
export function microRoleForParent(parent: Site, serial: number): SiteRole {
...
export function countMicroVoronoiDoors(world: World): number {
  let count = 0;
  for (const door of world.doors.values()) {
...
export function tryStampQuarantineRoom(
  world: World,
  owner: Int16Array,
  sites: readonly Site[],
  siteIdValue: number,
...
export function siteBounds(world: World, sites: readonly Site[], owner: Int16Array): Bounds[] {
  const bounds: Bounds[] = sites.map(() => ({ minDx: W, minDy: W, maxDx: -W, maxDy: -W, minX: W, minY: W, maxX: -1, maxY: -1, count: 0 }));
  for (let y = CORE_MIN; y <= CORE_MAX; y++) {
    for (let x = CORE_MIN; x <= CORE_MAX; x++) {
      const idx = world.idx(x, y);
...
export function decorateQuarantineRoom(world: World, room: Room, type: RoomType, serial: number): void {
...
export function decorateSites(world: World, sites: readonly Site[], owner: Int16Array, seed: number): void {
  for (let y = CORE_MIN; y <= CORE_MAX; y++) {
    for (let x = CORE_MIN; x <= CORE_MAX; x++) {
      const idx = world.idx(x, y);
      const id = owner[idx];
...
export function placeLifts(world: World, sites: readonly Site[], owner: Int16Array): void {
...
export function placeQuarantineEmergencyPanels(world: World, sites: readonly Site[], owner: Int16Array, seed: number): void {
...
export function placeDrops(world: World, entities: Entity[], nextId: { v: number }, sites: readonly Site[], owner: Int16Array): void {
...
export function spawnThreats(world: World, entities: Entity[], nextId: { v: number }, sites: readonly Site[], owner: Int16Array): void {
...
export function nearestSiteId(world: World, x: number, y: number, sites: readonly Site[]): number {
  let best = 0;
...
export function openOwnedCell(world: World, idx: number, roomId: number, site: Site, noise: number): void {
...
export function sortedCandidates(map: Map<string, RidgeCandidate>): RidgeCandidate[] {
  return [...map.values()].sort((a, b) => a.score - b.score);
}

export function sortedAdjacency(map: Map<string, RidgeCandidate>): [number, number][] {
  return [...map.values()]
    .map(edge => [edge.a, edge.b] as [number, number])
    .sort((a, b) => a[0] - b[0] || a[1] - b[1]);
}

export function spanningRidges(siteCount: number, candidates: readonly RidgeCandidate[]): RidgeCandidate[] {
...
export function ridgeGraphConnected(sites: readonly Site[], edges: readonly [number, number][]): boolean {
...
export function shouldAddExtraRidge(a: Site, b: Site): boolean {
...
export function ridgeDoorState(a: Site, b: Site, serial: number): { state: DoorState; keyId: string } {
...
export function placeDoubleDoor(
  world: World,
  sites: readonly Site[],
  edge: RidgeCandidate,
  state: DoorState,
...
export function setDoorCell(world: World, x: number, y: number, roomA: number, roomB: number, state: DoorState, keyId: string): void {
  const idx = world.idx(x, y);
...
export function placePanelNearSite(
  world: World,
  sites: readonly Site[],
  owner: Int16Array,
  key: keyof typeof VORONOI_QUARANTINE_ROOM_DEF_IDS,
...
export function addContainer(
  ...
  const container: WorldContainer = {
    ...
    kind,
    name,
    inventory: inventory.map(item => ({ ...item })),    ownerNpcId,
    ownerName,
...
export function findSiteCell(world: World, owner: Int16Array, sites: readonly Site[], id: number): { x: number; y: number } {
  const site = sites[id]!;
...
export function roomForSite(world: World, sites: readonly Site[], key: keyof typeof VORONOI_QUARANTINE_ROOM_DEF_IDS): Room {
  return world.rooms[siteId(sites, key)]!;
}

export function siteId(sites: readonly Site[], key: keyof typeof VORONOI_QUARANTINE_ROOM_DEF_IDS): number {
  const site = sites.find(item => item.key === key);
  return site?.id ?? 0;
}

export function sitePairHasRole(a: Site, b: Site, role: SiteRole): boolean {
  return a.role === role || b.role === role;
}

export function samePair(a: RidgeCandidate, b: RidgeCandidate): boolean {
  return a.a === b.a && a.b === b.b;
}

export function countSiteCells(owner: Int16Array, siteCount: number): number[] {
  const counts = new Array(siteCount).fill(0) as number[];
  for (const id of owner) if (id >= 0) counts[id]++;
  return counts;
}

export function siteRoleByRoomName(name: string): SiteRole | undefined {
  const seed = SITE_SEEDS.find(item => item.key && VORONOI_QUARANTINE_ROOM_DEF_IDS[item.key] === name);
  return seed?.role;
}

export function featureChance(role: SiteRole): number {
  switch (role) {
    case 'checkpoint':
    case 'clinic':
    case 'supply':
...
export function featureForRole(role: SiteRole, roll: number): Feature {
  switch (role) {
    case 'checkpoint':
