...
const KHRUSHCHEV_DEF: PlotNpcDef = {
  ...
  talkLines: [
    'Я — Генеральный секретарь Хрущёв. Дом стоит, значит власть работает.',
    'Гигахрущ не бесконечен, он просто достаточно большой, чтобы люди забыли слово «наружу».',
    'Каждый стояк, актовый зал и коллектор внесён в план. Даже те, которые ночью меняют адрес.',
    'В коллекторах завёлся Махно. Дикие режут кабель, воруют пайки и называют это волей.',
    'Ликвидируй Махно. Без атамана банда станет обычной очередью за хлебом.',
  ],
  talkLinesPost: [
    'Махно списан. Трубопроводы снова принадлежат государству, хотя сами об этом пока не знают.',
    'Порядок — это когда человек боится самосбора больше, чем начальства.',
    'Заходи. Власть благодарна молча, но иногда выдаёт премию.',
  ],
};
...
const ROTENBERGOV_DEF: PlotNpcDef = {
  ...
  talkLines: [
    ...
    'В Гигахруще всё облагается: вода, тишина, место в очереди, даже надежда на лифт.',
    'Самосбор начинается там, где баланс не сходится. Стены не терпят минуса.',
    'Нужен миллион рублей. Официально — налог, фактически — шов, которым мы сшиваем хрущ до утра.',
    'Не путайте бедность с невиновностью. Квитанция найдёт каждого.',
  ],
  talkLinesPost: [
    'Сумма поступила. Казна дышит, бухгалтерия крестится степлером.',
    'Финансовая дисциплина — это когда даже призрак платит за коммуналку.',
    'Будут новые сборы. Дом большой, совесть маленькая.',
  ],
};
...
registerSideQuest('khrushchev', KHRUSHCHEV_DEF, [
  {
    ...
    type: QuestType.KILL,
    desc: 'Генсек Хрущёв: «В коллекторах завёлся Махно. Он режет кабель и зовёт это волей. Ликвидируй атамана — хрущ должен слышать только свои трубы.»',
...
registerSideQuest('rotenbergov', ROTENBERGOV_DEF, [
  {
    ...
    desc: 'Министр Ротенбергов: «Налог чрезвычайный. Принеси 1 000 000 рублей, пока дебет с кредитом не разошлись вместе со стенами.»',
