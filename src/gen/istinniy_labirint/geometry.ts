...
import { ISTINNIY_LABIRINT_ROUTE_ID, ISTINNIY_LABIRINT_Z, ISTINNIY_LABIRINT_CHORD_KEY, GRID_W, GRID_H, GRID_N, PITCH, CENTER_OFFSET, START_GX, START_GY, SAFE_WALL_MIN, SAFE_WALL_MAX, ROOM_WALL, ROOM_FLOOR, MAZE_WALL, MAZE_FLOOR, THREAD_FLOOR, CHORD_FLOOR, SAFE_WALL_ROOM, LOST_ROOM, DOCUMENT_STASH_ROOM, LANDMARK_ROOM_NAMES, BIT_W, BIT_N, Dir, DIRS, MazeGraph, LockedChord, CellPoint, Landmark, LabyrinthOwnedRoom, RoomStampSpec, IstinniyLabirintMetrics, NPC_IDS, ARIADNA_DEF, LOST_PAVEL_DEF } from "./meta";
import { markAriadneCue, connectRoomToPoint, spawnPlotNpc, spawnMonster } from "./npcs";

export function gridIdx(gx: number, gy: number): number {
  const x = ((gx % GRID_W) + GRID_W) % GRID_W;
  const y = ((gy % GRID_H) + GRID_H) % GRID_H;
  return y * GRID_W + x;
}

export function gridX(idx: number): number {
  return idx % GRID_W;
}

export function gridY(idx: number): number {
  return (idx / GRID_W) | 0;
}

export function centerOf(idx: number): CellPoint {
  return {
...
export function degree(links: Uint8Array, idx: number): number {
  let count = 0;
...
export function shuffleDirs(): Dir[] {
  const dirs = [...DIRS];
...
export function buildGrowingTreeMaze(): MazeGraph {
...
export function braidDeadEnds(links: Uint8Array): number {
  let added = 0;
...
export function measureMazeDepth(links: Uint8Array, start: number): { parent: Int32Array; depth: Int32Array; exit: number } {
...
export function pathToRoot(parent: Int32Array, from: number): number[] {
  const path: number[] = [];
  let current = from;
  while (current >= 0) {
    path.push(current);
...
export function carvePatch(world: World, x: number, y: number, radius: number, floorTex: Tex): void {
...
export function carveLine(world: World, a: CellPoint, b: CellPoint, radius: number, floorTex: Tex, mark = false): number[] {
...
export function carveMaze(world: World, graph: MazeGraph): void {
  for (let idx = 0; idx < GRID_N; idx++) {
...
export function carveSafeWallRoute(world: World, graph: MazeGraph): number[] {
...
export function markMainThread(world: World, graph: MazeGraph): void {
...
export function linkedDirections(links: Uint8Array, cell: number): Dir[] {
...
export function addDoorToRoom(world: World, room: Room, wx: number, wy: number): void {
...
export function addDoorToRoomState(world: World, room: Room, wx: number, wy: number, state: DoorState, keyId = ''): void {
  const idx = world.idx(wx, wy);
  if (world.cells[idx] === Cell.DOOR) return;
  world.cells[idx] = Cell.DOOR;
...
export function connectLandmarkRoom(world: World, room: Room, cell: number, links: Uint8Array): void {
...
export function placeLandmarkRoom(world: World, graph: MazeGraph, spec: Landmark): Room {
...
export function pathCell(graph: MazeGraph, fraction: number): number {
...
export function deepestDeadEnds(graph: MazeGraph, count: number, exclude: ReadonlySet<number>): number[] {
  const cells: number[] = [];
  for (let idx = 0; idx < GRID_N; idx++) {
...
export function chooseLandmarks(graph: MazeGraph): Landmark[] {
...
export function roomCanOverwriteLabyrinth(world: World, x: number, y: number, w: number, h: number): boolean {
...
export function decorateLabyrinthRoom(world: World, room: Room, feature: Feature, serial: number): void {
...
export function tryStampLabyrinthRoom(world: World, spec: RoomStampSpec, serial: number): Room | null {
...
export function hqSupportSpecs(cell: number, owner: TerritoryOwner, prefix: string, serial: number, includeCore: boolean): RoomStampSpec[] {
...
export function selectDistinctCells(candidates: readonly number[], count: number, used: Set<number>, minGridDistance: number): number[] {
  const out: number[] = [];
...
export function placeFactionHqPackages(world: World, graph: MazeGraph, roomsByName: Map<string, Room>): LabyrinthOwnedRoom[] {
...
export function placeMidStations(world: World, graph: MazeGraph, used: Set<number>): void {
  const candidates = [
...
export function placeMicroRooms(world: World, graph: MazeGraph, used: Set<number>): void {
...
export function placeSideAlcoves(world: World, graph: MazeGraph): void {
...
export function placeLabyrinthMidMicro(world: World, graph: MazeGraph, roomsByName: Map<string, Room>): LabyrinthOwnedRoom[] {
...
export function selectLockedChords(graph: MazeGraph, count: number): { a: number; dir: Dir; b: number }[] {
...
export function carveLockedChord(world: World, chord: { a: number; dir: Dir; b: number }, serial: number): LockedChord {
...
export function placeActors(world: World, graph: MazeGraph, roomsByName: Map<string, Room>, entities: Entity[], nextId: { v: number }, chords: readonly LockedChord[]): void {
...
export function placeLandmarks(world: World, graph: MazeGraph): Map<string, Room> {
...
export function tuneLabyrinthZones(world: World): void {
  generateZones(world);
...
export function strictWalkable(world: World, idx: number): boolean {
...
export function reachableStrict(world: World, startIdx: number): Uint8Array {
...
export function reachableLift(world: World, reachable: Uint8Array, direction: LiftDirection): boolean {
  for (let i = 0; i < W * W; i++) {
...
export function findPathToLift(world: World, startIdx: number, direction: LiftDirection): number[] {
...
export function pathDirectionEntropy(path: readonly number[]): number {
...
export function minRoomSpacing(world: World, rooms: readonly Room[]): number {
  let best = Infinity;
