...
export function drawFastElevatorOverlay(
  ...
  ctx.fillText(fitText(ctx, 'СКОРОСТНОЙ ЛИФТ', panelW - 16 * s), w * 0.5, y + 16 * s);

  ctx.shadowBlur = 0;
  ctx.fillStyle = '#6a8';
  ctx.font = `${Math.round(8 * s)}px "Press Start 2P", monospace`;
  ctx.fillText(fitText(ctx, 'выбор уровня доступа', panelW - 16 * s), w * 0.5, y + 28 * s);
  ...
  for (let i = startIndex; i < endIndex; i++) {
    ...
    if (!isUnlocked) {
      ...
      ctx.fillText(fitText(ctx, `[НЕДОСТУПНО] Этаж ${floor > 0 ? '+' : ''}${floor}`, panelW - 32 * s), x + 20 * s, rowY);
    } else {
      ctx.fillStyle = isSelected ? '#fff' : '#8ac';
      ctx.shadowColor = isSelected ? '#8cf' : 'transparent';
      ctx.shadowBlur = isSelected ? 4 * s : 0;
      ctx.fillText(fitText(ctx, label, panelW - 32 * s), x + 20 * s, rowY);
      ctx.shadowBlur = 0;
    }
  }
  ...
  if (snapshot.message) {
    ...
    ctx.font = `${Math.round(8 * s)}px "Press Start 2P", monospace`;
    ctx.fillText(fitText(ctx, snapshot.message, panelW - 24 * s), w * 0.5, y + panelH - 24 * s);
  }
  ...
  ctx.fillText(fitText(ctx, `${controlBindingLabel('menuUp')}/${controlBindingLabel('menuDown')} выбор   ${controlHint('gameMenu')} ехать   ${menuCloseHint()} выйти`, panelW - 16 * s), w * 0.5, y + panelH - 8 * s);
