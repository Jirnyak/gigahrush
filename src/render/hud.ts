...
function drawSamosborActiveInstruction(
  ...
  ctx.font = `bold ${16 * sy}px "Pixelify Sans", monospace`;
...
function drawSamosborWarningInstruction(
  ...
  ctx.font = `${detailSize}px "Pixelify Sans", monospace`;
...
function drawRouteCueHint(
  ...
  ctx.font = `${6 * sy}px "Pixelify Sans", monospace`;
  ctx.fillStyle = '#9a8';
...
export function drawPointerCaptureGate(ctx: CanvasRenderingContext2D, time = 0): void {
  ...
  ctx.shadowBlur = 10 * s;
  ctx.fillStyle = '#bff';
  ctx.font = `bold ${Math.round(17 * s)}px "Pixelify Sans", monospace`;
...
function drawCombatSightFeedback(
  ...
  if (target) {
    ...
    ctx.font = `${6.5 * s}px "Pixelify Sans", monospace`;
    ...
    if (hasQuestMarker) {
      ...
      ctx.shadowBlur = 4;
      ctx.font = `bold ${9 * s}px "Pixelify Sans", monospace`;
...
function drawWorldSpeechBubbles(
  ...
  for (const e of entities) {
    ...
    ctx.font = `${6 * s}px "Pixelify Sans", monospace`;
...
export function drawHUD(
  ...
  ctx.font = `${10 * sy}px "Pixelify Sans", monospace`;
  ctx.textBaseline = 'top';
  ...
  if (showCompactPanels && showBottomTabs && player.needs) {
    ...
    bars.forEach(([label, current, max, color], i) => {
      ...
      ctx.textAlign = 'left';
      ctx.font = `${labelFont}px "Pixelify Sans", monospace`;
      ...
      ctx.textAlign = 'right';
      ctx.font = `${percentFont}px "Pixelify Sans", monospace`;
  ...
  if (showCompactPanels && showMessages && !state.samosborActive) {
    ...
    ctx.save();
    ctx.font = `${5.8 * s}px "Pixelify Sans", monospace`;
    ...
    if (plannedMsgs.length > 0 && summaryW >= 128 * s) {
      ...
      ctx.font = `${5.4 * s}px "Pixelify Sans", monospace`;
      ...
      for (const item of plannedMsgs) {
        ...
        ctx.font = `${5.3 * s}px "Pixelify Sans", monospace`;
  ...
  if ((showInteractionPrompt || manualItemPickup) && showCompactPanels && !emergencyPanelOpen && !state.showControls && !state.showUiSettings) {
    ...
    if (interaction) {
      ...
      if (showInteractionPrompt || interaction.kind === 'item_drop') {
        ...
        ctx.font = `bold ${11 * sy}px "Pixelify Sans", monospace`;
  ...
  if (state.gameOver && state.gameWon) {
    ...
    if (state.deathTimer > 1) {
      ...
      ctx.font = `bold ${28 * sy}px "Pixelify Sans", monospace`;
    ...
    ctx.font = `bold ${24 * sy}px "Pixelify Sans", monospace`;
    ctx.textAlign = 'center';
