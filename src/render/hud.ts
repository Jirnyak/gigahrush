...
function drawSamosborActiveInstruction(
  ...
  ctx.font = `bold ${16 * sy}px "VT323", monospace`;
...
function drawSamosborWarningInstruction(
  ...
  ctx.font = `${detailSize}px "VT323", monospace`;
...
function drawRouteCueHint(
  ...
  ctx.font = `${6 * sy}px "VT323", monospace`;
  ctx.fillStyle = '#9a8';
...
function drawFpsCounter(ctx: CanvasRenderingContext2D, perf: HudPerfDebugSnapshot | undefined, rect: UiRect, sx: number, sy: number): void {
  ...
  const lineH = 8 * s;
  ctx.save();
  ctx.font = `${7 * s}px "VT323", monospace`;
  ctx.textAlign = 'left';
...
export function drawPointerCaptureGate(ctx: CanvasRenderingContext2D, time = 0): void {
  ...
  ctx.shadowBlur = 10 * s;
  ctx.fillStyle = '#bff';
  ctx.font = `bold ${Math.round(17 * s)}px "VT323", monospace`;
  ...
  ctx.font = `${Math.round(7 * s)}px "VT323", monospace`;
...
function drawCombatSightFeedback(
  ...
  if (target) {
    ...
    ctx.font = `${6.5 * s}px "VT323", monospace`;
    ...
    if (hasQuestMarker) {
      ...
      ctx.shadowBlur = 4;
      ctx.font = `bold ${9 * s}px "VT323", monospace`;
...
export function drawHUD(
  ...
  ctx.font = `${10 * sy}px "VT323", monospace`;
  ctx.textBaseline = 'top';
  ...
  if (showCompactPanels && showBottomTabs && player.needs) {
    ...
    bars.forEach(([label, current, max, color], i) => {
      ...
      ctx.textAlign = 'left';
      ctx.font = `${labelFont}px "VT323", monospace`;
      ...
      ctx.textAlign = 'right';
      ctx.font = `${percentFont}px "VT323", monospace`;
  ...
  if (showCompactPanels && showMessages && !state.samosborActive) {
    ...
    ctx.save();
    ctx.font = `${5.8 * s}px "VT323", monospace`;
    ...
    if (plannedMsgs.length > 0 && summaryW >= 128 * s) {
      ...
      ctx.font = `${5.4 * s}px "VT323", monospace`;
      ...
      for (const item of plannedMsgs) {
        ...
        ctx.font = `${5.3 * s}px "VT323", monospace`;
  ...
  if (state.gameOver && state.gameWon) {
    ...
    if (state.deathTimer > 1) {
      ...
      ctx.font = `bold ${28 * sy}px "VT323", monospace`;
    ...
    ctx.font = `bold ${24 * sy}px "VT323", monospace`;
    ctx.textAlign = 'center';
