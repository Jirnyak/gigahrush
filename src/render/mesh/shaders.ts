...
void main() {
  ...
  // ДВОЙКА ЗДЕСЬ НЕ ЗАПАС, А КОМПЕНСАЦИЯ ДЕЛЕНИЯ ПОПОЛАМ. НЕ ТРОГАТЬ.
  //
  // Меш идёт обычным конвейером: z/w даёт NDC в [-1..1], окно получает (z/w+1)/2.
  // При z = ty - 0.2 это ровно 1 - 0.1/ty — та же формула, какой рейкастер пишет
  // gl_FragDepth напрямую (webgl.ts: 1 - 0.1/dist). То есть глубины совпадают
  // ТОЧНО, а нужный мешу мелкий перевес даёт polygonOffset(-1, -2).
  // Обратные кавычки в этом комментарии ставить нельзя: он живёт внутри
  // шаблонной строки и закроет её.
  //
  // Уменьшение этого числа выглядит как «сузить запас», а на деле уводит меш
  // ДАЛЬШЕ поверхности, на которой он стоит. Проверено дорого: 0.125 дало окну
  // глубину 1 - 0.0625/ty, меши стали проигрывать depth-тест полу, стенам и
  // потолку и пропали из игры целиком — лампы, линолеум, панели, всё.
...
void main() {
  ...
  // Растворение доводится до конца. Туман сам упирается в потолок 0.92, и на
  // кромке поля 8 % цвета меша ещё горело; edgeFade, урезанный до 0.86, перекрыть
  // это не мог даже сработав. Он ненулевой только в последних трёх клетках, так
  // что доведение до единицы гасит именно кромку и больше ничего.
  vec3 color = mix(finalColor, uFogColor, max(fog, edgeFade));
