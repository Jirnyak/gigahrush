...
function buildQuad(
  ...
  /* Руки прижаты к своим краям кадра: инструмент к левому, оружие к правому.
   *
   * Оружие стояло по центру-снизу, по домовской классике, и закрывало собой
   * прицел вместе с половиной обзора. Зеркало инструмента честнее: обе руки
   * уходят за СВОЙ край экрана, середина кадра остаётся зрителю. Смещение
   * общее — ни один пакет силуэта об этом не знает и не перерисовывается.
   *
  ...
  const baseX = slotName === 'weapon' ? u.screenW - VM : 0;
...
export function updateViewmodel(u: ViewmodelUpdate): void {
  ...
  // Инструмент рисуется первым и лежит ПОД оружием. Холсты рук разведены по
  // краям кадра и не пересекаются, но порядок остаётся объявленным: он решает
  // наложение, если силуэт однажды дотянется до чужой половины.
