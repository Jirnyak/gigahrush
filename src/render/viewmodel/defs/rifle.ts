 ...
 * закрывает полоса HUD. Холст прижат к ПРАВОМУ краю кадра, столбец `c` — это
 * столбец экрана `192 + c`: в ЛЕВЫЙ край упираться нельзя, он приходится на
 * середину кадра и дал бы прямой срез в воздухе. Наружу можно вправо и вниз.
 */
...
registerViewmodel({
  ...
  draw({ buf, frame, skin, rand }) {
     ...
     * входит из кадра и уходит ЗА нижний срез; в ЛЕВЫЙ край её пускать нельзя —
     * он приходится на середину кадра. */
