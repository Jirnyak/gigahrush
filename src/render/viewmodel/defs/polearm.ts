 ...
 * (85°) укладывается поперёк кадра и упирается в левый столбец холста, который
 * приходится на середину кадра; отвесное теряет вылет. Диагональ около 42°
 ...
 *
 * Строка 10 — это пиксель 20: выше него силуэт лезет к прицелу. Столбец 0 —
 * левый край холста, он приходится на середину кадра, и у ДЛИННОГО оружия он
 * опаснее всего: древко длиннее любого другого силуэта и первым уходит туда.
 * Столбец 63 упирается в край экрана: наружу можно вправо и вниз.
 */
const TOP_ROW = 10;
 ...
 *
 * Уходит вниз, а не в левый угол: кулаки сидят на строках 60..100, и конус из
 * них в угол пересёк бы левый край холста ВЫШЕ полосы HUD, а он приходится на
 * середину кадра.
 */
...
registerViewmodel({
  ...
  draw({ buf, frame, skin, rand }) {
     ...
     * за левый край холста, который приходится на середину кадра. */
