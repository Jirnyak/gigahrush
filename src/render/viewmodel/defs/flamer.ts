 ...
 * воздухе. Наружу можно вправо и вниз — поэтому струя на выстреле обрезана
 * сверху и слева, а вправо идёт свободно.
 */
...
registerViewmodel({
  ...
  draw({ buf, frame, skin, rand }) {
     ...
     * вправо и вниз, а ЛЕВЫЙ край приходится на середину кадра. */
    if (fire) {
      const nx = -AY;
      const ny = AX;
