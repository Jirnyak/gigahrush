...
export function drawNpcMenu(
  ...
  ctx.fillStyle = '#688';
  ctx.font = `${6.2 * sy}px "VT323", monospace`;
  ...
  if (state.npcMenuTab === 'main') {
    ...
    ctx.fillStyle = '#456';
    ctx.font = `${6.8 * sy}px "VT323", monospace`;
    ...
    ctx.fillStyle = '#ccc';
    ctx.font = `${7.8 * sy}px "VT323", monospace`;
    const maxW = pw - 16 * sx;
    ...
    ctx.font = `${9.2 * sy}px "VT323", monospace`;
    ...
    const drawColumnFrame = (gx: number, title: string, value: string, accent: string, stripBg: string) => {
      ...
      ctx.font = `${6.6 * sy}px "VT323", monospace`;
      ctx.textAlign = 'left';
      ...
      for (let row = 0; row < gridRows; row++) {
        for (let col = 0; col < gridCols; col++) {
          ...
          if (idx < inv.length) {
            ...
            ctx.font = `${Math.max(4, 4.3 * sy)}px "VT323", monospace`;
            ...
            ctx.font = `${Math.max(4, 4.7 * sy)}px "VT323", monospace`;
    ...
    ctx.font = `${6.1 * sy}px "VT323", monospace`;
    ...
    if (curIdx < curInv.length) {
      ...
      if (def) {
        ...
        ctx.fillStyle = '#888';
        ctx.font = `${6.3 * sy}px "VT323", monospace`;
