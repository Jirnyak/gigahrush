...
export function drawNpcMenu(
  ...
  drawGlitchText(ctx, fitTextStable(ctx, npc.name ?? '???', pw - 16 * sx), px + 8 * sx, py + 10 * sy, time, 900, '#4f9682', 9 * sy);
  ctx.fillStyle = '#688';
  ctx.font = `${6.2 * sy}px "Pixelify Sans", monospace`;
  ...
  if (state.npcMenuTab === 'main') {
    ...
    ctx.font = `${8.6 * sy}px "Pixelify Sans", monospace`;
    for (let i = 0; i < items.length; i++) {
      ...
      ctx.fillStyle = items[i].disabled
        ? selected ? '#9a6' : '#665'
        : selected ? `rgba(79,150,130,${flicker(time, 920 + i)})` : '#688';
      ctx.fillText(`${selected ? '▶ ' : '  '}${items[i].label}`, px + 16 * sx + mj.dx, yy + mj.dy);
    }
    ctx.fillStyle = '#456';
    ctx.font = `${6.8 * sy}px "Pixelify Sans", monospace`;
    ...
    ctx.fillStyle = '#ccc';
    ctx.font = `${7.8 * sy}px "Pixelify Sans", monospace`;
    const maxW = pw - 16 * sx;
    ...
    ctx.font = `${9.2 * sy}px "Pixelify Sans", monospace`;
    ...
    const drawColumnFrame = (gx: number, title: string, value: string, accent: string, stripBg: string) => {
      ...
      ctx.font = `${6.6 * sy}px "Pixelify Sans", monospace`;
      ctx.textAlign = 'left';
      ...
      for (let row = 0; row < gridRows; row++) {
        for (let col = 0; col < gridCols; col++) {
          ...
          if (idx < inv.length) {
            ...
            ctx.font = `${Math.max(4, 4.3 * sy)}px "Pixelify Sans", monospace`;
            ...
            ctx.font = `${Math.max(4, 4.7 * sy)}px "Pixelify Sans", monospace`;
    ...
    ctx.font = `${6.1 * sy}px "Pixelify Sans", monospace`;
    ...
    if (curIdx < curInv.length) {
      ...
      if (def) {
        ...
        ctx.fillStyle = '#888';
        ctx.font = `${6.3 * sy}px "Pixelify Sans", monospace`;
