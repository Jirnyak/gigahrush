...
/* ── Затухания в проекте НЕТ, и это решение, а не пропуск ─────────
 *
 * Здесь жил обратный ход: любое отношение медленно тянулось к тому числу, с
 * которого началось (четверть накопленного отклонения за визит, такт 30 с). Он
 * снят целиком по решению владельца, и снят вместе со всем обходом
 * (`systems/relation_decay.ts`), потому что мир не должен забывать.
 *
 * Прежний довод в его пользу — «влить в обиду можно бесконечно, вылить нечем,
 * поэтому долгий прогон сползает во всеобщую войну» — был измерен ТОГДА, когда
 * бой ещё двигал глобальную матрицу. Сейчас насилие в неё не пишет ни от чьей
 * руки (`systems/factions.ts`, `applyDamageRelationPenalty`), и главный разгон
 * перекрыт у истока: платят личные каналы тех, кто видел, а не шесть на шесть
 * чисел на всю игру. Обида стала фактом о ЛИЧНОСТИ, а факты не рассасываются.
 *
 * Если сюда снова захочется обратная тяга — это отдельное решение владельца, и
 * заводить её надо не как таймер забывания, а как ПОСТУПОК: извинение, откуп,
 * услуга, смена тела. */
