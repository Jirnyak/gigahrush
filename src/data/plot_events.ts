...
export function onVoidEntry(state: GameState): void {
  ...
  // Жильца этой камеры в игре нет: Пустота безлюдна, и указывать больше не на кого.
  // Остаётся само место — оно всё ещё генерируется и на него ведут два слуха.
  state.msgs.push(msg('Камера Пустотника открыта, хозяина в ней нет. Обратный портал ищите сами.', state.time, '#4af'));
}
