...
function plotNpcPackage(input: MainPlotNpcPackageInput): NpcPackageDef {
  ...
  /* Старшина базы ликвидаторов. Он объясняет игроку, почему низ закрыт, и он же
     после разворота Заслонова начинает снабжать: непустой инвентарь пакета — это
     и есть прилавок (товар генерится только пустому), так что склад старшины
     лежит здесь, а не в отдельной системе магазина. */
  export const BLINKOV_ID = registerPlotNpc(plotNpcPackage({
    id: 'blinkov',
    displayName: 'Старшина Блинков',
    publicLine: 'Старшина снабжения базы ликвидаторов, объясняющий блокаду нижних уровней и спецдопуск.',
    sex: 'male',
    age: 44,
    faction: Faction.LIQUIDATOR,
    ...
    homeFloorKey: floorKeyForDesign('liquidatorbase'),
    hp: 1400, maxHp: 1400, level: 10, money: 900, speed: 1.2,
    inventory: [
      { defId: 'ammo_762', count: 60 },
      { defId: 'ammo_9mm', count: 48 },
      { defId: 'bandage', count: 6 },
      { defId: 'gasmask_filter', count: 2 },
      { defId: 'canned', count: 4 },
      { defId: 'water', count: 4 },
      { defId: 'antidep', count: 2 },
    ],
    talkLines: [
      'Старшина Блинков, снабжение. Если пришёл от Якова Давидовича — садись, он мне ещё за общежитие должен.',
      'Вниз хода нет. Не «трудно», не «опасно» — приказом нет. Самосборы участились, и Министерство закрыло всё, что ниже промзоны.',
      'Спецдопуск. Бумага в две подписи, которую здесь никто не видел. Подписывают наверху, а спрашивают с нас.',
      'На перевалке сидят четверо, и каждый держит свои лифты за своей дверью. Приказ им, кстати, безразличен: они и до приказа так жили.',
      'Я снабженец, а не пропуск. Могу дать патроны, бинт и адрес. Дверь тебе всё равно открывать самому.',
      'Яков говорит, ты в поле не мешаешь. От него это почти орден.',
    ],
    talkLinesPost: [
      'Заслонова списали. Значит, приказ про низ подписывал не тот, кому мы верили, и я больше на него не оглядываюсь.',
      'Бери со стойки. Патроны, бинт, фильтр, вода. Накладную я закрою задним числом — не впервой.',
      'Пока наверху решают, чей это был приказ, склад работает по-старому: живому дают, мёртвому списывают.',
      'Гарнизон Громного просел. Часть наших уже ушла к нему вниз, и я их не держал.',
      'Если найдёшь ещё одну такую пластину — не носи её на шее. Ни в кармане, ни на шее.',
    ],
    talkQuestResponse: 'От Якова? Принял. Только сразу разочарую: вниз тебя не пущу, потому что вниз никого не пускают. Министерство закрыло нижние уровни: самосборы участились, а виноватым назначили лифт. Нужен спецдопуск. За ним не ко мне, за ним наверх, в Министерство. Спроси министра в лицо и посмотри, как он ответит.',
    voiceTags: ['liquidator', 'supply'],
  }));

  /* Завлаб НИИ слизи, однокашник Якова. Через него чёрная руна переходит из
     улики в задачу: три образца тканей, виды которых разыгрываются от сида
     прогона (`applyPlotSampleLottery` в `data/plot.ts`). */
  export const NII_GUSHCHIN_ID = registerPlotNpc(plotNpcPackage({
    id: 'nii_gushchin',
    displayName: 'Завлаб Гущин',
    publicLine: 'Заведующий баковым цехом НИИ слизи, читающий чёрную руну по реакции колонии.',
    sex: 'male',
    age: 59,
    faction: Faction.SCIENTIST,
    occupation: Occupation.SCIENTIST,
    sprite: Occupation.SCIENTIST,
    homeFloorKey: floorKeyForDesign('slime_nii'),
    hp: 900, maxHp: 900, level: 10, money: 240, speed: 1.0,
    inventory: [
    ...
    talkLines: [
      'Гущин, баковый цех. С Яковом Давидовичем мы делили общежитие и один микроскоп, так что «коллега» — это у нас не вежливость.',
      'Колония в третьем баке повторила ваш знак. Не похожий, а тот же: линия в линию, за одиннадцать секунд.',
      'Слизь ни на что так не отвечает. Ни на свет, ни на сирену, ни на начальство. На эту пластину — отвечает.',
      'Мне нужна не гипотеза, а выборка. Три вида, три среза, один протокол. Тогда я скажу, что она у вас в руках.',
      'Нет, я не подскажу, где их искать. Я биолог, а не диспетчер. Ходят они там, где им есть чем питаться.',
      'После третьего образца колония тянется ко дну бака. Всегда ко дну. Об этом мы ещё поговорим.',
    ],
    talkLinesPost: [
      'Выборка закрыта. Знак читается вниз, коллега, и мне это нравится не больше вашего.',
      'Контейнеры на полке. Берите, если снова пойдёте за срезом: без тары образец доезжает падалью.',
      'Яков просил передать: журнал он завёл отдельный, и вашу фамилию туда вписал чернилами.',
      'Колония спокойна. Спокойна — значит смотрит в одну сторону, и сторона эта под нами.',
    ],
    talkQuestResponse: 'Яков предупредил, но я не поверил, пока не поднёс пластину к стеклу. Колония выстроилась в тот же знак за одиннадцать секунд. Это не совпадение и не культ, коллега, это реакция. Мне нужна выборка: срезы тканей трёх разных видов, тогда я скажу, на что именно она отвечает. Где брать — ваша работа. Начнём с первого.',
    voiceTags: ['scientist', 'nii'],
  }));
