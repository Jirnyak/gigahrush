...
export const PLOT_NPCS: Record<string, PlotNpcDef> = {
  ...
  vanka: {
    ...
    talkQuestResponse: 'Яков послал? Иваном Захаровым назвал? Значит, всё ещё сердится за топологию. Слушай быстро: Петля был жильцом, потом тень пошла впереди. В широком коридоре держи свет, после рывка отходи, а холодный сгусток неси закрытым. Крышку не трогай.',
  },
...
export const PLOT_CHAIN: PlotStep[] = [
  ...
  // Step 4: Yakov → talk to Vanka Banchiny
  {
    giverNpcId: 'yakov',
    type: QuestType.TALK,
    desc: 'Найди Ваньку Банчина {dir}. Яков называет его Иваном Захаровым: бывший студент провалил базовую топологию и был исключён.',
    targetNpcId: 'vanka',
  ...
  // Step 14: Void warning → kill the Creator
  {
    giverNpcId: 'void_warning',
    type: QuestType.KILL,
    desc: 'Убей Творца в Пустоте. Держи укрытие между зелёными залпами; Жан отдаст пустотный шип.',
  ...
  // Step 15: Void warning → leave the return consequence behind
  {
    giverNpcId: 'void_warning',
    type: QuestType.FETCH,
    desc: 'Отдай пустотный шип Жану перед возвратом. Получишь святую воду, бинты и антидепрессант; не неси образец в жилую зону.',
