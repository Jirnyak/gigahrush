...
export const ITEM_TAGS: Record<string, readonly string[]> = {
  istotit_candle: ['istotit', 'candle', 'church_cache'],
  ...
  zhelemish_raw: ['zhelemish', 'raw', 'skin_status'],
  zhelemish_dried: ['zhelemish', 'treated', 'skin_status'],
  ...
  govnyak_roll: ['govnyak', 'contraband', 'pressure'],
  govnyak_brick: ['govnyak', 'contraband', 'trade'],
  govnyak_sample: ['govnyak', 'contraband', 'science'],
  govnyak_bad_batch: ['govnyak', 'contraband', 'bad_batch'],
...
export const ITEMS: Record<string, ItemDef> = {
  ...
  pills:     { id:'pills',     name:'Таблетки',     type:ItemType.MEDICINE, desc:'Обезболивающее из медшкафа. Лечит 25 HP, добавляет +5 ПСИ и немного тишины.',   spawnRooms:[RoomType.MEDICAL],                  spawnW:1, value:40, use:psiMedicine(25, 5) },
  antidep:   { id:'antidep',   name:'Антидепрессант',type:ItemType.MEDICINE, desc:'Психиатрическая роскошь для тех, кто слышит стены слишком отчётливо. +20 ПСИ.',           spawnRooms:[RoomType.MEDICAL],                  spawnW:1, value:80, use:psiMedicine(0, 20) },

  // ── Пасхальное (церковные припасы) ──
  holy_water:{ id:'holy_water', name:'Святая вода',  type:ItemType.MEDICINE, desc:'Освящённая вода из паломничьей фляги. Лечит 20 HP, +15 ПСИ; спорит с туманом недолго.', spawnRooms:[], spawnW:0, value:60, use:psiMedicine(20, 15) },
  kulich:    { id:'kulich',     name:'Кулич',        type:ItemType.FOOD,     desc:'Пасхальный кулич из храмовой кухни. Сытно, сладко, подозрительно светло.',  spawnRooms:[], spawnW:0, value:40, use:(e: Entity) => { if (e.needs) { e.needs.food = Math.min(100, e.needs.food + 35); e.needs.pendingPoo = (e.needs.pendingPoo ?? 0) + 25; } if (e.rpg) e.rpg.psi = Math.min(e.rpg.maxPsi, e.rpg.psi + 10); return 'Кулич согрел желудок и мысли: еда +35, ПСИ +10'; } },
  easter_egg:{ id:'easter_egg', name:'Пасхальное яйцо',type:ItemType.FOOD,  desc:'Крашеное яйцо с трещиной в форме подъезда. Восстанавливает 10 HP, +5 ПСИ.', spawnRooms:[], spawnW:0, value:20, use:(e: Entity) => { e.hp = Math.min((e.maxHp ?? 100), (e.hp ?? 0) + 10); if (e.rpg) e.rpg.psi = Math.min(e.rpg.maxPsi, e.rpg.psi + 5); return 'Скорлупа хрустнула, в голове стало ровнее: +10 HP, +5 ПСИ'; } },
  istotit_candle:{ id:'istotit_candle', name:'Истотитная свеча', type:ItemType.MEDICINE, desc:'Тонкая свеча из общего запаса. Успокаивает ПСИ на минуту, но вопрос "чья она" остается.', spawnRooms:[], spawnW:0, value:45, use:(e: Entity) => { if (e.rpg) e.rpg.psi = Math.min(e.rpg.maxPsi, e.rpg.psi + 8); return 'Свеча коптит золотым: ПСИ +8. Очередь за дверью этого не видела'; } },
  ...
  calm_brew:{ id:'calm_brew', name:'Успокоительный отвар', type:ItemType.DRINK, desc:'Тёплый отвар из аптечной кружки. Горчит так, будто знает диагноз.', spawnRooms:[RoomType.MEDICAL,RoomType.KITCHEN], spawnW:1, value:12, use:psiMedicine(0, 8) },
  ...
  antibiotic:{ id:'antibiotic', name:'Антибиотик', type:ItemType.MEDICINE, desc:'Таблетки против обычной грязи. Против самосборной — как повезёт.', spawnRooms:[RoomType.MEDICAL], spawnW:1, value:70, use:medicine(35) },
  morphine_ampoule:{ id:'morphine_ampoule', name:'Ампула морфина', type:ItemType.MEDICINE, desc:'Сильное обезболивание строгого учёта. Боль уходит, долг остаётся.', spawnRooms:[RoomType.MEDICAL,RoomType.HQ], spawnW:1, value:140, use:medicine(60) },
  psi_stabilizer:{ id:'psi_stabilizer', name:'ПСИ-стабилизатор', type:ItemType.MEDICINE, desc:'Гасит дрожь после сгустков. +30 ПСИ, минус один спор со стеной.', spawnRooms:[RoomType.MEDICAL,RoomType.OFFICE], spawnW:1, value:150, use:psiMedicine(0, 30) },
  sanitary_kit:{ id:'sanitary_kit', name:'Санитарный набор', type:ItemType.MEDICINE, desc:'Бинт, спирт, инструкция без подписи.', spawnRooms:[RoomType.MEDICAL,RoomType.STORAGE], spawnW:1, value:90, use:medicine(45) },
  antifungal_ointment:{ id:'antifungal_ointment', name:'Противогрибковая мазь', type:ItemType.MEDICINE, desc:'Пахнет подвалом и формальным спасением.', spawnRooms:[RoomType.MEDICAL,RoomType.BATHROOM], spawnW:1, value:55, use:medicine(20) },

  // ── Оружие ближний бой ──
  pipe:      { id:'pipe',      name:'Труба',        type:ItemType.WEAPON,    desc:'Тяжёлая труба с запахом стояка. Урон 20. Длинный замах. Прочность 55', spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE], spawnW:1, value:40 },
  wrench:    { id:'wrench',    name:'Ключ гаечный', type:ItemType.WEAPON,    desc:'Надёжный ключ слесаря. Урон 14. Прочность 85. Чинит хуже, чем убеждает',     spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE], spawnW:1, value:30 },
  knife:     { id:'knife',     name:'Нож',          type:ItemType.WEAPON,    desc:'Кухонный нож из общей кухни. Урон 8. Быстрый аварийный бой. Прочность 35',   spawnRooms:[RoomType.KITCHEN],                  spawnW:1, value:15 },
  rebar:     { id:'rebar',     name:'Арматура',     type:ItemType.WEAPON,    desc:'Кусок арматуры из старой стены. Урон 27. Дальше трубы. Прочность 90', spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE], spawnW:1, value:80 },
  axe:       { id:'axe',       name:'Топор',        type:ItemType.WEAPON,    desc:'Пожарный топор. Урон 34. Короткий сильный удар. Прочность 70', spawnRooms:[RoomType.PRODUCTION],                  spawnW:1, value:140 },
  chainsaw:  { id:'chainsaw',  name:'Бензопила',    type:ItemType.WEAPON,    desc:'Бензопила. Урон 80. Очень быстро пилит и быстро ломается. Прочность 18', spawnRooms:[RoomType.PRODUCTION], spawnW:1, value:3000 },

  // ── Оружие огнестрельное ──
  makarov:   { id:'makarov',   name:'Макаров',      type:ItemType.WEAPON,    desc:'Пистолет ПМ. Урон 18. Точный дешёвый выстрел. Патроны 9мм',    spawnRooms:[RoomType.STORAGE],                  spawnW:1, value:200 },
  ppsh:      { id:'ppsh',      name:'ППШ',          type:ItemType.WEAPON,    desc:'Пистолет-пулемёт. Урон 8. Жрёт 9мм очередью', spawnRooms:[RoomType.STORAGE], spawnW:1, value:650 },
  shotgun:   { id:'shotgun',   name:'Обрез',        type:ItemType.WEAPON,    desc:'Обрез. Урон 9×6. Широкий коридорный стоппер. Дробь',               spawnRooms:[RoomType.STORAGE],                  spawnW:1, value:340 },
  nailgun:   { id:'nailgun',   name:'Гвоздомёт',    type:ItemType.WEAPON,    desc:'Промышленный гвоздомёт. Урон 14. Точный, но гвозди дефицитны',      spawnRooms:[RoomType.PRODUCTION],                  spawnW:1, value:300 },
  ak47:      { id:'ak47',      name:'Калашников',    type:ItemType.WEAPON,    desc:'АК-47. Урон 30. Сильный автомат под редкие 7.62мм', spawnRooms:[], spawnW:0, value:1200 },
  machinegun:{ id:'machinegun', name:'Пулемёт',     type:ItemType.WEAPON,    desc:'ПКМ. Урон 16. Безумная скорострельность и разброс. Ленточное питание', spawnRooms:[], spawnW:0, value:2800 },
  grenade:   { id:'grenade',   name:'Граната',      type:ItemType.WEAPON,   desc:'РГД-5. Урон 90 по площади. Кидай и прячься', spawnRooms:[RoomType.STORAGE], spawnW:1, value:100, stack:999 },
  gauss:     { id:'gauss',     name:'Гаусс-винтовка', type:ItemType.WEAPON,  desc:'Рельсотрон. Урон 140. Медленный точный выстрел. Энергоячейки', spawnRooms:[], spawnW:0, value:6000 },
  plasma:    { id:'plasma',    name:'Плазмаган',    type:ItemType.WEAPON,    desc:'Плазменное оружие. Урон 30. Быстрая неточная трата энергоячеек', spawnRooms:[], spawnW:0, value:3200 },
  bfg:       { id:'bfg',       name:'БФГ-9000',     type:ItemType.WEAPON,    desc:'BIG FUCKING GUN. Урон 240 по огромной площади. Энергоячейки', spawnRooms:[], spawnW:0, value:11000 },
  flamethrower:{ id:'flamethrower', name:'Огнемёт', type:ItemType.WEAPON,    desc:'Короткая струя огня. Урон 6 за плевок. Выжигает слизь и грибной налёт, быстро тратит бензин', spawnRooms:[], spawnW:0, value:2500 },

  // ── Физическое оружие AG07: только существующие механики ──
  hammer:   { id:'hammer', name:'Молоток', type:ItemType.WEAPON, desc:'Рабочий молоток. Урон 13. Быстрее ключа. Прочность 70', spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE], spawnW:1, value:30 },
  crowbar:  { id:'crowbar', name:'Лом', type:ItemType.WEAPON, desc:'Тяжёлый лом. Урон 28. Средняя дистанция, высокая прочность 105', spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE], spawnW:1, value:85 },
  sledgehammer:{ id:'sledgehammer', name:'Кувалда', type:ItemType.WEAPON, desc:'Медленная кувалда. Урон 50. Большой замах. Прочность 90', spawnRooms:[RoomType.PRODUCTION], spawnW:1, value:220 },
  fire_hook:{ id:'fire_hook', name:'Пожарный багор', type:ItemType.WEAPON, desc:'Самый длинный ближний бой. Урон 21. Прочность 80', spawnRooms:[RoomType.STORAGE,RoomType.CORRIDOR], spawnW:1, value:100 },
  entrenching_spade:{ id:'entrenching_spade', name:'Саперная лопатка', type:ItemType.WEAPON, desc:'Короткая военная лопатка. Урон 18. Надёжная. Прочность 90', spawnRooms:[RoomType.STORAGE,RoomType.HQ], spawnW:1, value:70 },
  bayonet:{ id:'bayonet', name:'Штык', type:ItemType.WEAPON, desc:'Узкий быстрый укол. Урон 16. Прочность 60', spawnRooms:[RoomType.HQ,RoomType.STORAGE], spawnW:1, value:90 },
  chain:{ id:'chain', name:'Цепь', type:ItemType.WEAPON, desc:'Ржавая цепь. Урон 18. Длинная и неровная. Прочность 75', spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE], spawnW:1, value:55 },
  metal_chair:{ id:'metal_chair', name:'Металлический стул', type:ItemType.WEAPON, desc:'Стул из актового ада. Урон 24. Хрупкий широкий удар. Прочность 32', spawnRooms:[RoomType.COMMON,RoomType.OFFICE], spawnW:1, value:25 },
  tt_pistol:{ id:'tt_pistol', name:'ТТ', type:ItemType.WEAPON, desc:'Пистолет ТТ. Урон 26. Мощнее ПМ, патроны 7.62 ТТ', spawnRooms:[RoomType.HQ,RoomType.STORAGE], spawnW:1, value:280 },
  nagant:{ id:'nagant', name:'Револьвер Наган', type:ItemType.WEAPON, desc:'Старый револьвер. Урон 34. Медленный точный выстрел', spawnRooms:[RoomType.OFFICE,RoomType.STORAGE], spawnW:1, value:280 },
  homemade_pistol:{ id:'homemade_pistol', name:'Самодельный пистолет', type:ItemType.WEAPON, desc:'Грубая однозарядная поделка. Урон 22. Сильный разброс. 9мм', spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE], spawnW:1, value:90 },
  toz_shotgun:{ id:'toz_shotgun', name:'ТОЗ', type:ItemType.WEAPON, desc:'Длинный охотничий дробовик. Урон 8x8. Туже и медленнее обреза. Дробь', spawnRooms:[RoomType.STORAGE,RoomType.HQ], spawnW:1, value:460 },
  harpoon_gun:{ id:'harpoon_gun', name:'Гарпун', type:ItemType.WEAPON, desc:'Для затопленных ходов. Урон 70. Медленный точный гарпун', spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE], spawnW:1, value:900 },

  // ── Патроны ──
  ammo_9mm:  { id:'ammo_9mm',  name:'Патроны 9мм',  type:ItemType.AMMO,     desc:'Девятка для ПМ, самоделок и ППШ. Маленькая валюта коридорной уверенности',                  spawnRooms:[RoomType.STORAGE], spawnW:1, value:3 },
  ammo_shells:{ id:'ammo_shells',name:'Дробь',       type:ItemType.AMMO,     desc:'Дробовые патроны для коридорных стволов',                     spawnRooms:[RoomType.STORAGE],                  spawnW:1, value:8 },
  ammo_nails:{ id:'ammo_nails', name:'Гвозди',      type:ItemType.AMMO,     desc:'Промышленные гвозди для гвоздомёта',                spawnRooms:[RoomType.PRODUCTION], spawnW:1, value:6 },
  ammo_762:  { id:'ammo_762',   name:'Патроны 7.62',  type:ItemType.AMMO,     desc:'Редкие винтовочные патроны для Калашникова',              spawnRooms:[RoomType.STORAGE], spawnW:1, value:14 },
  ammo_belt: { id:'ammo_belt',  name:'Лента 7.62',   type:ItemType.AMMO,     desc:'Пулемётная лента. Большой вес и большой расход',      spawnRooms:[RoomType.STORAGE], spawnW:1, value:80 },
  ammo_energy:{ id:'ammo_energy', name:'Энергоячейка', type:ItemType.AMMO,    desc:'Редкая энергоячейка для плазмы, гаусса и БФГ', spawnRooms:[RoomType.HQ], spawnW:1, value:180 },
  ammo_fuel: { id:'ammo_fuel',  name:'Канистра бензина', type:ItemType.AMMO,  desc:'Дефицитное топливо для огнемёта',     spawnRooms:[RoomType.PRODUCTION], spawnW:1, value:45 },
  ammo_762tt:{ id:'ammo_762tt', name:'Патроны 7.62 ТТ', type:ItemType.AMMO, desc:'Пистолетные патроны для ТТ', spawnRooms:[RoomType.STORAGE,RoomType.HQ], spawnW:1, value:8 },
  ammo_nagant:{ id:'ammo_nagant', name:'Патроны Наган', type:ItemType.AMMO, desc:'Старые револьверные патроны', spawnRooms:[RoomType.STORAGE,RoomType.OFFICE], spawnW:1, value:7 },
  ammo_harpoon:{ id:'ammo_harpoon', name:'Гарпуны', type:ItemType.AMMO, desc:'Тяжелые редкие гарпуны для водных ходов', spawnRooms:[RoomType.PRODUCTION], spawnW:1, value:35 },

  // ── Сгустки (ПСИ-руны) — очень дорогие, ультраредкие ──
  psi_strike:   { id:'psi_strike',    name:'Сгусток: Пси удар',        type:ItemType.WEAPON, desc:'Простейший мысленный снаряд. 2 ПСИ, 14 урона; стена вздрагивает вместе с целью',                                     spawnRooms:[RoomType.MEDICAL,RoomType.OFFICE,RoomType.COMMON], spawnW:1, value:120 },
  psi_rupture:  { id:'psi_rupture',   name:'Сгусток: Разрыв',          type:ItemType.WEAPON, desc:'Медленный взрыв ПСИ-энергии. 5 ПСИ, 18 урона по площади; не стойте рядом с аргументом',                    spawnRooms:[RoomType.MEDICAL,RoomType.STORAGE],               spawnW:1, value:450 },
  psi_storm:    { id:'psi_storm',     name:'Сгусток: Пси буря',        type:ItemType.WEAPON, desc:'Дорогая волна боли. 12 ПСИ, бьёт всех врагов в поле зрения и часть здравого смысла',                       spawnRooms:[RoomType.MEDICAL],                                spawnW:1, value:1800 },
  psi_brainburn:{ id:'psi_brainburn', name:'Сгусток: Выжиг мозга',     type:ItemType.WEAPON, desc:'Мгновенная смерть цели ≤ вашего уровня. 10 ПСИ',                     spawnRooms:[RoomType.MEDICAL],                                spawnW:1, value:3200 },
  psi_madness:  { id:'psi_madness',   name:'Сгусток: Безумие',         type:ItemType.WEAPON, desc:'Цель нападает на всех. 7 ПСИ, 15с',                                 spawnRooms:[RoomType.OFFICE,RoomType.COMMON],                 spawnW:1, value:650 },
  psi_control:  { id:'psi_control',   name:'Сгусток: Контроль',        type:ItemType.WEAPON, desc:'Цель становится союзником. 12 ПСИ, 15с',                              spawnRooms:[RoomType.MEDICAL],                                spawnW:1, value:3400 },
  psi_phase:    { id:'psi_phase',     name:'Сгусток: Фазовый сдвиг',   type:ItemType.WEAPON, desc:'Проходить сквозь стены. 12 ПСИ, 15с',                                 spawnRooms:[RoomType.STORAGE],                                spawnW:1, value:5200 },
  psi_mark:     { id:'psi_mark',      name:'Сгусток: Метка',           type:ItemType.WEAPON, desc:'Запомнить позицию для телепорта. 4 ПСИ',                              spawnRooms:[RoomType.MEDICAL,RoomType.OFFICE],                spawnW:1, value:240 },
  psi_recall:   { id:'psi_recall',    name:'Сгусток: Возврат',         type:ItemType.WEAPON, desc:'Телепорт к метке. 4 ПСИ',                                             spawnRooms:[RoomType.MEDICAL,RoomType.OFFICE],                spawnW:1, value:240 },
  psi_beam:     { id:'psi_beam',      name:'Сгусток: Хамехамеха',     type:ItemType.WEAPON, desc:'Мощный ПСИ-луч. 6 ПСИ за импульс, 20 урона по пути', spawnRooms:[RoomType.MEDICAL], spawnW:1, value:8500 },
  psi_concrete_splinter:{ id:'psi_concrete_splinter', name:'Сгусток: Бетонный осколок', type:ItemType.WEAPON, desc:'ПСИ-снаряд из памяти стены. 3 ПСИ, 22 урона', spawnRooms:[RoomType.MEDICAL,RoomType.PRODUCTION], spawnW:1, value:550 },
  psi_shadow_lance:{ id:'psi_shadow_lance', name:'Сгусток: Теневая пика', type:ItemType.WEAPON, desc:'Быстрый темный прокол. 5 ПСИ, 42 урона', spawnRooms:[RoomType.MEDICAL,RoomType.OFFICE], spawnW:1, value:1100 },
  psi_order_seal:{ id:'psi_order_seal', name:'Сгусток: Печать порядка', type:ItemType.WEAPON, desc:'Плотный взрыв печати. 7 ПСИ, 30 урона по малой площади', spawnRooms:[RoomType.OFFICE,RoomType.HQ], spawnW:1, value:1400 },
  psi_void_needle:{ id:'psi_void_needle', name:'Сгусток: Пустотная игла', type:ItemType.WEAPON, desc:'Редкий точный снаряд. 9 ПСИ, 80 урона', spawnRooms:[RoomType.MEDICAL], spawnW:1, value:3000 },
  psi_meat_hook:{ id:'psi_meat_hook', name:'Сгусток: Мясной крюк', type:ItemType.WEAPON, desc:'Тяжелый органический снаряд. 4 ПСИ, 34 урона', spawnRooms:[RoomType.MEDICAL,RoomType.STORAGE], spawnW:1, value:850 },
  psi_siren_pulse:{ id:'psi_siren_pulse', name:'Сгусток: Сиренный импульс', type:ItemType.WEAPON, desc:'ПСИ-снаряд с малым разрывом. 6 ПСИ, 28 урона', spawnRooms:[RoomType.HQ,RoomType.OFFICE], spawnW:1, value:1800 },
  ...
  manometer:{ id:'manometer', name:'Манометр', type:ItemType.MISC, desc:'Стрелка показывает давление и настроение труб.', spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE], spawnW:1, value:35 },
  ...
  shark_scale:{ id:'shark_scale', name:'Акулья чешуя', type:ItemType.MISC, desc:'Легендарный мокрый трофей. Возможно, подделка.', spawnRooms:[], spawnW:0, value:1000 },
  ...
  clean_health_cert:{ id:'clean_health_cert', name:'Справка об отсутствии заражения', type:ItemType.MISC, desc:'Сухая печать, мокрая подпись.', spawnRooms:[RoomType.MEDICAL,RoomType.OFFICE], spawnW:1, value:35 },
  ...
  quarantine_medcard:{ id:'quarantine_medcard', name:'Карантинная медкарта', type:ItemType.MISC, desc:'Карта пациента с красной полосой допуска.', spawnRooms:[], spawnW:0, value:45 },
  ...
  forged_stamp_sheet:{ id:'forged_stamp_sheet', name:'Лист с поддельной печатью', type:ItemType.MISC, desc:'Печать похожа на настоящую и поэтому опасна.', spawnRooms:[RoomType.OFFICE,RoomType.SMOKING], spawnW:1, value:70 },
  ...
  asbestos_cord:{ id:'asbestos_cord', name:'Асбестовый шнур', type:ItemType.MISC, desc:'Огнеупорная нить для горячих дверей.', spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE], spawnW:1, value:18 },
  sealant_tube:{ id:'sealant_tube', name:'Герметик', type:ItemType.MISC, desc:'Тюбик серого состава для щелей.', spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE,RoomType.BATHROOM], spawnW:1, value:20 },
  ...
  nii_sample_container:{ id:'nii_sample_container', name:'Тара НИИ для пробы', type:ItemType.MISC, desc:'Пустая опломбированная банка с актом ответственности. Сама ничего не ловит, но без неё образец становится кражей воздуха.', spawnRooms:[], spawnW:0, value:65 },
  nii_contraband_manifest:{ id:'nii_contraband_manifest', name:'Ведомость утечки НИИ', type:ItemType.MISC, desc:'Акт ревизии: опечатанные пробы вышли из НИИ как списанная тара и всплыли у рыночных посредников.', spawnRooms:[], spawnW:0, value:140 },
  nii_market_receipt:{ id:'nii_market_receipt', name:'Рыночная расписка НИИ', type:ItemType.MISC, desc:'Неровная расписка за серебристую пробу. Вместо подписи - номер шкафа и чужая печать.', spawnRooms:[], spawnW:0, value:95 },
  nii_forged_audit:{ id:'nii_forged_audit', name:'Подложный акт НИИ', type:ItemType.MISC, desc:'Акт списания, где заражённая проба стала пустой ампулой, а пустая ампула стала премией.', spawnRooms:[], spawnW:0, value:110 },
  ...
  valve_tag:{ id:'valve_tag', name:'Бирка вентиля', type:ItemType.MISC, desc:'Металл с чужим номером давления.', spawnRooms:[RoomType.PRODUCTION,RoomType.STORAGE], spawnW:1, value:12 },
  ...
  weapon_permit_signed:{ id:'weapon_permit_signed', name:'Разрешение на короткоствол', type:ItemType.MISC, desc:'Официальная бумага на один пистолетный случай. Автомат не признает.', spawnRooms:[RoomType.OFFICE,RoomType.HQ], spawnW:1, value:95 },
  weapon_permit_forged:{ id:'weapon_permit_forged', name:'Липовое оружейное разрешение', type:ItemType.MISC, desc:'Подделка под оружейный допуск. Печать ровная, риск кривой.', spawnRooms:[RoomType.OFFICE,RoomType.SMOKING], spawnW:1, value:58 },
  ammo_issue_order:{ id:'ammo_issue_order', name:'Ордер на выдачу патронов', type:ItemType.MISC, desc:'Одноразовый ордер на малую горсть девятки.', spawnRooms:[RoomType.OFFICE,RoomType.HQ], spawnW:1, value:72 },
