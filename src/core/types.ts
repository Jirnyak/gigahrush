...
export interface Quest {
  id: number;
  type: QuestType;
  /* Дающий. КАКОЕ это число — говорит `giverBySlot`, а не происхождение
   * задания: у авторского это слот личности, у процедурного он был номером
   * СУЩНОСТИ. Замерено: при перестройке этажа номер сущности сменился у 24
   * личностей из 24, а слот не сдвинулся ни у одной, — то есть задание,
   * взятое у обычного жильца, после поездки на лифте сдать было некому. */
  giverId: number;
  /** `giverId` — слот личности (`alifeId`), а не номер сущности. */
  giverBySlot?: boolean;
  giverName: string;
