...
**Личное отношение к фракциям не сохраняется — сохраняется только его дрейф.** Восемь ячеек личности (`alife.md`) выводятся из `alifeSeed` и `id` детерминированно, поэтому при загрузке они пересчитываются, а в сейв уходит лишь НАКОПЛЕННОЕ отклонение от рождённого числа: поле `factionAttitude` в `AlifeNpcOverride`, массив по ячейке на фракцию, и только у тех личностей, у кого отклонение ненулевое. На чистом прогоне таких записей нет вовсе. Восстановление кладёт отклонение поверх пересчитанной базы и строго ПОСЛЕ восстановления собственной фракции: её запись рождает ячейки заново. Поле необязательное и старую форму сейва не ломает.
