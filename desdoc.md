...
- Использовать строго шрифт `Press Start 2P` с идеальными метриками кириллицы и палитру VHS/CRT терминала. Плоские фоны заменять на анимированный `drawNeuroPanel`.
...
## 28. База Ликвидаторов: форт и арена (замысел владельца, 2026-08-24)

Этаж `liquidatorbase` (z = −16) сейчас заглушка, и это измерено, а не на глаз.

| этаж | проходимо | комнат | медиана комнаты | крупнейшая |
| --- | --- | --- | --- | --- |
| жилой (референс) | 404 728 | 10 121 | **21 клетка** | 17×17 = **0.1%** этажа |
| коллекторы | 262 528 | 3 125 | 20 клеток | 50×50 = 1.0% |
| **База Ликвидаторов** | **4 992** | **4** | **1 500 клеток** | 50×50 = **50%** этажа |

Проходимого объёма в 81 раз меньше жилого, комнат в 2530 раз меньше, медианная
комната в 71 раз больше, половину этажа занимает один зал. Это ровно та болезнь,
которую владелец назвал неприемлемой: «генерится на весь 1024×1024 один пустой
большой зал, и типа весь этаж». Референс плотности и размеров комнат — жилой.

### Форт

Четверть этажа в центре (порядка 500×500) — форт гарнизона ликвидаторов:
плац, бараки, оружейные комнаты, стрельбища, штаб командира гарнизона, и в
сердце форта — арена. Остальные три четверти — земли диких и мутантов: руины,
логова, подходы к стене. Форт читается как остров порядка среди чужой земли.

Комнаты форта нарезаются по мерке жилого: медиана около 20 клеток, ни одна
комната не занимает заметной доли этажа. Арена — единственное исключение, и
даже она обязана быть долями процента, а не половиной.

### Арена

Образец, названный владельцем, — арена Обливиона.

- Боец арены — это ЛИЧНОСТЬ A-Life с флагом, а не спавн. Лестница строится по
  силе: от слабейшего до действующего чемпиона.
- Игрок дерётся сам. Убил чемпиона в бою на арене — сам стал чемпионом.
- **Титул надо держать:** претенденты приходят вызывать игрока, как приходил он.
- Флаг бойца арены **редко переходит другим личностям по холодной симуляции
  A-Life во всём мире**, не только на базе, и с очень малой вероятностью. Новых
  людей при этом не рождается — просто кто-то решается выйти на песок. Закон
  «никакого добора фонового населения» не нарушен, а лестница не кончается
  навсегда после того, как игрок вырезал ростер.
- Ставки на чужие бои и бесконечные бои с мутантами остаются отдельными
  режимами.
- Марко Лоло и мастер арены — на месте, при арене.

**Отгружено 2026-08-24.** Все шесть пунктов выше живут в `systems/arena_ladder.ts`,
флаг и титул — в личности A-Life и в секции сейва (`SAVE_SHAPE_VERSION` 25 → 26).
Награда чемпиона (`systems/arena_rewards.ts`) больше не висит без вызова: её условие
победы — смерть действующего чемпиона от руки игрока на песке. Подробности
реализации и границы — `manifest.md`, «Лестница арены»; форма сейва — `save.md`.

### Что уже сделано по этому этажу

Защита разжата: `aptMask` покрывал 99.9% этажа (4955 клеток из 4961) при медиане
0.0% по остальным, и шахтам лифтов было некуда сесть; защищённым остался только
гермоштаб. Медпункт открылся — 500 клеток из 500 были недостижимы, двери штаба и
медпункта стояли на разных рядах. Арена перестала называться игроку
`liquidator_arena_main`. Трибуны легли рядами через один с поперечными
проходами; при этом замерено, что `Feature.CHAIR` вообще не блокирует путь, и
записанная ранее причина «зал непроходим из-за стульев» неверна.
