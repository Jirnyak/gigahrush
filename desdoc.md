...
Версия: 4.8
Дата: 2026-05-23
Статус: planning/design snapshot after route persistence pass; не authoritative факт-карта билда
...
Исторические агентские prompts/status/logs, batch-файлы и старые root-планы теперь лежат в `appendix.md`; исходники сохранены в `gatbage/`. `Docs/ScenarioWriters/` активен, не archive: это рабочие сценарные пакеты для тона, NPC voices, barks, rumors, notes и quest copy. Этот документ и `README.md` должны получать только сжатые важные выводы, а не новые папки логов.
...
- 25 базовых монстров и 23 варианта.
- 16 шагов основного сюжета.
- 313 runtime plot/side NPC ids после production manifests: 9 базовых и 304 content registrations.
- 372 runtime side quest steps.
- 201 системный assignment template.
- 17 ресурсов экономики, 12 описаний производств, 19 production recipes, контейнеры и production ticks.
- 515 статических слухов плюс слухи из событий.
- 8 вариантов самосбора, 21 модификатор, 40 aftermath-последствий и 34 registered director beats.
- 8 номерных лифтовых инстансов.
- Вертикальный `FloorRun`: нормальный лифтовой span `z=-50..+50`, 26 authored/story stops, из них 6 story `FloorLevel` anchors и 20 design floor routes/generators, плюс 75 процедурных/fallback сидовых этажей без добавления новых `FloorLevel`.
- Посещенные route stops в текущей runtime-сессии больше не пересоздаются обычным лифтом: `systems/floor_memory.ts` держит отдельный живой `World` и неплеерские сущности по каждому stable floor key. Кровь, пули, двери, контейнеры, монстры и раскрытая карта принадлежат конкретному этажу, а самосбор/rebuild меняет активный `World` и делает результат новым слепком для следующего ухода с этажа. Полный снимок этажа пока не сериализуется в `localStorage`.
- Manifest-imported content modules по аудиту: `living` 31, `ministry` 15, `kvartiry` 21, `maintenance` 45, `hell` 7, `void` 9.
- `npm run content:audit` проходит без Errors, не видит unimported content modules и подтверждает, что `samosbornyy_ostov` имеет player-facing zone title. Его статические счетчики намеренно ниже runtime-счетов для динамических route-floor/contract registrations: 303 plot NPC ids, 348 side quest steps и 133 literal contract entries. Это не заменяет playtest: аудит доказывает связность registry/data, но не доказывает читаемость маршрута игроком.
...
- Старый root monster-design bible сохранен в `appendix.md` / `gatbage/monsters.md`; prompt/audit-файлы `Monster_N.md` архивированы как `gatbage/Monster_N.md`, и новые root `Monster_N.md` без кода больше не нужны. Новый монстр начинается с readable counterplay, reachable encounter и записи в существующих `entities`/`data`/`systems` rails.
...
- `src/data/procedural_floors.ts` - 10 geometry-профилей, 5 majority faction-профилей, 18 anomaly-профилей, danger и seed-biased loot/monster списки.
- `src/systems/procedural_floors.ts` - run seed, текущий `z`, 61 procedural spec, save/load и лифтовой маршрут.
...
- Персистентность маршрута теперь считается базовым дизайном вылазки: игрок постепенно раскрывает карту конкретных route stops, а кровь, пули, открытые контейнеры и живые угрозы должны оставаться на своем этаже. Самосбор не обнуляет весь этаж по факту тревоги: локальная волна вшивает измененную область в существующий этаж, а full/rebuild-событие делает замененный/сшитый результат новым слепком этого floor key.
...
Текущий `src/**/*.ts`: 389 файлов, 196 522 строки; без generated `src/data/bad_apple_frames.ts` это 388 файлов и 153 414 строк. Проект уже прошел рубеж 100k, поэтому 100k+ больше не цель сама по себе. Для ГИГАХРУЩА масштаб считается не строками, а количеством playable contracts, проверенных маршрутов и последствий в симуляции.
...
`scenarist.md` и `Docs/ScenarioWriters/` активны только как text-pass references. Они помогают чинить голос персонажей, barks, слухи, записки, квестовые строки, контракты, самосборные сообщения, монстр-подсказки и terminal text. Они не являются shipped-behavior source of truth и не создают backlog сами по себе: сначала проверять README, architecture, текущий `src/` и конкретную задачу.

Правила для сценарных документов:

1. Читать `Docs/ScenarioWriters/README.md`, потом только нужный packet.
...
4. Исторические `gatbage/istotit.md`, `gatbage/maronary.md`, `gatbage/veretar.md` и agent prompts дают мотивы, но не заменяют текущие `src/data/samosbor_variants.ts` и `src/data/samosbor_director.ts`.
...
| Вертикальный маршрут | `FloorRun` `z=-50..+50`, 20 design floors, 75 procedural/fallback floors, numbered lift anomalies, runtime floor memory | понятная навигация: seed, route id, danger, anomaly, return path, contract target | понимает ли игрок, куда его везет лифт и почему это не просто случайность? |
...
| Самосбор | 8 variants, 21 modifiers, 40 aftermath beats, 34 director beats, shelter/fog/seal loop | сделать warning/shelter/aftermath главной драмой каждой вылазки | переживает ли игрок самосбор решением, а не только бегом/DPS? |
...
| Экономика | 17 resources, 12 factories, 19 recipes, scarcity prices, containers, production ticks | owner/theft/audit, reward pressure, production outputs as route goals | создает ли дефицит маршрут, конфликт или сделку? |
...
| QA/docs | README fact map, architecture contract, content audit, 66 `*.test.ts` files, smoke script, `gatbage/` cleanup | close audit blockers, sync counters after verified code, route audit notes | можно ли доказать shipped behavior командой и маршрутом? |
...
- **Вертикаль масштабна, но не всегда намеренна.** `z=-44..40`, 18 design floors и 61 procedural floor должны читаться как план вылазки: куда, почему, опасность, как вернуться.
- **Плотность `MAINTENANCE` выше читаемости.** 45 manifest entries нужно группировать в 4-6 промышленных маршрутов, иначе игрок запоминает отдельные страшные названия, а не планирует экспедицию.
...
- New agents can start from `README.md`, `desdoc.md`, `architecture.md`, `scaling.md` and active reference folders, including `Docs/ScenarioWriters/`, without reopening archived AGXX logs.
...
- Keep this `desdoc.md` short enough to steer work; move only durable historical notes to `appendix.md`.
