...
Исторические агентские prompts/status/logs и batch-файлы остаются в `gatbage/history/root_notes/appendix.md` как сжатый контекст; исходники и старые root-планы сохранены в `gatbage/`. `gatbage/reference/scenario_writers/` активен, не archive: это рабочие сценарные пакеты для тона, NPC voices, barks, rumors, notes и quest copy. Этот документ и `README.md` должны получать только сжатые важные выводы, а не новые папки логов.

`gatbage/reference/ux_rework/` - активный пакет осторожных rework-брифов по первому публичному фидбеку. Его задача не "упростить игру" и не вырезать системы, а снизить первое отторжение от громоздкости: сделать первый экран, первую цель, первый NPC, бой, сообщения и опасности читаемыми без поломки уже работающего функционала.
...
- **Макро-жизнь не равна полной realtime-симуляции невидимых этажей.** A-Life держит до `1_000_000` procedural NPC identities при достаточной памяти и `100_000` fallback; off-floor NPC сохраняют identity, floor assignment, death/override facts и должны меняться только через bounded events, explicit migration, caravans, quests, resettlement или economy/faction consequences. Они не pathfind, не дерутся и не тикают needs вне активного этажа.
...
- `src/` содержит 569 TypeScript-файлов и 275 888 строк TypeScript без `dist/` и `node_modules`; без `src/data/bad_apple_frames.ts` это 568 файлов и 275 806 строк.
- Есть 387 `*.test.ts` `node:test` файлов. `tests/net-sphere.test.ts` покрывает optional Cloudflare Net Sphere config/schema/API path, procedural anomaly/rail train/zombie paths получили focused tests.
...
Текущий `src/**/*.ts`: 569 файлов, 275 888 строк; без `src/data/bad_apple_frames.ts` это 568 файлов и 275 806 строк. Проект уже прошел рубеж 100k, поэтому 100k+ больше не цель сама по себе. Для ГИГАХРУЩА масштаб считается не строками, а количеством playable contracts, проверенных маршрутов и последствий в симуляции.
...
| QA/docs | README fact map, architecture contract, content audit, 387 `*.test.ts` files, smoke script, `gatbage/` cleanup | close audit blockers, sync counters after verified code, route audit notes | можно ли доказать shipped behavior командой и маршрутом? |
...
- New agents can start from `README.md`, `desdoc.md`, `architecture.md`, root system docs and active reference folders, including `gatbage/reference/scenario_writers/`, without reopening archived AGXX logs.
