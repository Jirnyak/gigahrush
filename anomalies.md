...
- Запирание лифтов, spawn, hermetic/protected комнат или единственного прохода без repair/escape path.
  Конкретно: **рантайм-мутатор геометрии обязан держать буфер `ROUTE_ANCHOR_PROTECT_RADIUS = 3`
  вокруг `Cell.LIFT` и `Feature.LIFT_BUTTON`.** Защищать РОВНО клетку лифта мало —
  мутация переводит клетку ПОДХОДА, и подойти к шахте становится нечем. Замерено
  2026-08-27 на коллекторах: из четырёх кнопок лифта у одной мутабельный пол стоял
  вплотную, у двух — в радиусе трёх. Генерация под правило не попадает, и это не
  поблажка, а порядок шагов: `stampRouteLiftShafts` ставит шахты ПОСЛЕ
  `applyProceduralAnomalyProfile` и сам прорубает к ним подход, тогда как рантайм
  чинить некому. Замок — `tests/anomaly-halves-agree.test.ts`, он же требует, чтобы
  одноимённые числовые константы обеих половин одной аномалии совпадали.
- Полный скан окна ПЕРЕД гейтом каденции. Ловушка «Жизни Конвея»: выбор окна стоял
  перед гейтом, и пока поле активно, ранний выход отбивал его бесплатно — но стоило
  полю погаснуть, и окно выбиралось заново каждый кадр со сканом 16 384 клеток, по
  девять обращений к карте дверей в каждой. Около 150 тысяч обращений на кадр,
  до минуты подряд.
