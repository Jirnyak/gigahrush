...
test('using a fair ration coupon spends stock and grants ration output', () => {
  ...
  assert.equal(economy.floors[0]?.resources.drink_water.stock, 119);
...
test('water trades for a queue place only inside a ration queue room', () => {
  ...
  const economy = ensureEconomyState(state);
  assert.equal(economy.floors[14]?.resources.drink_water.stock, 121);
  assert.equal(economy.floors[14]?.resources.food.stock, 139);
...
test('bread can jump the ration queue at crowd risk', () => {
  ...
  const economy = ensureEconomyState(state);
  assert.equal(economy.floors[14]?.resources.food.stock, 141);
  assert.equal(economy.floors[14]?.resources.drink_water.stock, 119);
  const event = getRecentEvents(state)[0];
  assert.equal(event.type, 'player_use_item');
...
test('reporting a forged ration card resolves the audit into Kvartiry stock', () => {
  ...
  const economy = ensureEconomyState(state);
  assert.equal(economy.floors[14]?.resources.food.stock, 144);
  assert.equal(economy.floors[14]?.resources.drink_water.stock, 122);
...
test('stolen coupon events publish a ration-specific theft consequence', () => {
  ...
  const economy = ensureEconomyState(state);
  assert.equal(economy.floors[14]?.resources.drink_water.stock, 118);
});
