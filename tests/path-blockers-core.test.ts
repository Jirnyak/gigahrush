...
test('world path blockers are a flat 8-row-per-cell layer', () => {
  ...
  assert.equal(pathBlockerRowOffset(cellIdx, 5), cellIdx * PATH_BLOCKER_ROWS_PER_CELL + 5);
  assert.equal(getPathBlockerRow(world, cellIdx, 5), EMPTY_PATH_BLOCKER_ROW);
  ...
  assert.throws(() => setPathBlockerRow(world, cellIdx, 0, 256), RangeError);
});
...
test('path blocker row writes bump version only on actual changes', () => {
  ...
  assert.equal(setPathBlockerRow(world, cellIdx, 3, 0b0001_1000), true);
  assert.equal(world.pathBlockerVersion, before + 1);
  assert.equal(world.pathBlockerDirtyVersion, world.pathBlockerVersion);
  assert.equal(setPathBlockerRow(world, cellIdx, 3, 0b0001_1000), false);
  assert.equal(world.pathBlockerVersion, before + 1);
});
...
test('clearing path blockers at one cell clears all rows with one dirty bump', () => {
  ...
  setPathBlockerRow(world, cellIdx, 1, 0xff);
  setPathBlockerRow(world, cellIdx, 6, 0x81);
...
test('pathBlockedAt maps centers, subcell edges and torus wrapping', () => {
  ...
  setPathBlockerRow(world, cellIdx, 0, 0b0000_0001);
  setPathBlockerRow(world, cellIdx, 7, 0b1000_0000);

  assert.equal(pathBlockerSubcell(0.124), 0);
  assert.equal(pathBlockerSubcell(0.125), 1);
