 ...
 * 28 → 29 → 30 → 31 → 32 (2026-09-09): караванные полосы, память режиссёра
 * самосбора, выбранная цель квеста, адрес дающего у задания. Полосы поставок
 * (пошлины, открытость, устойчивость, счёт рейсов и налётов) до сих пор не
 * уезжали в сейв вовсе и молча обнулялись при загрузке. */
test('Demos persistent social batch owns the current save shape', () => {
  assert.equal(currentSaveShapeVersion(), 32);
});
