...
test('на выстрелы идут через этаж: шум ведёт маршрутом', () => {
  ...
  /* И патроны к нему: пустой ствол с 2026-08-29 не считается оружием, а на
   * выстрелы через этаж идёт именно ВООРУЖЁННЫЙ — безоружному там делать
   * нечего. Фикстура повторяет живого бойца, который выходит с магазинами. */
  armed.inventory = [{ defId: 'ammo_9mm', count: 24 }];
  armed.x = ROOM_X + 3.5;
  armed.y = ROOM_Y + 12.5;
