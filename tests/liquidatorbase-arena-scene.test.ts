 ...
 */
/* Сиды ТОЛЬКО для этого замка. Двух не хватает, и это доказано замером:
 * распределение пути после дуэли имеет длинный левый хвост — на тридцати сидах
 * трое падают ниже половины клетки в секунду, И ДО правки, И ПОСЛЕ. Порог,
 * настроенный по двум сидам, лежал ВНУТРИ этого хвоста, поэтому любая правка,
 * сдвигающая поток случайных чисел (например, состав карманов бойца), красила
 * замок случайным образом, ничего не сломав. */
const WALK_SEEDS = [20_881, 7, 11, 23, 41, 57, 73, 91];

/** Дефектное значение, ради которого замок и написан: 0.07 клетки в секунду —
 *  это «стоит столбом», человек не идёт никуда. */
const FROZEN_ON_LEASH = 0.1;
...
test('после дуэли выживший продолжает жить, а не стоит столбом на поводке', () => {
  ...
  for (const seed of WALK_SEEDS) {
    ...
    const rate = run.pathAfterDeath / after;
    rates.push(rate);
    /* Порог на КАЖДЫЙ сид держит только сам дефект: замороженный на поводке
     * человек давал 0.07 клетки в секунду. Тянуть его выше нельзя — хвост
     * распределения законный, победитель имеет право отстояться у стены. */
    assert.ok(rate > FROZEN_ON_LEASH,
      `${at}: за ${after.toFixed(1)}s после дуэли выживший прошёл ${run.pathAfterDeath.toFixed(1)} клетки (${rate.toFixed(2)} кл/с)`);
  }

  /* А вот ТИПИЧНОЕ поведение обязано быть далеко от дефекта, и судит его
   * медиана, а не отдельный сид. Замерено на тридцати сидах: 1.95 до правки
   * пачки и 1.96 после — то есть форма распределения от правок не зависит,
   * зависит только то, какой сид куда упал. */
  assert.ok(rates.length >= 4, `слишком мало состоявшихся дуэлей: ${rates.length}`);
  const sorted = [...rates].sort((a, b) => a - b);
  const median = sorted[sorted.length >> 1];
  assert.ok(median > 1, `медиана пути после дуэли ${median.toFixed(2)} кл/с — выживший в среднем не ходит`);
});
