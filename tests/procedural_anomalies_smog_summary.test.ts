...
import { makeGameState , makeProceduralSpecFixture } from './helpers';
...
test('summarizeProceduralSmog ignores non-smog anomaly', () => {
  ...
  setFloorRunState(state, { currentZ: testZ, specs: { [testKey]: makeProceduralSpecFixture({ key: testKey, z: testZ, anomalyId: 'none' }) } });
...
test('summarizeProceduralSmog handles smog active but ungenerated source', () => {
  ...
  const state = makeGameState();
  setFloorRunState(state, { currentZ: testZ, specs: { [testKey]: makeProceduralSpecFixture({ key: testKey, z: testZ, anomalyId: 'smog' }) } });
