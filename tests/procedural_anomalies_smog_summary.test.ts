...
test('summarizeProceduralSmog ignores non-smog anomaly', () => {
  ...
  setFloorRunState(state, { currentZ: testZ, specs: { [testKey]: { key: testKey, z: testZ, ordinal: 1, seed: 1, depth: Math.abs(testZ), danger: 1, geometryId: 'living_blocks', baseFloor: 'living', majorityId: 'citizens', anomalyId: 'none', title: 'none' } } });
...
test('summarizeProceduralSmog handles smog active but ungenerated source', () => {
  ...
  const state = makeGameState();
  setFloorRunState(state, { currentZ: testZ, specs: { [testKey]: { key: testKey, z: testZ, ordinal: 1, seed: 1, depth: Math.abs(testZ), danger: 1, geometryId: 'living_blocks', baseFloor: 'living', majorityId: 'citizens', anomalyId: 'smog', title: 'smog' } } });
