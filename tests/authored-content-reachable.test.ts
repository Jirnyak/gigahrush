 ...
 *  `arena_master` несёт Марко Лоло, `void_warning` и два клерка протокола следа
 *  ушли вместе с зачисткой Пустоты, Инга Трубная — вместе с вырезанной
 *  пневмопочтой, а второй амбиентный житель коллекторов — вместе со снесённой
 *  серобурмалиновой системой (2026-09-10, решение владельца: «не был задуман в
 *  игре»). Равенство, а не `<=`: десятая такая строка означает, что кто-то
 *  потерял человека, а не убрал дубль. */
...
const CONTENT_FLOOR = {
  // Пол опускают ТОЛЬКО вместе со снятой механикой, и каждый шаг вниз назван:
  //   −1 человек, −1 контракт — вырезанная пневмопочта (Инга Трубная, «Журнал
  //     из трубы»);
  //   −1 человек, −1 контракт — снесённая серобурмалиновая система
  //     (2026-09-10, решение владельца): амбиентный житель её POI и «Слепой
  //     соскоб серобурмалина».
  // Потерянный модуль так не выглядит: он роняет счёт десятками и молча.
  packages: 466,
  plotSlots: 475,
  sideQuests: 430,
  contracts: 203,
