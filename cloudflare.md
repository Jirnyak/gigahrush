...
- Панель и запросы разведены: терминал открывается везде, где фича вообще разрешена, а сеть требует бэкенда `/api/net`. На сборке без него (dev-сервер, itch) видно локальную личность, историю и статус `КАНАЛ ЗАКРЫТ` со строкой «в этой сборке нет сети»; опросы не идут, отправка в чат кладёт строку в локальный терминал и не дёргает `/chat`. Раньше `bindNetSphereInput` в таком билде просто не вешал обработчик, и клавиша выглядела сломанной. Строгие порталы (`yandex`, `gamepush`, `pikabu`) прячут фичу целиком — там панель не открывается.
