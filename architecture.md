...
- Desktop input treats `Esc` as browser/pointer-lock territory, not as a gameplay/window key. `Enter` opens the game menu from normal gameplay and is the universal accept/confirm key inside canvas menus; in Net Sphere it submits the current line and closes the terminal, including an empty line that sends nothing. `Space` is the universal back/close key for ordinary canvas menus, while Net Sphere keeps `Space` as chat text input, `Backspace` as chat erase and `Delete` as terminal close. Top-level shortcut menus close on their own shortcut key where that does not conflict with accept. Handled window keys are consumed so they do not fall through to the game menu. `E` stays reserved for in-world interaction: pickups, doors, NPCs, containers and aimed interactables.
...
- Save/load uses `systems/save_runtime.ts` and `systems/save_payload.ts`. Current save shape version is `15`; old or unversioned saves are rejected rather than migrated. `save.md` is the detailed persistence contract.
