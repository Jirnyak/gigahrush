...
#### Ядро двери — отдельный лист (2026-08-27)

`src/systems/actor_damage.ts` держит ЯДРО двери: `runActorDamageCore` (тип, броня, снятие здоровья,
толчок), `finishActorDeath` и средовой вход `damageActorByEnvironment`. Дверь по-прежнему одна —
`damageActor`; ядро вынесено по ИЗМЕРЕННОЙ причине, и она архитектурная. Полная дверь импортирует
`factions` (штраф отношениям, свидетели), `factions` тянет `ai/pathfinding`, `noise` и `inventory`,
а те лежат НАД клеточной опасностью: пока среда звала полную дверь, цикл рантайм-импортов рос
с 4 файлов до 18. Лист `factions` не импортирует, поэтому звать его может кто угодно снизу.

Правило слоя: **невозможность позвать общую дверь из-за цикла закрывается выносом её ядра листом,
а не заведением второй двери.** Социальная половина удара при этом не теряется — она нужна только
когда автор ЕСТЬ (`applyCombatRelationOutcome` стоит под `if (attacker)`), а у среды его нет по
определению. Средовой путь — это `damageActor` с пустой рукой, а не параллельный закон.

#### Граф связей в расстановке — инъекция, а не ребро (2026-09-12)

Раздача мест A-Life обязана знать, кто кому родня (`materializeAlifeFloorPopulation` →
`pool.takeNear`), но `demos_social` УЖЕ импортирует `alife`, и обратный импорт добавил бы
рантайм-цикл систем. Вынести ядро листом здесь нечего: нужен не кусок графа, а весь его
ленивый разбор строки. Поэтому применён второй способ из того же семейства — источник связей
регистрируется у потребителя (`registerAlifeSeatingBondSource` в `systems/alife.ts`, зовёт его
`demos_social` на импорте), ровно как самосбор получает генератор этажа, а дверь смерти —
обработчик. Цикл от этого не вырос: `check:invariants` показывает те же 4.

Правило выбора между двумя способами: **ядро листом — когда зовущему нужна часть общей
функции; инъекция — когда нужна вся чужая система, а владелец у неё один.** Признак второго
случая: источник регистрируется ровно в одном месте и подменяется только тестом.

Три правила средового входа, каждое снято с дефекта:

- **Тип урона обязателен и умолчания не имеет.** У кислоты, пара, обвала и разряда он разный,
  а «не назвал» значит «броня молча не сработала».
- **Объявленный клеткой урон — ПОТОЛОК.** Броня умеет только уменьшать. Конвейер умеет и
  увеличивать (`MonsterDef.damageFloor`, строка матрицы больше единицы), но это про ОРУЖИЕ В РУКАХ:
  за одиночным ударом стоит выбор игрока и патрон. Клетка тикает четырежды в секунду и выбора за
  собой не имеет — без потолка Туманная акула (`damageFloor[FIRE] = 1`) умирала бы от первого же
  такта парового сброса.
- **Порог выживания игрока принадлежит двери** (`lethal`), а не пятнадцати копиям
  `Math.max(1, hp - amount)` по вызывающим. По умолчанию среда добивает всех, кроме игрока: это
  снятый с места закон, а не поблажка. Кто убивает игрока — состав на рельсах — говорит это явно.

Голод и жажда осознанно остались мимо двери: у них нет ни автора, ни типа, ни брони — это не удар,
а исход. Тем же классом идёт цена добровольного действия (тухлая еда, снотворное, вскрытая проба):
`data/` не имеет права импортировать `systems/`, и это первый инвариант того же скрипта.

Механический замок — `npm run check:invariants`, проверка «урон мимо двери». С 2026-08-27 она
разбирает AST, а не строки: ловит уменьшение `hp` в любой обёртке (`-=`, присваивание, выведенное
вычитанием, сквозь скобки, `??` и `as`). Створка (`door.hp`) исключена по получателю — у неё нет
ни брони, ни смерти. Храповик `DAMAGE_DOOR_BASELINE = 5`, число только вниз.
...
### Floor Decision Seam Contract (2026-09-09)

Пятый шов, и он не хук, а ПОДПИСКА. Этажу часто надо не «сделать что-то в кадре», а
«узнать, что игрок принял решение, и объявить это миру». Своего рантайма для этого не
нужно: решения уже публикуются общей шиной — закрытое дело (`quest_completed` с
`data.sideQuestId`), вскрытый ящик (`item_stolen` / `container_looted` с `containerId`),
предъявленная бумага (`access_granted` / `permit_exposed` с `itemId`).

Образец — `gen/hell/choir_tax.ts`; по нему сделаны `gen/production_belt/decisions.ts`,
`gen/raionsovet_archive/decisions.ts`, `gen/bank_floor/decisions.ts`. Форма одна:

```txt
registerWorldEventObserver(handle)     // подписка снизу вверх, ребра systems → gen НЕТ
registerFloorScopedReset(...)          // состояние живёт ровно столько, сколько мир
bind<Floor>Decisions(world, state)     // зовёт генератор, когда ящики и линии готовы
```

Правила, и каждое куплено дефектом:

- **Сторож этажа — первой строкой.** Дела, кражи и бумаги есть на каждом этаже.
- **Собственный факт обратно не заходит.** Публикация несёт теги этажа и лежит на его же
  высоте; без сторожа рекурсии наблюдатель зациклится на своём выхлопе.
- **Решение объявляется один раз за прогон.** Повторное закрытие того же дела и второй
  заход в тот же ящик трубопровод не двигают.
- **Без привязки шов молчит.** Не «на всякий случай», а потому что состояние этажа —
  единственный источник id ящиков и комнат.
- **`z` берётся из константы этажа, не из литерала.** `publishRaionsovetArchiveEvent`
  печатал `z: 30` при этаже на `z = 22`, и это не было замечено ровно потому, что
  функцию не звали ни разу.
- **Зона обязана быть.** Событие без `zoneId` выпадает из зонных буферов шины, то есть
  слух о решении не дойдёт до соседей — ради которых шов и заводится. `quest_completed`
  координат не несёт вовсе, поэтому зона берётся по комнате линии.
- **Публикатор принимает источник объектом, а не `Entity`.** В момент подписки живой
  сущности под рукой нет. Тот же приём у `publishDarkMetroAmbushWarning`.
...
| Вещь объявляет себя меткой | `data/items.ts`, `itemIdHasTag` | Система НЕ разбирает id предмета. Метка спрашивается только через `itemIdHasTag`: метки живут в ДВУХ домах (`ITEM_TAGS` и поле `tags` определения), и чтение одного даёт пустой список. **Метка заводится под ВОПРОС системы, а не под смысл вещи:** общую `evidence` носят 65 предметов, и переиспользовать её вместо узкого набора значит молча расширить набор. **Ловушка: `ITEM_TAGS` кормит ещё и генератор процедурного арта** (`render/item_sprites.ts` выбирает силуэт по меткам), поэтому описательная метка «для порядка» меняет иконку — однажды так поехали пять наборов. Замок — `tests/item-self-declaration.test.ts`. |
| Имя комнаты — подпись, а не истина | `Room.type`, `Room.tags` | Поведение решают тип и тег, не подстрока в подписи. Замер 2026-09-12 на 28 148 комнатах: «архив»/«кабин» находили 59 комнат при 2004 настоящих кабинетах (пересечение 2), а «Караван»/«рынок»/«88» — 346 комнат, из которых ложные ВСЕ 346 («Жилая #88», «Зал #880»), при нулевом пересечении с торговыми. Тот же закон снял `ownerForLabyrinthRoomName`. Замок — `tests/room-identity-not-name.test.ts`. |
| Сгенерированный мир становится живым | `core/world.ts`, `replaceWorldFromGeneration(generation)` | Одна форма: новый этаж — НОВЫЙ объект `World`. Ветка «скопировать этаж внутрь прежнего объекта» снесена 2026-09-12 вместе с причиной: ею пользовался один путь из шести (загрузка этажа у гостя), и на нём молча отключалась вся разгрузка этажа, потому что она устроена на СРАВНЕНИИ объектов (`store.world() !== current`, `menuState?.world !== current`). Замер: аварийных щитков на новом этаже 8 вместо 3. |
| Регенерация «на выброс» | `systems/generation_runtime_guard.ts`, `withPreservedGenerationRuntime` | Игра генерирует этаж не только чтобы играть: при сохранении ей нужна чистая база для дельты. Обёртка снимает и возвращает ДВА рода состояния: все хранилища комнатного контекста разом (`snapshotAllWorldContexts`) и произвольные синглтоны модулей, которые завели личный страж (`registerGenerationRuntimeGuard`). Первое обязательно потому, что `register` при смене владельца стирает карту комнат целиком — без обёртки состояние живого этажа не терялось, а ПОДМЕНЯЛОСЬ состоянием выброшенной базы. Возврат в `finally`: падение генерации тоже обязано вернуть живой этаж. |
| Тело игрока приходит в мир | `main.ts`, `buildPlayerBody` + `capturePlayerCarry` | Пять входов (новая игра, загрузка сейва, смена этажа, возврат из Пустоты, локальный актёр гостя) собирает один шаг. Что уносится через границу мира — один список. Живое число, принадлежащее ПРЕДМЕТУ, в список не добавляется: магазин переносит та же пара, что и смена оружия (`stashEquippedMagazine` до, `loadEquippedMagazine` после), иначе заряд пропадает на каждом переезде. Шестой литерал — актёр ГОСТЯ у хозяина — свой намеренно. |
| Рука гостя = рука хозяина | `main.ts`, `applyPeerToolUse` | Гость — такой же член фракции `PLAYER`, и его действие идёт теми же шагами: инструмент (`handle*Tool`), удар в заблокированную дверь (`bashBlockedDoorFor`). Различие вынесено в параметры, а не в ветку: приёмник сообщений приходит аргументом (у гостя он выбрасывается), откат возвращается в секундах, звук зовётся под `isPlayerEntity`. Признак нарушения — своя реализация в ветке гостя: она всегда оказывается ОБЕДНЁННОЙ копией (у гостя не изнашивался инструмент и не начислялись отношения за уборку). |

Признак нарушения: `if (e.monsterKind === MonsterKind.X)` или `profile.id === 'x'` в общем
слое. Правильная форма — флаг в `aiFlags` и проверка `monsterHasAIFlag`. То же и для вещей
(`defId === 'x'` → метка), и для комнат (`room.name.includes('x')` → тип или тег).

Строки ниже первых шести добавлены 2026-09-12 и получены ЗАМЕРОМ, а не чтением: у каждой в
`problems.md` лежит число, которое её вызвало.
