...
- Save/load uses `systems/save_runtime.ts` and `systems/save_payload.ts`. Current save shape version is `12`; old or unversioned saves are rejected rather than migrated.
...
The player is also an A-Life actor for shared social math: `karma`, kill counters and rank score live on the player entity too, with `playerRelation = 100` for self-relation. The player is not counted as an ordinary NPC pool slot, but the top-100 A-Life rating includes the player with a real rank among alive persistent NPCs. Future possession would require separating controlled-entity ownership from `EntityType.PLAYER`, because current input, save, quest and event paths still assume the controlled actor is the player entity.
