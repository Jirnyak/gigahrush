...
Samosbor semantics are update-in-place from the memory system's point of view on every floor. The normal path starts from a random mutable source point on the current map, runs a bounded local wave, keeps the active `World`, and splices/regrows only the affected generated area from the current story/design/procedural/floor-instance generator through the same two-phase loading gate used by heavy transitions. If a local wave cannot start, the runtime still has a heavier rebuild fallback, so docs and tests should treat local splice as the intended path rather than the only possible path. The post-samosbor active world simply stays the authoritative state for the current key; there is no separate parked copy, and it is what the browser save delta-encodes if the player saves on this floor. Timing is route-depth based: active duration is a random 30 seconds to a depth-scaled cap up to 15 minutes at `abs(z)=50`, while cooldown is the inverse random interval from up to 30 minutes near `z=0` down to 1 minute at `abs(z)=50`.
