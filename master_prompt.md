...
### 1. Домашний этаж пакета NPC — ЗАКРЫТО 2026-08-16, 160 → 0

Замер воспроизвёл цифры передачи один в один: 473 пакета, 160 объявляли не тот этаж, на котором
спавнятся. Способ замера тот же, что и предписан: прогон всех 51 дизайн-этажа, сбор `npcPackageId`
каждой сущности, сверка с `placement.homeFloorKey`.

Правка: 153 пакета в 63 модулях `gen/*` получили свой этаж (`registerFloorSideQuest(KEY, ...)`
вместо `registerSideQuest(...)`, либо `homeFloorKey` в `registerAuthoredNpc`); ключ берётся из
`designNpcFloorKey('<этаж>')` — константой на модуль, без числовых координат. Семь оставшихся —
поимённо, по единственному правилу «дом там, где человек материализуется»: `liq_armorer/medic/
quartermaster` → база ликвидаторов, `hell_contact` → подад, `batushka`, `ag43_seva_cartographer`,
`stalker_mecheny` → жилой (все трое регистрируются модулями `gen/living/` и больше нигде не спавнятся).

Сверка численности до/после, резерв A-Life по бакетам (`buildAlifePopulationPlan`, seed 61061):

| бакет | было | стало |
| --- | --- | --- |
| `design:living` | 257 | 104 |
| `design:kvartiry` | 27 | 79 |
| `design:ministry` | 38 | 69 |
| `design:maintenance` | 16 | 64 |
| остальные 97 бакетов | +0..5 каждый | без сюрпризов |

Обычное население (`targetCount`) не сдвинулось нигде: сюжетные резервы вычитаются из общего числа,
а не из бакета. Замок — `tests/npc-home-floor.test.ts` (генерационная сьюта, ~77 с).

Ловушка, стоившая отдельного прохода: константу `HOME_FLOOR_KEY` нельзя ставить после последнего
импорта — в модулях контента импорты идут ВПЕРЕМЕЖКУ с кодом, и `registerAuthoredNpc` наверху файла
успевает выполниться до объявления (TDZ). Ставить сразу после первого импорта; тайпчек это ловит
(TS2448), но только если константа объявлена в том же файле.

Осталась вторая половина находки: **16 пакетов не спавнятся ни на одном дизайн-этаже** — строка
в `problems.md`, нужно авторское решение по каждому.
...
### 4. Клубок из 10 файлов в цикле — ЗАКРЫТО 2026-08-16, цикл 10 → 4

Разобран по ответственностям, как и требовалось (переносом листа он не брался — ни одно ребро
не роняло компоненту больше чем на 3). Две правки:

- словарь дельт матрицы отношений (`applyFactionRelationDeltas`, `applyTheftRelationPenalty`,
  `applyRoomMemoryRelationPenalty`, `applyInfrastructureRelationResponse`, тип `FactionRelationDelta`)
  живёт рядом с самой матрицей в `data/relations.ts`. Пропускам, контейнерам и аварийным панелям
  нужно подвинуть число в матрице, а не вся фракционная логика;
- снятие зонного тумана после смерти босса — лист `systems/fog_zone.ts`. Бой перестал импортировать
  весь `systems/samosbor` ради одной функции.

Осталось 4: `ai/combat ↔ ai/monster ↔ ai/micro_goals ↔ ai/khorovaya_matka` — взаимная логика внутри
одного слоя, а не чужой лист. `RUNTIME_CYCLE_BASELINE` опущен до 4, храповик держит.

Побочно вскрылось: `tests/systems-economy.test.ts` ждал караванный тариф в цене, а модуль караванов
приезжал случайно через срезанное ребро. Зависимость объявлена явно; разбор — `tests.md`.
...
- **A9 Orphan-table (owner: «сохранить фичи» ⇒ wire, НЕ delete-on-sight):**
  ...
  - **O6 [ЗАКРЫТО 2026-09-10]** test-only больше не test-only: `markov_log_speech.ts` СНЯТ (в игре не звали ни разу), `demos_ai_social.ts` ПОДКЛЮЧЁН к утилити-оценке NPC (`#87`). Остался `render/material_patterns.ts`.
...
- **I6 — переработать плейсхолдеры «не та дверь»** — `ЗАКРЫТО 2026-09-10`: система `wrong_door.ts` снесена целиком по решению владельца («точно нахер это говно»), вместе с подсистемой самосбора, двумя битами, слухом и цепочкой `wrongDoorIdx`. Убирать больше нечего.
