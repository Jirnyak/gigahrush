...
- `conveyor_sorter` - tags: `conveyor`, `items`, `industrial`, `movement`; runtime/rebuild: generation names conveyor rooms, paints loops and places controls/receivers; runtime pushes only the player on cached loop cells every 0.35s and supports a temporary control shutdown.
