...
- `living_blocks` - tags: `residential`, `civil`; runtime/rebuild: base `FloorLevel.LIVING`, `minZ=-3`, `maxZ=7`, panel/lino residential rooms with no geometry-specific runtime tick.
- `apartment_pressure` - tags: `residential`, `crowd`, `riot`; runtime/rebuild: base `FloorLevel.KVARTIRY`, `minZ=-15`, `maxZ=3`, dense residential/social room mix with no geometry-specific runtime tick.
- `communal_knots` - tags: `residential`, `crowd`, `queue`, `canteen`, `civil`; runtime/rebuild: base `FloorLevel.KVARTIRY`, `minZ=-11`, `maxZ=11`, communal/common/kitchen-heavy room mix. Current active generator effects are room mix, textures, tags and `Узел` room naming; the local `applyCommunalKnots()` helper exists but is not called by `generateProceduralFloor()`.
- `attic_weatherworks` - tags: `admin`, `roofline`, `antenna`, `wind`, `documents`; runtime/rebuild: base `FloorLevel.MINISTRY`, `minZ=-43`, `maxZ=-29`, pipe/concrete vent-camera rooms. Current active generator effects are industrial room sizing/decor, textures, tags and `Венткамера` naming; the local `applyAtticWeatherworks()` helper exists but is not called by `generateProceduralFloor()`.
- `archive_warrens` - tags: `admin`, `documents`, `archive`, `paper_dust`, `maze`; runtime/rebuild: base `FloorLevel.MINISTRY`, `minZ=-31`, `maxZ=-13`, office/storage archive room mix. Current active generator effects are room mix, textures, tags and `Стеллаж` naming; the local `applyArchiveWarrens()` helper exists but is not called by `generateProceduralFloor()`.
...
- `sump_causeways` - tags: `industrial`, `water`, `sump`, `blackwater`, `transit`, `abyss`; runtime/rebuild: base `FloorLevel.MAINTENANCE`, `minZ=21`, `maxZ=39`, high-danger flooded industrial route mix. Current active generator effects are industrial room sizing/decor, textures, tags and `Эстакада` naming; the local `applySumpCauseways()` helper exists but is not called by `generateProceduralFloor()`.
