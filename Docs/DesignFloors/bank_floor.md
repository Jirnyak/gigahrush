...
Status: implemented authored route floor. Route id: `bank_floor`. Anchor: `z=-22`. Base floor: `MINISTRY`. Shipped HUD name: `Банковский этаж`.
...
`bank_floor` is an inserted Ministry-band route stop between the `MINISTRY` story anchor at `z=-24` and `raionsovet_archive` at `z=-20`. It is not part of the original 4-z design-floor plan.
...
Use the normal lift route to `z=-22` or debug route teleport to `bank_floor`. Spawn starts in `Лифтовый вестибюль банка Б-22`.
