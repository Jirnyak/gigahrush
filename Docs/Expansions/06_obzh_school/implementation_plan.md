...
Статус: planning artifact for future implementation. Этот документ не утверждает наличие кода. Он задает phased MVP для `Docs/Expansions/06_obzh_school/expansion.md` и не требует нового `FloorLevel` до доказанного room/pocket slice.
...
Dump path для аварий будущей реализации: `Docs/AgentLogs/Dump_EXP06_SCHOOL.bin`. Документы не создают bin-файл заранее, но контракт требует dump при NaN, невозможном room id, отрицательном count или routeState, который не может завершиться.
