...
On NaN, impossible act, negative cooldown, invalid chain step, registry duplicate or adapter exception, the runtime must keep the game stable, write a trace entry and expose a dump path. When browser runtime cannot write `Docs/AgentLogs/Dump_DIRPASS_EXP00.bin`, debug must expose a copyable hex/base64 dump or structured trace print. "No explanation available" is not an acceptable failure mode for director behavior.
