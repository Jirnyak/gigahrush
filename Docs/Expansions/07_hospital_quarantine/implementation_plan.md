...
Статус: planning artifact for future implementation. Этот документ не утверждает наличие кода. Он задает phased playable MVP для `Docs/Expansions/07_hospital_quarantine/expansion.md` и не требует нового `FloorLevel` до доказанного room/pocket slice.
...
| Debug dump on invalid state | Last 300 medical telemetry frames written to `Docs/AgentLogs/Dump_EXP07_HOSPITAL.bin`. |
