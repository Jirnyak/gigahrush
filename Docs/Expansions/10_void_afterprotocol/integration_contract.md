...
Runtime keeps exactly 300 entries in a circular buffer. On impossible state, NaN, missing target for armed mark, or adapter exception, dump to `Docs/AgentLogs/Dump_EXP10_VOID.bin` if the runtime environment permits file writes; otherwise expose copyable debug output. The dump is diagnostic, not gameplay save.
