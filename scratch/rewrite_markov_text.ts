...
const content = `/* ── Bounded deterministic Markov NPC text core ───────────────── */

import { SeedRng, rng } from '../core/rand';
...
export function generateMarkovText(request: SpeechRouterRequest): SpeechRouterResult {
  ...
  if (skeletons.length === 0) {
    return {
      text: request.exactFallback || MARKOV_TEXT_DEFINITIONS.intentFallbacks[request.intent as string] || '...',
      source: 'fallback',
      intent: request.intent,
      tags: [],
      fallbackUsed: true
...
fs.writeFileSync('src/systems/markov_text.ts', content, 'utf8');
console.log('markov_text.ts rewritten.');
