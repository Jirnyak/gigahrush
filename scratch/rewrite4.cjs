...
// 1. Memory constants
const memStart = `const TREES_COUNT = 4;`;
const memEnd = `const _navQueue = new Int32Array(SW2);`;
...
`;
replaceBlock(memStart, memEnd, newMem + "const _navQueue = new Int32Array(SW2);");


// 2. LUT logic and bakeNavigationTree (and remove visitNavNeighbor)
...
// 3. Path reconstruction methods
// find the first occurrence of getLcaPathLength to end marker buildFlowFieldPath
const pathStart = `function getLcaPathLength(t: number, start: number, end: number): number {`;
const pathEnd = `function buildFlowFieldPath(`;

const newPath = `function getMacroLca(t: number, mStart: number, mEnd: number): number {
  _lcaMarkId++;
...
`;
replaceBlock(pathStart, pathEnd, newPath);
