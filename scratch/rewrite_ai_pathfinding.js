...
function bakeMacroFlowField(world: World, sources: number[], out: Uint8Array): void {
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    if (_macroPassable[s]) {
      ...
      _navQueue[_navTail1++] = s;
    }
  }

  while (_navHead1 < _navTail1) {
    const cur = _navQueue[_navHead1++];
    const cx = cur % W;
    const cy = (cur / W) | 0;
...
export function bakeNavigationTree(
  ...
  for (const r of world.rooms) {
    ...
    const roomField = new Uint8Array(MACRO_W2 / 2);
    bakeMacroFlowField(world, r.cells, roomField);
    _roomFlowFields.set(r.id, roomField);
  }
...
function dynamicMacroBfs(mStart: number, mEnd: number): number[] {
  ...
  _dynamicNavQueue[_navTail1++] = mEnd;
  _dynamicNavMark[mEnd] = _dynamicNavMarkId;
  _dynamicNavParent[mEnd] = -1;

  let found = false;
  while (_navHead1 < _navTail1) {
    const cur = _dynamicNavQueue[_navHead1++];
    if (cur === mStart) {
      found = true;
      break;
    }
    ...
    if (_dynamicNavMark[nW] !== _dynamicNavMarkId && _macroPassable[nW]) {
      _dynamicNavMark[nW] = _dynamicNavMarkId;
      _dynamicNavParent[nW] = cur;
      _dynamicNavQueue[_navTail1++] = nW;
    }
    if (_dynamicNavMark[nE] !== _dynamicNavMarkId && _macroPassable[nE]) {
      _dynamicNavMark[nE] = _dynamicNavMarkId;
      _dynamicNavParent[nE] = cur;
      _dynamicNavQueue[_navTail1++] = nE;
    }
    if (_dynamicNavMark[nN] !== _dynamicNavMarkId && _macroPassable[nN]) {
      _dynamicNavMark[nN] = _dynamicNavMarkId;
      _dynamicNavParent[nN] = cur;
      _dynamicNavQueue[_navTail1++] = nN;
    }
    if (_dynamicNavMark[nS] !== _dynamicNavMarkId && _macroPassable[nS]) {
      _dynamicNavMark[nS] = _dynamicNavMarkId;
      _dynamicNavParent[nS] = cur;
      _dynamicNavQueue[_navTail1++] = nS;
    }
  }
