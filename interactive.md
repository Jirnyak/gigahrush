...
## Запертый контейнер (с 2026-08-27)

Замок контейнера подчиняется общему закону о замках: **непроходимых нет, но обход
стоит ресурса**. Три канала, в порядке цены:

1. **Свой ключ** — мгновенно и тихо. Ключ признаётся по КЛАССУ ДОПУСКА ящика
   (`permitAccessTagsFromContainerTags` → `resolvePermitAccess`), а не по общему
   списку предметов. Одноразовые бумаги (пропуск, ордер, допуск) расходуются.
2. **Лом** — всегда доступен. Прочность замка = `lockDifficulty × 30`, поэтому тир 5
   даёт 150, ровно столько же, сколько запертая створка (`defaultDoorMaxHp`). Каждое
   нажатие публикует шум дверной громкости; сорванный замок помечает содержимое
   краденым, дальше работает готовая машина свидетелей, кармы и ревизии.
3. **Ключа может не существовать вовсе** — это законный случай, тогда только лом.

Два правила, за которые уже платили:

- **`lockDifficulty` — это ЦЕНА, а не запрет.** До 2026-08-27 её не читал никто:
  авторские тиры 2..5 в шести модулях контента были мёртвыми данными, а один
  предмет `key` отпирал ВСЕ 36 запертых контейнеров этажа и не расходовался.
- **Замок не зависит от того, что в нём лежит.** Класс `general_admin` приписан
  ящику по СОДЕРЖИМОМУ (тег `paper`/`permit`/`document` значит «внутри бумаги»), и
  совпадения по любому классу хватало: сейф с бумагами открывали 15 пропусков из 17,
  а такой же сейф без бумаг — два. Теперь узкое требование побеждает широкое, и
  `general_admin` учитывается, только если ящик не просит ничего конкретнее.
  Замок — `tests/container-locks.test.ts`, обе стороны правила.
