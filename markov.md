...
1. Развиваем словарь и веса переходов прямо в `markov_core_prototype.ts`. Мы не используем наивную Regex-замену слов (это просто замаскированный хардкод). Вместо этого граф строится **поверх классов атомов** (Theory of Categories). Узлы графа — это не слова, а абстрактные классы (например, `place_ref -> state_fact -> order_marker`).
2. Мы не завязываемся на конкретные ID или захардкоженные ситуации (например, `monster === 'Бенник'`). 
3. Мы опираемся на **системный контекст** (`MarkovContext`). Если в игру добавляется новый монстр, мы не меняем код генератора — статы монстра сами перетекут в параметр `dangerLevel`, и НПЦ начнут выбирать атомы с тегами "убить", "быстро" или "прятаться". 
...
### Универсальный Пайплайн (Математический Изоморфизм и Векторные Веса)

Для обучения генератора структуре и ритму живого русского языка мы используем внешние литературные и лорные корпуса (10 лучших источников):
- **«Пикник на обочине»** (`src/data/training_corpus/piknik.txt`) — классика атмосферы аномального выживания и хабара.
- **«Самосбор и Гигахрущ»** (`src/data/training_corpus/samosbor.txt`) — 1920+ фрагментов канона, тредов и вики-описаний.
- **«Лор Гигахруща»** (`src/data/training_corpus/gigahrush_lore.txt`) — 3880+ извлеченных предложений из игровых фактов, руморов, записок, диалогов и контрактов.
- **«Метро 2033»** (`src/data/training_corpus/metro2033.txt`) — гермозатворы, чистые фильтры, ордена, Ганза, туннельные перегоны и посты выживания.
- **«S.T.A.L.K.E.R.»** (`src/data/training_corpus/stalker.txt`) — аномалии, хабар, Долг, Свобода, байки у костра, выбросы, пси-излучение и детекторы.
- **«Град обреченный»** (`src/data/training_corpus/grad_obrechenny.txt`) — бессмысленный советский Эксперимент, бесконечный Город, Здания-Стена, инспекции и сюрреалистический индастриал-быт Стругацких.
- **«Советский Индастриал-Хоррор»** (`src/data/training_corpus/soviet_industrial_horror.txt`) — ГОСТы гермодверей ДУ-I-4 и АФУ-фильтров, трансформаторные ячейки, манометры, уплотнительные сальники, НИИ "Щит" и инструкции ГО.
- **«Улитка на склоне & Био-Хоррор»** (`src/data/training_corpus/strugatsky_bio_horror.txt`) — ползучая биомасса, Управление биозащиты, вязкий плазмодий, биоинспекции и советская НИИ-бюрократия.
- **«Обитаемый остров & Пси-Вышки»** (`src/data/training_corpus/inhabited_island_dystopia.txt`) — башни психоизлучения, Боевая Гвардия, Выродки, субкультуры подземелий и зачистки секторов.
- **«Солярис & Станционный Хоррор»** (`src/data/training_corpus/solaris_station_horror.txt`) — солярические фантомы, мимикрия стальных переборок, аварийные гермошлюзы и журналы дежурных океанологов.

Это **полностью расширяемая база** из 16,690+ предложений: любые открытые тексты в `src/data/training_corpus/` автоматически подтягиваются скриптами. Мы **никогда не парсим их напрямую в игру и не используем Regex-замены в рантайме**.

Команды управления обучающим корпусом и выявления математических весов:
```bash
# 1. Сбор и обновление фрагментов Самосбора из архива
node scripts/collect-samosbor-source-texts.mjs --max-per-source 50

# 2. Формирование текстов всех обучающих корпусов в src/data/training_corpus/
npx tsx scripts/fetch_training_corpus.ts

# 3. Векторная дистрибутивная токенизация и вывод категорий с PCA-осями
npx tsx scripts/discover_categories.ts

# 4. Извлечение синтаксических скелетов (паттернов) по всем 7 корпусам
npx tsx scripts/discover_patterns.ts

# 5. Прогон Марковского ядра на мульти-корпусном объединении
npx tsx scripts/markov_core_prototype.ts
```

Вместо жестких замен мы применяем **дистрибутивную семантику и изоморфизм** (прототип в [`scripts/discover_categories.ts`](file:///Users/jirnyak/Mirror/gigahrush/scripts/discover_categories.ts)):

1. **Математическая токенизация (Дистрибутивная семантика):** Алгоритм читает все файлы в `src/data/training_corpus/` и для каждого слова строит векторы контекста (соседние слова слева и справа).
2. **Формирование абстрактных множеств:** С помощью косинусного сходства алгоритм автоматически группирует слова со схожей синтаксической ролью в математические кластеры. Он сам выделяет категории `[ликвидатор, артем, стрелок, слесарь]`, `[гермозатвор, бункер, полис, трансформатор]` или `[хабар, пустышка, фильтр, дозиметр, тушенка]`.
3. **Изоморфизм категорий (Маппинг словарей):** Алгоритм смотрит на содержимое математического кластера и проецирует его на абстрактный тег: `<СУБЪЕКТ>`, `<МЕСТО>`, `<УГРОЗА>`, `<ЦЕННЫЙ_ПРЕДМЕТ>`, `<ФРАКЦИЯ>`, `<ИНСТРУМЕНТ>`. Марков-ядро вставляет слова из контекста игры в структуру фразы.
4. **Майнинг Синтаксических Паттернов (Извлечение Скелетов):** Скрипт (`scripts/discover_patterns.ts`) повторно проходит по тексту всех корпусов и выявляет естественные последовательности абстрактных категорий (например, `<СУБЪЕКТ> -> <МЕСТО> -> <УГРОЗА>`), игнорируя обычные слова. Это дает нам набор живых синтаксических скелетов, защищая генератор от "случайного блуждания" и нарушения логики фразы.
5. **Абстрактные Веса (Метод главных компонент / PCA):** Внутри каждой категории слова не однородны. Применяя PCA к кластеру, мы находим "математические оси дисперсии" (числа от -1.0 до 1.0). Например, на Оси 1 в кластере предметов "грязный бинт" лежит на `-0.9`, а "гаусс-пушка" на `+0.9`. Мы изоморфно связываем эту абстрактную математическую шкалу с конкретным Игровым Контекстом `itemPrice`. Ось "аномалия -> выброс Зоны / Самосбор" связывается с `dangerLevel`. Thus, элементы множества получают свои абстрактные веса.
6. **Направляемая Генерация (Весовая Матрица + BFS Эвристика):** Игровой движок формирует контекст (например, `Цена: 5000, Опасность: Высокая`). Модель выбирает синтаксический скелет, подходящий под контекст. Ядро генерирует **Весовую матрицу**, где вероятности переходов смещаются в сторону нужных абстрактных весов. Дополнительно применяется **Поиск в ширину (BFS)** по марковскому графу: алгоритм просчитывает дистанцию от текущего слова до следующего целевого узла в паттерне (например, до `<ЦЕННЫЙ_ПРЕДМЕТ>`). Пути, ведущие к цели, получают огромный математический бонус. Марковская цепь формирует фразу, где абстрактные слоты заменяются на те предметы Гигахруща (`гаусс-пушка`), чей игровой вес идеально подошел под математический вектор сгенерированного слота.

**Итог Двухслойной Архитектуры:** Марковская цепь целенаправленно движется по извлеченным из литературы синтаксическим скелетам, а промежутки заполняются лорными фактами Гигахруща с учетом осей PCA. Это гарантирует отсутствие "бредогенерации", сохраняя идеальный синтаксис и безупречно попадая в экономический и событийный контекст игры.
...
## Контракт скоринга: что отгружено (2026-08-16)

Формула `Score(A,C) = W_base + α·bitCount(Tags_A ∩ Tags_C) + β·CosineSim(V(A), V(C))`
работает целиком; до этого не работало ни одно из трёх слагаемых.

- **Теги — битовая маска, а не строки.** Канонический словарь (32 бита) живёт в
  `scripts/compile_markov_matrix.ts` и выпускается в матрицу как
  `COMPILED_TAG_BITS` (имя → маска). Рантайм резолвит имена через него и не
  хардкодит порядок бит. Синонимы (`thirst` → `water|need`) раскрываются в той же
  таблице, поэтому новая система добавляет тег в словарь, а не ветку в генератор.
  Единственная точка, где игровой контекст становится битами, —
  `buildContextTagMask()` в `src/systems/markov_text.ts`.
- **Теги корпуса выводятся механически:** из плейсхолдеров классов
  эквивалентности и из канонических слов игры (словарь основ строится из самих
  `CANONICAL_CATEGORIES`). Ручной разметки нет. Раньше каждому из ~15k
  предложений вешался литерал `['lore','neutral']`, которого не было ни в одном
  игровом контексте, — пересечение с контекстом было пустым всегда.
  Бит ребра выживает по большинству: простой OR насыщал частые истории.
- **PCA-оси выпускаются компилятором** в `COMPILED_CATEGORIES` и выводятся из
  источника, а не из таблицы: ценность — из `def.value` по логарифмической шкале,
  растянутой по самому дорогому предмету реестра; опасность монстра — из его
  `dmg`/`hp` по максимуму реестра; опасность аномалии — из её веса раскладки.
  Где числа нет, ось выводится из состава маски по объявлению осей.
- **Выбор атома вероятностный** (`resolveCategory`), а не argmin с полосой 0.1;
  `weight` атома наконец читается. Два прохода без аллокаций.
- **Ребро графа — одно SMI-число:** `count * 2^14 + индекс маски` в
  `COMPILED_MASK_TABLE`. Полная упаковка `count * 2^32 + mask` давала
  десятизначные уникальные числа и раздувала gzip.

Две ловушки, обе стоили механики:

- **Матрица компилируется на порядке 1** — истории графа и ключи
  `COMPILED_PATTERN_DISTANCES` это одиночные токены. Рантайм собирал
  двухсловный ключ: в графе его спасал откат, а в карте расстояний отката не
  было, промах давал `×0.01` всем кандидатам сразу, и скелеты не вели генерацию
  вообще (все интенты выдавали статистически одинаковый текст).
- **Скелеты ссылаются на категории, которых нет в корпусе.** Абстрагированы
  только `<SUBJ> <PLACE> <THREAT> <ITEM> <FACTION> <ACTION_VERB> <EMOTION>
  <NPC_NAME>`; `<ACTION>`, `<ZONE_TYPE>`, `<ANOMALY>`, `<DOCUMENT>` и прочие
  недостижимы обходом. Такая цель пропускается (`STEERABLE_TARGETS`), иначе
  `patternIndex` не двигается, конец фразы давится и реплика идёт до потолка в
  100 слов. Слот всё равно наполняется из `COMPILED_CATEGORIES`. Компилятор
  печатает список недостижимых целей и не выпускает пустые карты.

Спуск к цели скелета экспоненциальный (`SKELETON_GRADIENT = 8`): пологий
`100/(d+1)` тонул в разбросе `count`, обход гулял на постоянной дистанции.

## Точка сборки доменов: что отгружено (2026-08-20)

`MARKOV_DOMAINS` / `MARKOV_TEMPLATES` были объявлены, но рантайм их не читал
вообще: `src/data/markov_text.ts` отдавал ядру только `intentFallbacks`. Роутер
при этом честно объявлял источник `curated_pool` (`curatedPoolResult()` в
`speech_router.ts` зовёт ядро именно с ним), а ядро этот источник игнорировало и
всё равно шло по графу. То есть весь авторский корпус, вписанный в домены,
существовал как данные и не существовал как речь.

Отгружено:

- **`selectCuratedCorpusLine()`** (`src/systems/markov_text.ts`) — вторая ветка
  единого ядра, точная строка сценариста без обхода графа. Реестр доменов — её
  единственный адрес, поэтому корпус, не вписанный в домен, недостижим **по
  построению**, а не по забывчивости.
- **Полосы контекста — условие И, а не сумма баллов.** Строка со своими
  `contextTags` обязана совпасть по всем; `blockedTags` роняет её. Иначе реплика
  «Гражданским после фракционной драки...» (размечена `event`+`faction`) звучала
  бы там, где драки не было, и NPC ссылался бы на событие, которого в контексте
  нет.
- **Отбор** — верхний ярус по числу совпавших полос (адресная реплика профессии
  бьёт общую реплику домена), внутри яруса — вероятностный по `weight` строки
  через общий `rng()`. При одном сиде выбор воспроизводим.
- **`contextFacets()`** строит полосы из тега целиком, его префиксов и его
  сегментов: `need.food.low` даёт `need`, `need.food`, `need.food.low`, `food`,
  `low`. Корпус размечен листьями, а контекст приходит составным — без этого
  разбора они бы не встретились. Профессия и фракция приходят числами и дают
  полосу `occupation.5` / `faction.1`; самосбор и полосы нужды/опасности
  добавляют свои листья.
- **Ключ записи отделён от префикса** (`corpusRecord(..., keyTag)`): четыре
  корпуса профессий и пять фракционных ведут счёт по одному перечислению, значит
  и полоса у них обязана быть одна, а не производная от имени массива. Иначе
  запись доезжала бы до соседа по этажу вместо своего NPC.
- **Подключены 13 осиротевших корпусов** и заведён домен `occupations`:
  `OCC_LINES`, `CONTEXT_OCCUPATION_LINES`, `MINISTRY_OCC_LINES`,
  `CONTEXT_BARK_OCCUPATION_AMBIENT`, `MINISTRY_CLERK_LINES`,
  `CONTEXT_FACTION_LINES`, `CONTEXT_BARK_FACTION`, `CONTEXT_BARK_FACTION_HIDE`,
  `CONTEXT_BARK_FACTION_WOUNDED`, `CONTEXT_FACTION_EVENT_FACTION_LINES`,
  `CONTEXT_LIFT_ANOMALY_FLOOR_LINES`, `CONTEXT_MONSTER_KILL_FLOOR_LINES`,
  `MEDICAL_ROOM_LINES`.
- **Снята молчаливая обрезка корпуса.** `cap = 14` на корпус и `capPerKey = 5` на
  ключ отбрасывали хвост авторского текста — та же болезнь, что и корпус, не
  вписанный в реестр. Реестр — не бюджет, а адрес; строку роняет только чёрный
  список тона. Итог: **305 → 873 строки** в реестре.

Замок — `tests/markov-corpus-reachability.test.ts`. Он не перечисляет корпуса
поимённо: берёт все экспорты `data/markov_text.ts`, которые выглядят как корпус
реплик (массив строк или запись «числовой ключ перечисления → реплики»), и
требует, чтобы каждая пережившая чёрный список строка лежала в каком-нибудь
домене. Новый массив, написанный и не вписанный в `MARKOV_DOMAINS`, валит тест
сам, без правки списка. Рядом проверяются полнота объявления домена (теги,
интенты, fallback, совпадение интента строк, непустые полосы), адресность семьи
профессий, фракционные барки по обстановке, запрет ссылки на несуществующее
событие и детерминизм выбора от сида.
...
Отгружено 12 доменов: десять базовых из таблицы ниже, `occupations` (профессия
как полоса контекста, см. «Точка сборки доменов») и служебный
`procedural_skeletons` — он несёт атомы для обхода графа и корпуса не имеет.
...
| `occupations` | `occupation` из снимка NPC, министерский профиль, роль | чем человек занят, чем меряет день, что для него норма ремесла |
