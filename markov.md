...
Для обучения генератора структуре и ритму живого русского языка мы используем внешние литературные и лорные корпуса:
- **«Пикник на обочине»** (`src/data/training_corpus/piknik.txt`) — классика атмосфера аномального выживания и хабара.
...
- **«Советский Индастриал-Хоррор»** (`src/data/training_corpus/soviet_industrial_horror.txt`) — ГОСТы гермодверей и АФУ-фильтров, трансформаторные ячейки, манометры, уплотнительные сальники, НИИ "Щит" и бункерные инструкции ГО.
- **«Лор Гигахруща»** (`src/data/training_corpus/gigahrush_lore.txt`) — 2960+ извлеченных предложений из игровых фактов, руморов и контрактов.

Это **полностью расширяемая база**: любые открытые тексты в `src/data/training_corpus/` автоматически подтягиваются скриптами. Мы **никогда не парсим их напрямую в игру и не используем Regex-замены в рантайме**.
