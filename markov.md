...
Для обучения генератора структуре и ритму живого русского языка мы используем внешние литературные корпуса (например, «Пикник на обочине», «Метро 2033»). Это **полностью расширяемая база**: чем больше книг, тем богаче языковое ядро. Но мы **никогда не парсим их напрямую в игру и не используем Regex-замены**.

Вместо этого мы применяем **дистрибутивную семантику и изоморфизм** (прототип в [`scripts/discover_categories.ts`](file:///Users/jirnyak/Mirror/gigahrush/scripts/discover_categories.ts)):

1. **Математическая токенизация (Дистрибутивная семантика):** Алгоритм читает внешний текст и для каждого слова строит векторы контекста (соседние слова слева и справа).
2. **Формирование абстрактных множеств:** С помощью косинусного сходства алгоритм автоматически группирует слова со схожей синтаксической ролью в математические кластеры. Он сам выделяет категорию `[рэдрик, нунан, шухарт]` или `[хабар, пустышка, ржавчина]`, не зная правил грамматики.
3. **Изоморфизм категорий (Маппинг словарей):** Разработчик (или нейросеть) смотрит на содержимое математического кластера и дает ему абстрактное название, ориентируясь на контекст Гигахруща. Кластер имен становится `<СУБЪЕКТ>`, кластер хабара — `<ЦЕННЫЙ_ПРЕДМЕТ>`. Эти теги мы используем, чтобы Марков-ядро правильно вставляло слова из контекста игры в структуру фразы.
4. **Майнинг Синтаксических Паттернов (Извлечение Скелетов):** Скрипт (прототип `scripts/discover_patterns.ts`) повторно проходит по внешнему тексту и выявляет естественные последовательности абстрактных категорий (например, `<СУБЪЕКТ> -> <МЕСТО> -> <УГРОЗА>`), игнорируя обычные слова. Это дает нам набор живых синтаксических скелетов, защищая генератор от "случайного блуждания" и нарушения логики фразы.
5. **Абстрактные Веса (Метод главных компонент / PCA):** Внутри каждой категории слова не однородны. Применяя PCA к кластеру, мы находим "математические оси дисперсии" (числа от -1.0 до 1.0). Например, на Оси 1 в кластере предметов "ржавчина" лежит на `-0.9`, а "золото" на `+0.9`. Мы изоморфно связываем эту абстрактную математическую шкалу с конкретным Игровым Контекстом `itemPrice`. Ось "крыса -> мясорубка" связывается с `dangerLevel`. Таким образом, элементы множества получают свои абстрактные веса.
6. **Направляемая Генерация (Весовая Матрица + BFS Эвристика):** Игровой движок формирует контекст (например, `Цена: 5000, Опасность: Низкая`). Модель выбирает синтаксический скелет, подходящий под контекст. Ядро генерирует **Весовую матрицу**, где вероятности переходов смещаются в сторону нужных абстрактных весов. Дополнительно применяется **Поиск в ширину (BFS)** по марковскому графу: алгоритм просчитывает дистанцию от текущего слова до следующего целевого узла в паттерне (например, до `<ЦЕННЫЙ_ПРЕДМЕТ>`). Пути, ведущие к цели, получают огромный математический бонус. Марковская цепь формирует фразу, где абстрактные слоты заменяются на те предметы Гигахруща (`гаусс-пушка`), чей игровой вес идеально подошел под математический вектор сгенерированного слота.
