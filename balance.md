...
Цена рейса живёт там, где её списывают: сайд-квест `ag108_pay_queue_tariff` берёт 18 ₽
за водно-хлебную линию, контракт `caravan_buy_queue_seat` — 24 ₽ за место. Теневые копии
`feeRubles` / `seatFeeRubles` в `data/caravans.ts` удалены 2026-08-27: их не читал никто,
а для пяти линий и четырёх шаблонов они называли цену, которой не существует. Рекомендация
самого этого документа — «иначе не показывать цену» — тем самым выполнена.
...
| Караваны могут быть бесплатными (частично закрыто 2026-08-27) | цена обязана жить там, где списывается | тариф и место списывают деньги через квест и контракт; неподключённые `feeRubles` / `seatFeeRubles` удалены. Остаток: у четырёх линий платёжного пути нет вовсе | `caravan_paid_actions_without_cost = 0` |
...
## 14. Доля монстров: глубина плюс авторская опасность (2026-08-13)

До этой правки состав населения этажа зависел **только** от |z|: `monsterShareForRouteZ(z)` брал долю из кривой глубины, а авторское поле `danger` давало множитель 1.01 при danger 2 против 1.14 при danger 5, то есть не значило почти ничего. Следствие — вся верхняя половина башни была монстровым логовом вопреки собственному замыслу: крыша («воздух, антенны, видимость», danger 2) 96% монстров, наружный микрорайон («улицы, домики, тишина») 94%, антенный двор 88%, пионерлагерь («социальный лагерь») 81%.

Теперь `danger` читается как **отклонение от нормы для своей высоты**: `expectedDangerForRouteZ(z)` даёт 1 у жилого этажа и 5 у терминусов, а доля монстров умножается на коэффициент от этого отклонения. Этаж, чья опасность совпадает с глубиной, не меняется вовсе — в том числе единственный геймплейно проверенный жилой этаж (2.44% монстров до и после правки; это и было критерием безопасности).

Коэффициент асимметричный сознательно: вниз 0.25 за единицу отклонения, вверх 0.08. Вверх кривая уже насыщена (потолок 0.96), и симметричный коэффициент сплющил бы все глубокие этажи на потолок, выбив авторских NPC из Ада и Нижнего пропускника.

Итог по этажам: крыша 33%, наружный микрорайон 33%, антенный двор 36%, пионерлагерь 40%, спецприёмник 59%, министерство 55%, ад 84%. Абсолютное население верхних этажей вышло на порядок проверенного жилого (около 2000–2800 человек), поэтому нагрузка на AI не выросла — сменился состав, а не размер.

Оговорка про пояс |z| ≥ 44 снята 2026-08-13, но не так, как можно подумать по числам выше. Глушитель по поясу убран из `baseNpcTarget` — предикат населённости теперь один, `floorRunZAllowsNpcs()` (то есть только спуск ниже −48), и формула по высоте непрерывна. Безлюдность верхних этажей при этом сохранена полностью, просто она переехала из пояса в их собственные профили:

- крыша `npcTarget: 0` — открытая площадка терминуса, на ней не живут;
- наружный микрорайон `npcTarget: 0` — улицы и домики, из которых все ушли; тишина этажа означает брошенный город, а не спящий, и «население» там — монстры и авторские фигуры;
- чердак техслужб `npcTarget: 0` — служебный чердак;
- радоновый обменник `npcTarget: 420` — единственный верхний этаж с людьми: смена операторов заслонок плотнее, чем даёт одна высота.

Поэтому доли монстров из списка выше (крыша 33%, наружный микрорайон 33%) описывают состав, который сложился бы, будь на этих этажах обычное население; фактически они монстровые на 100% по замыслу. Численно правка не сдвинула ни один этаж маршрута — она убрала второй источник истины. Правило про низкую опасность осталось прежним и относится теперь только к безлюдным этажам: там она **прореживает этаж**, а не переселяет людей, и переливать освободившийся бюджет в монстров нельзя — иначе мирные этажи станут самыми враждебными на маршруте.

## 15. Размер населения: кривая по глубине с асимптотой в мягкий предел (2026-08-27)

Предыдущий раздел описывает СОСТАВ этажа (доля монстров). Его РАЗМЕР до этой правки не считался вовсе: `basePopulationTotalAtDefaultSoftLimit(z)` игнорировала `z` и возвращала сам мягкий предел акторов, 4096. Дальше цели людей и тварей умножались на классовые и авторские множители, и `fitActiveActorCounts` сажала сумму ровно на предел. Так стояли **двадцать четыре дизайн-этажа из пятидесяти одного**, а не один Ад: пионерлагерь, квартиры, больничный корпус, коммунальное кольцо, оранжерея, баня, склад Гильберта, тёмная пересадка, аттракторный двор, Ад, Нижний пропускник, Подад, часовня, хоррор, тёмный отсек, Пустота и другие.

Последствие не в числах, а в том, что мягкий предел — потолок, а не цель. Этаж, набивший его ровно, отдаёт из `entitySpawnSlots` ноль, и любой рантайм-спавн исчезает молча: волны шага удержания сюжетной цепочки в Аду (`holdSpawnMonsters`, `killPressure`), кат-сцены, караваны, прибытия фракций, самосбор. Ошибки нет, записи в лог нет — просто никто не появляется. И Ад при этом упирался в тот же потолок, что Пустота, хотя под ним ещё семь стопов маршрута.

Теперь размер задан явно и гладко. Свободный запас над населением равен целому МИНИМАЛЬНОМУ бюджету акторов у верха шахты и на спуске умножается на `MIN/DEFAULT` (= 1/4):
...
Своих чисел у кривой нет: обе константы — канонические границы того же бюджета. Кривая монотонна по `|z|`, гладкая, и к пределу только подходит: **3072** у жилого этажа (`|z| = 0`), 3401 у квартир, 3650 у министерства, 3719 у Ада, **3840** у терминусов — то есть даже на `|z| = 50` остаётся 256 свободных слотов. Каждый узел — степень двойки от бюджета акторов (запас идёт 1024 → 512 → 256), фактическое заселение плавает вокруг цели.

Подгонка `fitActiveActorCounts` теперь ужимает обе цели под бюджет ЭТАЖА, а не под мягкий предел, — иначе авторский множитель (`hell` носит `monsterMult: 2.0`, `void` — 2.5) переливает через край и возвращает этаж на потолок. То же сделано для процедурных этажей в `proceduralPopulationBudget`.

Замер по маршруту: этажей на потолке 24 → **0**. Ад 4096 → 3719 (запас 377 слотов), Нижний пропускник 3739, Подад 3758, часовня 3776, хоррор 3810, тёмный отсек 3825, Пустота 3840 — то есть ниже Ада действительно плотнее, как и задумано. Верх при этом узнаваем: министерство 2101 → 2091, коллекторы 1769 → 1563, квартиры 4096 → 3401, жилой этаж 2125 → 1767. Авторские оверрайды работают поверх кривой без изменений: крыша, наружный микрорайон и чердак несут `npcTarget: 0`, радоновый обменник — ровно 420, арена «стенка на стенку» — 0/0.

Замок — `tests/population-depth-curve.test.ts`: монотонность по `|z|`, недостижимость мягкого предела на всех 51 дизайн-этаже и всех процедурных, ненулевой запас на дне, живучесть авторских оверрайдов и пропорциональность бюджета настраиваемому софт-лимиту.

## 16. Броня: восемь ступеней и её выдача населению (2026-08-27)

Числа лестницы и правило колонки БИО — `items.md`, «Броня». Здесь только то, что относится
к балансу: почему цены такие и как броня доходит до NPC.

### Цены выведены из полос

Лестница: лёгкая 12 000 · ОЗК 16 000 · ТОК-200 18 000 · средняя 20 000 · ряса культиста 100 000 ·
тяжёлая 140 000 · ликвидаторская 200 000 · СЗК-9 500 000. Ни одного своего числа у правила нет,
всё считается из `ECONOMY_MONEY_BANDS`:

1. лестница монотонна по цене;
2. ни один комплект не влезает в `lootValueCap` полосы, в которой его впервые можно купить, —
   иначе он находка, а не покупка;
3. каждый комплект по карману своей полосе с торговым спредом покупки, иначе он недостижим;
4. броня не начинается раньше СРЕДНЕГО маршрута: `100 ₽` обязаны остаться значимыми деньгами.

Верх лестницы стоит столько же, сколько верх оружейной (гравилуч 500 000 ₽).

### Потолок снаряжения NPC был заперт на 1000 ₽

`calculateMaxLootValue` начинался с `Math.max(1000, …)`, и этот ПОЛ перебивал собственную кривую
до 49-го уровня. Следствие замерено: медиана цены ствола NPC **135–240 ₽**, максимум 1250 —
то есть вся оружейная лестница выше ~2 000 ₽ была для NPC невидима, сколько бы ни росли глубина
и ранг.

Теперь потолок выводится из тех же полос: **глубина даёт целую ступень, ранг — дробную**.
Лестница `GEAR_CAP_LADDER` — это `min(lootValueCap, ordinaryQuestCap)` полос E0..E4
(90 / 450 / 4 000 / 45 000 / 250 000), шаг `danger - 2 + rank`, интерполяция геометрическая,
`rank` = уровень личности, нормированный по `ALIFE_MAX_LEVEL`. Фракционные множители прежние
(ликвидаторы ×1.8, НИИ ×1.4). Своих чисел в файле лута нет ни одного.
Замок — `tests/npc-gear-value-cap.test.ts`.

### Брони на NPC не было вовсе

`armorDefId` теперь раздаётся через существующий `generateNpcLoadout`, и та же вещь кладётся
в инвентарь, чтобы падать с трупа. Четыре правила выдачи:

- **Гейт разведён по `armorType`.** `PLATE` — боевое снаряжение: `militarization × riskTolerance`,
  где милитаризация фракции нормирована по самой военной. `CLOTH` — рабочая одежда: один
  `riskTolerance`. Милитаризация — неверный посредник для ЗАЩИТЫ: химзащита и роба нужны тому, кто
  работает в опасном месте, а не тому, чья фракция воюет.
- **Адресация по рабочему месту:** `spawnRooms` предмета × `workRoomWeights` занятия. Промах даёт
  нейтральную единицу, а не ноль. Без этого штурмовая броня доставалась механику котельной чаще,
  чем охотнику гарнизона (864 против 0 на danger 5).
- **Множитель доступности** `min(1, потолок / цена дешевейшей брони)` СТОИТ ПЕРЕД отбором.
  До него ценовой потолок не гасил броню вовсе: взвешенный отбор нормируется по остатку и всегда
  выдаёт победителя, поэтому пул, задавленный целиком, всё равно кого-то возвращал. Симптом —
  циркач 41 %, уборщица 31 %, ребёнок 17.7 % в бронежилете за 12 000 ₽ на этаже опасности 1, где
  потолок снаряжения 90 ₽. Общее правило класса: **взвешенный отбор, нормирующийся по остатку,
  НЕ является порогом.**
- **Тег-лицензия `LOOT_LICENSE_TAGS`.** Обычный тег — предпочтение, лицензия — ДОПУСК: вещь с
  лицензионным тегом не попадает в пул вовсе, пока фракция не объявила этот тег весом больше нуля.
  Молчание анкеты = ноль, а не «как у всех». Сейчас лицензия одна — `psi_clot`, и объявляют её
  ровно две фракции: культ и НИИ.

Замки — `tests/npc-gear-license.test.ts`, `tests/npc-gear-workplace.test.ts`,
`tests/armor-price-ladder.test.ts` (последний держит «броня по-прежнему доходит до людей»).

**Открытый долг, не дефект:** химзащиты у населения практически нет. Замерено — на коллекторах ОЗК
носит 0 из 1297, на бане 5 из 521, на поясе 4 из 1185, в Яслях 26 из 2004; вся броня населения —
`armor_light` с БИО 5. Ставить лестницу химзащиты в выдачу населения на опасных этажах — отдельная
работа и решение владельца.

## 17. Коллекторы: опасность остаётся 3 (решение по замеру, 2026-08-27)

Поставленный вопрос — «поднять коллекторам `danger` с 3 до 4, чтобы рабочие получали химзащиту».
Ответ: **нет, и это характеристика этажа, а не дыра.**

Коллекторы людны, бедны и тихи. Замерено (сид 1337): **0 носителей химзащиты из 1354**, смертность
13.3–15.3 % — ровно в полосе соседей той же опасности (Служебный 13.1 %) и вдвое-втрое ниже
соседей четвёртой (31.3 %, 48.6 %). Доля монстров держится авторским `monsterMult: 0.1`, а не
опасностью. Опасности коллекторов калечат и держат (пар, борщевик, лотки), а не разъедают.

Отсутствие химзащиты — свойство ВСЕЙ полосы danger 3 (17 этажей), а не этого этажа: потолок
снаряжения на danger 3 ниже цены ОЗК для всех фракций, а на danger 4 уже выше. Поднять число
значило бы переписать полосу лута целиком ради одной вещи. Замок — `tests/maintenance-danger-band.test.ts`,
и его второй кейс отдельно опровергает мотив «поднять опасность ради лута»: полоса лута
на этой глубине от `danger` 3 → 4 не меняется вовсе.
