...
Ключевой принцип: **одновременно живет один этаж**. Активный `World` - это 1024x1024 тороидальное клеточное поле, где симулируются игрок, NPC, монстры, предметы, projectiles, двери, контейнеры, следы, кровь, пули, фракции, события, самосбор и локальная топология. Остальные этажи не pathfind, не дерутся, не тикают needs и не хранятся: этаж — чистая функция `(runSeed, z)` и детерминированно регенерируется при возврате, а его устойчивые факты живут в A-Life records, route specs, economy/faction/caravan summaries и explicit events. Floor memory и saved snapshot — только для одного активного этажа (в сейве — дельтой поверх регенерируемой базы, см. `save.md`).
...
- своя память: открытые двери, кровь, пули, украденные предметы, мертвые persistent NPC, последствия самосбора — держится, пока этаж активен, и переживает сохранение (дельтой в сейве); при возврате через лифт геометрия регенерируется идентично, а смерти и последствия возвращаются через A-Life.
...
- **Odd-numbered floors** (`Z % 2 !== 0`) are assembled by `src/gen/procedural_floor.ts` from the geometry `recipePool` of the rolled profile plus `src/gen/procedural_structure_library.ts` families, and then get a procedural anomaly. They do NOT import or splice pieces of authored design floors — `procedural_floor.ts` imports no design-floor generator, and ничего от них не наследуют: тем нет (снято 2026-08-25), а обстановку процедурный этаж берёт по своей геометрии (`PROCEDURAL_GEOMETRY_OBJECT_LAYER`).

Монстры на процедурном этаже выбираются из общей таблицы экологии его собственным составом (`monsterBiasKinds` и `monsterBiasTags` спеки, см. `ecology.md`) и расставляются стаями через общий модуль `src/gen/monster_packs.ts` — тот же, которым пользуются дизайн-этажи. Отдельной «процедурной» таблицы монстров и отдельного расселения нет.
...
**Этаж вправе быть замком вертикального маршрута — но замком по цене, а не по топологии.** Отгруженный образец — Перевалка (Z=-16, `src/gen/perevalka/`, 2026-08-27): ВСЕ шестнадцать лифтов вниз лежат внутри четырёх баз четырёх фракций, каждый за дверью `DoorState.LOCKED` со своим `keyId`. Контракт, который делает это законным, а не «непроходимой без ключа дверью»:

- **Этаж не решает, где лифты.** Их ставит общая система шахт (`src/world/route_lifts.ts`) на шаге манифеста, ПОСЛЕ авторских хуков территории. Этаж читает результат в `onAfterPopulate` и обносит его — не двигает, не добавляет, не удаляет. Собственных лифтовых координат в пакете этажа нет ни одной.
- **Связность не рвётся.** `classifyReachabilityCell` считает `LOCKED` проходимой с пометкой `gated by key`, поэтому проверки достижимости видят замок как замок и не прорубают обход мимо него.
- **Периметр — обычный бетон.** `aptMask` и гермостены на него ставить нельзя: они дают полный иммунитет к пробивному заряду, гравилучу и самосбору, то есть убивают один из путей игрока.
- **Один ключ — одна база, не одна дверь.** Все четыре лифтовые двери базы знают её ключ: игрок решает вопрос с базой, а не с дверью. Ключ лежит первым слотом в инвентаре держателя, поэтому его добывают поручением, боем, кражей и покупкой — и ни один из этих путей не пришлось программировать отдельно.
- **Замок проверяется генерацией**, а не чтением кода: `tests/perevalka-lift-lock.test.ts` требует шестнадцать запертых лифтов вниз, ноль отрезанных клеток пола, ноль замурованных комнат и ломаемый зарядом тамбур.

Это дополняет, а не заменяет data-owned route gates (пункт 6 правил ниже): гейт открывает НАПРАВЛЕНИЕ маршрута, а замок делает уже существующее направление платным.
