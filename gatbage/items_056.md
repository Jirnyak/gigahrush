# Items 056: p41_heavy_mg

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `p41_heavy_mg` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_2.md:106 (Liquidator weapons, ammo and combat roles)
...
| --- | --- |
| id | `p41_heavy_mg` |
| Russian name | 6П41 пулемёт |
| mode | new |
| intended type | WEAPON |
| gameplay role | Engineer HMG |
| reachability target | Exosuit/stationary only |
| implementation note | Defer if movement support is absent. |

## Worker Outcome 2026-05-25

Accepted as a route-only stationary weapon, not generic room loot. `p41_heavy_mg` now has an item definition, physical weapon stats, an `ammo_burn` role tier, ammo economy mapping and a faction-theft station on `production_belt` with belt ammo. No new movement/exosuit system was added; existing projectile and container theft rules carry the reachable path.
...
- src/data/item_packs/p41_heavy_mg.ts plus src/data/weapon_packs/p41_heavy_mg.ts for weapons
...
- Add or improve only `p41_heavy_mg`.
...
- Whether `p41_heavy_mg` was added, improved, merged into an existing id or rejected.
