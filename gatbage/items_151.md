# Items 151: white_concentrate

Status: merged into `grey_briquette`, not shipped as a separate id. Created 2026-05-24. Reviewed 2026-05-25.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `white_concentrate` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:109 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `white_concentrate` |
| Russian name | Белый концентрат |
| mode | new |
| intended type | FOOD |
| gameplay role | Common low-protein ration |
| reachability target | Kitchen/vending |
| implementation note | Avoid duplicate with grey_briquette. |

## Worker Outcome 2026-05-25

Merged into existing `grey_briquette` instead of adding `white_concentrate`. Current source already defines `grey_briquette` as `Концентрат-беляк`: a white daily concentrate ration with FOOD type, `concentrate` and `daily_ration` tags, kitchen/storage/common-room reachability, food-resource mapping, the standard `feed(18)` use handler, `concentrate_coupon` redemption, and concentrate press production.

No source item id was added. Adding `white_concentrate` would duplicate the same common low-protein kitchen/vending ration and split existing coupon, press, food-economy and test coverage without creating a new player decision.
...
- src/data/item_packs/white_concentrate.ts
...
- Add or improve only `white_concentrate`.
...
- Whether `white_concentrate` was added, improved, merged into an existing id or rejected.
