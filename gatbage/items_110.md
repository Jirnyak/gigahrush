# Items 110: hermodoor_service_log

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `hermodoor_service_log` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_3.md:102 (Documents, permits, bureaucracy and access goods)
...
| Field | Value |
| --- | --- |
| id | `hermodoor_service_log` |
| Russian name | Журнал обслуживания гермодвери |
| mode | new |
| intended type | MISC |
| gameplay role | Door repair/audit proof |
| reachability target | Maintenance/office |
| implementation note | Existing hermodoor_journal may cover; dedupe. |

## Worker Outcome 2026-05-25

Merged into existing `hermodoor_journal` instead of adding `hermodoor_service_log`. Current source already defines `hermodoor_journal` as the hermodoor service/audit log: it is reachable from office/storage/production rooms, mapped to documents, used by the hospital quarantine theft contract, and carries `document`, `hermodoor`, `service_log`, `repair`, `audit`, `maintenance`, `official` and `trade` tags.

No source item id was added. Adding `hermodoor_service_log` would duplicate the same door repair/audit proof role and split existing contract/document coverage.
...
- src/data/item_packs/hermodoor_service_log.ts
...
- Add or improve only `hermodoor_service_log`.
...
- Whether `hermodoor_service_log` was added, improved, merged into an existing id or rejected.
