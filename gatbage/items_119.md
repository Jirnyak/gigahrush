# Items 119: sealed_sample_jar

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `sealed_sample_jar` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:75 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `sealed_sample_jar` |
| Russian name | Опломбированная банка для пробы |
| mode | new |
| intended type | MISC |
| gameplay role | Official sealed sampleware |
| reachability target | NII/medical/HQ |
| implementation note | Maybe alias to existing nii_sample_container; dedupe first. |

## Worker Outcome 2026-05-25

Merged into existing `nii_sample_container` instead of adding `sealed_sample_jar`. Current source already defines `nii_sample_container` as official sealed NII sampleware: it is route/content reachable through `slime_nii` and sample handoff paths, carries `sampleware`, `official`, `document`, `sealed`, `legal_handoff` and audit/science/trade tags, and participates in both slime-sample and document resource pressure.

No source item id was added. Adding `sealed_sample_jar` would duplicate the same legal sealed sampleware role without creating a new player decision.
...
- src/data/item_packs/sealed_sample_jar.ts
...
- Add or improve only `sealed_sample_jar`.
...
- Whether `sealed_sample_jar` was added, improved, merged into an existing id or rejected.
