...
On invalid state, NaN, impossible severity, unknown service hard failure or corrupted save normalization, dump the buffer to `gatbage/history/agent_logs/Dump_EXP07_HOSPITAL.bin`. The dump path is fixed by agent id and must not write outside `gatbage/history/agent_logs`.
