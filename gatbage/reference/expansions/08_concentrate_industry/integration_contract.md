...
Critical runtime implementation must keep a fixed 300-entry circular buffer. On NaN or crash path it writes `gatbage/history/agent_logs/Dump_EXP08_INDUSTRY.bin`. Quantized fields are intentional: tiny binary state, fast dump, enough to explain production failure.
