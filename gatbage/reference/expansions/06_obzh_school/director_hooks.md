...
Owner: `DIRPASS_EXP06`
Scope: `gatbage/reference/expansions/06_obzh_school` only
...
School-local black-box telemetry remains separate from director trace. Future school evacuation code still owns `SchoolEvacTelemetryEntry[300]` and dumps `gatbage/history/agent_logs/Dump_EXP06_SCHOOL.bin` on impossible evacuation state. Director trace explains selection; school telemetry explains execution.
