...
No file under `gatbage/reference/scenario_writers/` is archived as of this map. If a packet becomes stale, mark its status here before moving or deleting anything.
...
Do not recreate `gatbage/history/agent_tasks`, `gatbage/history/agent_logs`, `gatbage/history/agent_prompts` or `gatbage/reference/design_floors/AgentPrompts` for scenario work. Keep durable conclusions in `gatbage/history/root_notes/appendix.md` only when they are worth preserving.
