...
For English term choices, use `gatbage/reference/localization/en-style.md`.

## Commands

- `npm run l10n:audit` scans player-facing text and compares it with `locales/*.json`.
- `npm run l10n:report` writes `gatbage/reference/localization/audit.json` and `gatbage/reference/localization/missing-<locale>.md`.
...
- `npm run l10n:extract` keeps the scenario-writer inventory at `gatbage/reference/scenario_writers/game_text_inventory.md`.
