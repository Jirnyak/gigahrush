# genfix_099: Темный отсек

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/099_z-48_design_darkness_Темный_отсек.png`
- Route: design `darkness`, z-48, index 099/101
- Design route id: `darkness`, role: позднее давление, danger 5
- Current metrics: rooms 17, doors 20, containers 10, entities 4098, reachable 53982, walls 994592, floors 53984
...
- Target control shares: граждане 6%, ликвидаторы 8%, культисты 24%, учёные 8%, дикие 36%, самосбор 18%.
...
```text
099_z-48_design_darkness_Темный_отсек.png
ИНТЕРЕСНО И оригинально но недоделано надо доделать в центре чтото есть интересно и какието сткрутуры но видно что недоделано
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 6%, ликвидаторы 8%, культисты 24%, учёные 8%, дикие 36%, самосбор 18%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 099 --out-dir tmp/floor-maps/genfix_099_after`
...
- `Add/update a focused design-floor test under tests/ and run it with ./node_modules/.bin/tsx --test <test-file>`
- `npm run typecheck`
