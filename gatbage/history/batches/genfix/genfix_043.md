# genfix_043: Перекрестки

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/043_z+8_design_manhattan_crossroads_Перекрестки.png`
- Route: design `manhattan_crossroads`, z+8, index 043/101
- Design route id: `manhattan_crossroads`, role: городской обход и развилки, danger 3
- Current metrics: rooms 288, doors 379, containers 4, entities 4057, reachable 344762, walls 703811, floors 344765
...
- Target control shares: граждане 44%, ликвидаторы 22%, культисты 10%, учёные 10%, дикие 14%.
...
```text
043_z+8_design_manhattan_crossroads_Перекрестки.png
ХОРОШИЙ ОРИГ ЭТАЖ
но - дороги обрезаны а должны тянуться до конца
в комнатах масштабы не учтены комнат должно быть больше и они должны быть меньше по ращмеру
```
...
- Road-grid hint: roads should continue to meaningful edges/termini; room blocks need smaller cells and more doors.
...
- Faction control starts from HQs and approximately matches target shares: граждане 44%, ликвидаторы 22%, культисты 10%, учёные 10%, дикие 14%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 043 --out-dir tmp/floor-maps/genfix_043_after`
