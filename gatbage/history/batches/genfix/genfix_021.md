# genfix_021: Министерство

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/021_z+30_story_story_ministry_Министерство.png`
- Route: story `story_ministry`, z+30, index 021/101
- Story route key: `story_ministry`
- Current metrics: rooms 609, doors 609, containers 30, entities 1770, reachable 394873, walls 653350, floors 395226
...
- Target control shares: граждане 48%, ликвидаторы 24%, культисты 8%, учёные 14%, дикие 6%.
...
```text
021_z+30_story_story_ministry_Министерство.png
хорошо 
возможно немного улучшить 
но в целом хороший этаж
длинные коридоры центральный крест можно продлить до упора 
плюс убедиться чтобы слишком не сливались маленькие коридоры
но в целом это хороший этаж
```
...
- `tests/ministry-geometry.test.ts`
- `factions.md`
...
- Ministry hint: the central cross may be extended, but preserve bureau density and avoid merging small corridors into one undifferentiated mass.
...
- Faction control starts from HQs and approximately matches target shares: граждане 48%, ликвидаторы 24%, культисты 8%, учёные 14%, дикие 6%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 021 --out-dir tmp/floor-maps/genfix_021_after`
...
- `./node_modules/.bin/tsx --test tests/ministry-geometry.test.ts`
- `npm run typecheck`
