# genfix_041: Ясли Тьюринга

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/041_z+10_design_turing_nursery_Ясли_Тьюринга.png`
- Route: design `turing_nursery`, z+10, index 041/101
- Design route id: `turing_nursery`, role: реакционно-диффузионные ясли, чаши, слизевые мосты, danger 4
- Current metrics: rooms 71, doors 69, containers 11, entities 2507, reachable 118450, walls 930124, floors 118452
...
- Target control shares: граждане 16%, ликвидаторы 14%, культисты 8%, учёные 50%, дикие 12%.
...
```text
041_z+10_design_turing_nursery_Ясли_Тьюринга.png
возможно чтото интересное но масштабы не учтены
алгоритм видно что не доделан 
надо доделывать
также машсштабы не учтены
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 16%, ликвидаторы 14%, культисты 8%, учёные 50%, дикие 12%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 041 --out-dir tmp/floor-maps/genfix_041_after`
...
- `./node_modules/.bin/tsx --test tests/turing-nursery.test.ts`
- `npm run typecheck`
