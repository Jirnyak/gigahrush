# genfix_065: Производственный пояс

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/065_z-14_design_production_belt_Производственный_пояс.png`
- Route: design `production_belt`, z-14, index 065/101
- Design route id: `production_belt`, role: лут и ремонт, danger 4
- Current metrics: rooms 113, doors 94, containers 16, entities 2569, reachable 149540, walls 899034, floors 149542
...
- Target control shares: граждане 14%, ликвидаторы 50%, культисты 6%, учёные 18%, дикие 12%.
...
```text
065_z-14_design_production_belt_Производственный_пояс.png
алгоритм недоделан
нужно добавлять деталей и микро
но потенциал есть надо доделать
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 14%, ликвидаторы 50%, культисты 6%, учёные 18%, дикие 12%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 065 --out-dir tmp/floor-maps/genfix_065_after`
...
- `./node_modules/.bin/tsx --test tests/production-belt.test.ts`
- `npm run typecheck`
