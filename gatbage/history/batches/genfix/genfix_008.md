# genfix_008: тихий блок: чердачные венткамеры, этаж ликвидаторов

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/008_z+43_procedural_z43_тихий_блок_чердачные_венткамеры,_этаж_ликвидаторов.png`
- Route: procedural `z43`, z+43, index 008/101
- Procedural profile: geometry `attic_weatherworks`, anomaly `false_safe_block`, majority `liquidators`, danger 4
- Current metrics: rooms 84, doors 205, containers 23, entities 1720, reachable 76471, walls 972089, floors 76487
...
- Target control shares: граждане 16%, ликвидаторы 53%, культисты 12%, учёные 8%, дикие 11%.
- Dominant owner: ликвидаторы; reason: procedural majority liquidators plus false-safe cult foothold.
...
```text
008_z+43_procedural_z43_тихий_блок_чердачные_венткамеры,_этаж_ликвидаторов.png
дефолт проблема аналогичный скучный пустые коридоры
ПЛОХО
```
...
- Процедурный профиль: geometry=attic_weatherworks, anomaly=false_safe_block, majority=liquidators, danger=4. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- Faction control starts from HQs and approximately matches target shares: граждане 16%, ликвидаторы 53%, культисты 12%, учёные 8%, дикие 11%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 008 --out-dir tmp/floor-maps/genfix_008_after`
