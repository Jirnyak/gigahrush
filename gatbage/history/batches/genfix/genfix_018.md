# genfix_018: архивные норы, гражданский этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/018_z+33_procedural_z33_архивные_норы,_гражданский_этаж.png`
- Route: procedural `z33`, z+33, index 018/101
- Procedural profile: geometry `archive_warrens`, anomaly `none`, majority `citizens`, danger 2
- Current metrics: rooms 225, doors 556, containers 17, entities 1260, reachable 50827, walls 997733, floors 50843
...
```text
018_z+33_procedural_z33_архивные_норы,_гражданский_этаж.png
макроструктура хорошая
но пока это пустые коридоры
нужно добавить микросткрутры (возможно совместить этот генератор как макро и в пустые квадраты микрокомнаты много соед 
возможно даже раз архивы то упорядочена сетка одинаковых квадратных комнат соед
```
...
- Процедурный профиль: geometry=archive_warrens, anomaly=none, majority=citizens, danger=2. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 018 --out-dir tmp/floor-maps/genfix_018_after`
