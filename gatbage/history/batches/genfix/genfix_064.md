# genfix_064: грибница: коммунальные узлы, гражданский этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/064_z-13_procedural_z-13_грибница_коммунальные_узлы,_гражданский_этаж.png`
- Route: procedural `z-13`, z-13, index 064/101
- Procedural profile: geometry `communal_knots`, anomaly `mushroom_mycelium`, majority `citizens`, danger 2
- Current metrics: rooms 124, doors 249, containers 38, entities 1102, reachable 167210, walls 881350, floors 167226
...
```text
064_z-13_procedural_z-13_грибница_коммунальные_узлы,_гражданский_этаж.png
потенциал есть но надо добавить разнообразия
```

## Diagnosis

- См. дословный фидбек: закрыть замечание видимой геометрией и проверяемыми метриками, а не только переименованием или лутом.
- Процедурный профиль: geometry=communal_knots, anomaly=mushroom_mycelium, majority=citizens, danger=2. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 064 --out-dir tmp/floor-maps/genfix_064_after`
