# genfix_057: Общежитие смены

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/057_z-6_design_obschezhitie_smeny_Общежитие_смены.png`
- Route: design `obschezhitie_smeny`, z-6, index 057/101
- Design route id: `obschezhitie_smeny`, role: сон, тихая кража, свидетели и укрытие, danger 2
- Current metrics: rooms 71, doors 107, containers 13, entities 2463, reachable 64040, walls 984534, floors 64042
...
- Target control shares: граждане 56%, ликвидаторы 16%, культисты 7%, учёные 8%, дикие 13%.
...
```text
057_z-6_design_obschezhitie_smeny_Общежитие_смены.png
макро интересное но нужно больше таких колец
алгоритм не доделан и больше стаков комнат
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 56%, ликвидаторы 16%, культисты 7%, учёные 8%, дикие 13%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 057 --out-dir tmp/floor-maps/genfix_057_after`
...
- `./node_modules/.bin/tsx --test tests/obschezhitie-smeny.test.ts`
- `npm run typecheck`
