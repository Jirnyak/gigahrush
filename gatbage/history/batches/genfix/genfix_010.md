# genfix_010: хладон: административные карманы, дикий этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/010_z+41_procedural_z41_хладон_административные_карманы,_дикий_этаж.png`
- Route: procedural `z41`, z+41, index 010/101
- Procedural profile: geometry `admin_pockets`, anomaly `hladon`, majority `wild`, danger 4
- Current metrics: rooms 98, doors 286, containers 27, entities 1788, reachable 111405, walls 937155, floors 111421
...
```text
010_z+41_procedural_z41_хладон_административные_карманы,_дикий_этаж.png
интересно но надо добавить больше комнат небольших
```

## Diagnosis

- См. дословный фидбек: закрыть замечание видимой геометрией и проверяемыми метриками, а не только переименованием или лутом.
- Процедурный профиль: geometry=admin_pockets, anomaly=hladon, majority=wild, danger=4. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 010 --out-dir tmp/floor-maps/genfix_010_after`
