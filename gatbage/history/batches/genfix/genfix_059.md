# genfix_059: Прачечная Пенроуза

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/059_z-8_design_penrose_laundry_Прачечная_Пенроуза.png`
- Route: design `penrose_laundry`, z-8, index 059/101
- Design route id: `penrose_laundry`, role: апериодичная прачечная, пар, тайники, danger 3
- Current metrics: rooms 62, doors 90, containers 5, entities 2219, reachable 56714, walls 991860, floors 56716
...
- Dominant owner: граждане; reason: base-floor inferred fallback.
...
```text
059_z-8_design_penrose_laundry_Прачечная_Пенроуза.png
интересное
алгоритм недоделан
в центре чтото интересное есть но пусто в осталбьном нужно доделывать
```
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 059 --out-dir tmp/floor-maps/genfix_059_after`
...
- `./node_modules/.bin/tsx --test tests/penrose-laundry.test.ts`
- `npm run typecheck`
