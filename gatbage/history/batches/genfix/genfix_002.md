# genfix_002: перескоки клеток: административные карманы, научная смена

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/002_z+49_procedural_z49_перескоки_клеток_административные_карманы,_научная_смена.png`
- Route: procedural `z49`, z+49, index 002/101
- Procedural profile: geometry `admin_pockets`, anomaly `teleport_cells`, majority `scientists`, danger 3
- Current metrics: rooms 88, doors 216, containers 19, entities 1379, reachable 52883, walls 995498, floors 53078
...
```text
002_z+49_procedural_z49_перескоки_клеток_административные_карманы,_научная_смена.png
нарушены масштабы дефолт этаж в плохом смысле что длинные пустые коридоры
ПЛОХО
```
...
- Процедурный профиль: geometry=admin_pockets, anomaly=teleport_cells, majority=scientists, danger=3. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 002 --out-dir tmp/floor-maps/genfix_002_after`
