# genfix_049: Мёбиус-подъезд

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/049_z+2_design_moebius_podezd_Мёбиус-подъезд.png`
- Route: design `moebius_podezd`, z+2, index 049/101
- Design route id: `moebius_podezd`, role: ориентация, зеркальные квартиры, паритетный шов, danger 2
- Current metrics: rooms 65, doors 108, containers 8, entities 3442, reachable 98714, walls 949858, floors 98718
...
- Target control shares: граждане 52%, ликвидаторы 18%, культисты 8%, учёные 12%, дикие 10%.
...
```text
049_z+2_design_moebius_podezd_Мёбиус-подъезд.png
масштабы не учтены
алгоритм не доделан 
нужно доделывать
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 52%, ликвидаторы 18%, культисты 8%, учёные 12%, дикие 10%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 049 --out-dir tmp/floor-maps/genfix_049_after`
...
- `./node_modules/.bin/tsx --test tests/moebius-podezd.test.ts`
- `npm run typecheck`
