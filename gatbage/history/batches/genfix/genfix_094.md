# genfix_094: грибница: затопленные эстакады, гражданский этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/094_z-43_procedural_z-43_грибница_затопленные_эстакады,_гражданский_этаж.png`
- Route: procedural `z-43`, z-43, index 094/101
- Procedural profile: geometry `sump_causeways`, anomaly `mushroom_mycelium`, majority `citizens`, danger 4
- Current metrics: rooms 85, doors 244, containers 38, entities 1849, reachable 64036, walls 984524, floors 64052
...
```text
094_z-43_procedural_z-43_грибница_затопленные_эстакады,_гражданский_этаж.png
грибница это хорошо 
но геометрия этажа плохая
```

## Diagnosis

- См. дословный фидбек: закрыть замечание видимой геометрией и проверяемыми метриками, а не только переименованием или лутом.
- Процедурный профиль: geometry=sump_causeways, anomaly=mushroom_mycelium, majority=citizens, danger=4. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 094 --out-dir tmp/floor-maps/genfix_094_after`
