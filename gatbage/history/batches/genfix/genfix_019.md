# genfix_019: Числовой реестр

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/019_z+32_design_number_registry_Числовой_реестр.png`
- Route: design `number_registry`, z+32, index 019/101
- Design route id: `number_registry`, role: остатки, модули, простые коридоры, danger 3
- Current metrics: rooms 12, doors 15, containers 4, entities 1963, reachable 58485, walls 990089, floors 58487
...
- Target control shares: граждане 30%, ликвидаторы 18%, культисты 10%, учёные 34%, дикие 8%.
...
```text
019_z+32_design_number_registry_Числовой_реестр.png
плохо 
просто коридоры и пустое место
нужно переделывать генератор
не соотвтестует назыанию
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 30%, ликвидаторы 18%, культисты 10%, учёные 34%, дикие 8%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 019 --out-dir tmp/floor-maps/genfix_019_after`
...
- `./node_modules/.bin/tsx --test tests/number-registry.test.ts`
- `npm run typecheck`
