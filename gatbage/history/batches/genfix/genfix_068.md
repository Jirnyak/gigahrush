# genfix_068: поражение самосбором: сервисные штреки, культовый этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/068_z-17_procedural_z-17_поражение_самосбором_сервисные_штреки,_культовый_этаж.png`
- Route: procedural `z-17`, z-17, index 068/101
- Procedural profile: geometry `service_spines`, anomaly `samosbor_seed`, majority `cultists`, danger 4
- Current metrics: rooms 89, doors 211, containers 16, entities 1989, reachable 113484, walls 935076, floors 113500
...
- Target control shares: граждане 15%, ликвидаторы 11%, культисты 36%, учёные 7%, дикие 22%, самосбор 9%.
- Dominant owner: культисты; reason: procedural majority cultists plus samosbor pressure.
...
- Samosbor share is pressure/scar territory, not a human mini HQ; it should create contested hazard pockets around organic damage or lower-route pressure.
...
```text
068_z-17_procedural_z-17_поражение_самосбором_сервисные_штреки,_культовый_этаж.png
потенциал есть но недоделано
```

## Diagnosis

- Главная проблема: генератор останавливается слишком рано и оставляет внешний/межкоридорный объем без структуры.
- Процедурный профиль: geometry=service_spines, anomaly=samosbor_seed, majority=cultists, danger=4. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- Faction control starts from HQs and approximately matches target shares: граждане 15%, ликвидаторы 11%, культисты 36%, учёные 7%, дикие 22%, самосбор 9%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 068 --out-dir tmp/floor-maps/genfix_068_after`
