# genfix_060: хладон: жилая нарезка, научная смена

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/060_z-9_procedural_z-9_хладон_жилая_нарезка,_научная_смена.png`
- Route: procedural `z-9`, z-9, index 060/101
- Procedural profile: geometry `living_blocks`, anomaly `hladon`, majority `scientists`, danger 3
- Current metrics: rooms 104, doors 94, containers 18, entities 1309, reachable 26941, walls 1021619, floors 26957
- Priority: P1 fill pass
...
- Target control shares: граждане 22%, ликвидаторы 16%, культисты 8%, учёные 42%, дикие 12%.
- Dominant owner: учёные; reason: procedural majority scientists.
...
```text
060_z-9_procedural_z-9_хладон_жилая_нарезка,_научная_смена.png
не доделан алгоритм надо заполнить этаж
```

## Diagnosis

- Главная проблема: генератор останавливается слишком рано и оставляет внешний/межкоридорный объем без структуры.
- Процедурный профиль: geometry=living_blocks, anomaly=hladon, majority=scientists, danger=3. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- Faction control starts from HQs and approximately matches target shares: граждане 22%, ликвидаторы 16%, культисты 8%, учёные 42%, дикие 12%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 060 --out-dir tmp/floor-maps/genfix_060_after`
