# genfix_028: поезда: административные карманы, этаж ликвидаторов

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/028_z+23_procedural_z23_поезда_административные_карманы,_этаж_ликвидаторов.png`
- Route: procedural `z23`, z+23, index 028/101
- Procedural profile: geometry `admin_pockets`, anomaly `rail_trains`, majority `liquidators`, danger 2
- Current metrics: rooms 82, doors 192, containers 19, entities 1275, reachable 51727, walls 996833, floors 51743
...
```text
028_z+23_procedural_z23_поезда_административные_карманы,_этаж_ликвидаторов.png
плохо слишком пустые коридоры длинные
но есть обещающие синие линии воды?
но плохо
```
...
- Процедурный профиль: geometry=admin_pockets, anomaly=rail_trains, majority=liquidators, danger=2. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- Transit-specific hint: make rail/tunnel macro carry stations, platforms, service rooms and cross-passages, not only long tracks.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 028 --out-dir tmp/floor-maps/genfix_028_after`
