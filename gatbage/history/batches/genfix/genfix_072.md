# genfix_072: песчаное перекрытие: коллекторы, этаж ликвидаторов

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/072_z-21_procedural_z-21_песчаное_перекрытие_коллекторы,_этаж_ликвидаторов.png`
- Route: procedural `z-21`, z-21, index 072/101
- Procedural profile: geometry `collectors`, anomaly `sandpile_perekrytie`, majority `liquidators`, danger 4
- Current metrics: rooms 96, doors 212, containers 25, entities 1997, reachable 201280, walls 847280, floors 201296
...
```text
072_z-21_procedural_z-21_песчаное_перекрытие_коллекторы,_этаж_ликвидаторов.png
плохо
```
...
- Процедурный профиль: geometry=collectors, anomaly=sandpile_perekrytie, majority=liquidators, danger=4. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `src/gen/procedural_anomalies/sandpile_perekrytie.ts`
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 072 --out-dir tmp/floor-maps/genfix_072_after`
