...
- Map: `tmp/floor-maps/all_route_seed_61061/101_z-50_story_story_void_Пустота.png`
- Route: story `story_void`, z-50, index 101/101
- Story route key: `story_void`
- Current metrics: rooms 19, doors 33, containers 22, entities 1772, reachable 171515, walls 877059, floors 171517
...
- Target control shares: граждане 4%, ликвидаторы 8%, культисты 24%, учёные 6%, дикие 34%, самосбор 24%.
...
```text
101_z-50_story_story_void_Пустота
оригинально но масштабы не учтены слишком много незаполнены можно ещё больше хаоса как раз чтобы места где ничего нет были заполнены чемто
```
...
- `src/gen/void/geometry.ts`
- `src/gen/void/index.ts`
- `tests/void-floor.test.ts`
- `factions.md`
...
- Faction control starts from HQs and approximately matches target shares: граждане 4%, ликвидаторы 8%, культисты 24%, учёные 6%, дикие 34%, самосбор 24%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 101 --out-dir tmp/floor-maps/genfix_101_after`
...
- `./node_modules/.bin/tsx --test tests/void-floor.test.ts`
- `npm run typecheck`
