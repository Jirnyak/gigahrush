# genfix_066: поезда: сервисные штреки, гражданский этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/066_z-15_procedural_z-15_поезда_сервисные_штреки,_гражданский_этаж.png`
- Route: procedural `z-15`, z-15, index 066/101
- Procedural profile: geometry `service_spines`, anomaly `rail_trains`, majority `citizens`, danger 3
- Current metrics: rooms 85, doors 211, containers 15, entities 1618, reachable 75052, walls 973508, floors 75068
...
```text
066_z-15_procedural_z-15_поезда_сервисные_штреки,_гражданский_этаж.png
поезда это хорошо
но генерация самого этажа плохая
нужно доделывать 
генерация недоделана
мб как систему туннелей метро со станциями и переходами и микрокомнатами межда (реф - реальное метро лондона и тд)
```

## Diagnosis

- Главная проблема: генератор останавливается слишком рано и оставляет внешний/межкоридорный объем без структуры.
- Процедурный профиль: geometry=service_spines, anomaly=rail_trains, majority=citizens, danger=3. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 066 --out-dir tmp/floor-maps/genfix_066_after`
