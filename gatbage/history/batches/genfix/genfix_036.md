# genfix_036: перескоки клеток: административные карманы, гражданский этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/036_z+15_procedural_z15_перескоки_клеток_административные_карманы,_гражданский_этаж.png`
- Route: procedural `z15`, z+15, index 036/101
- Procedural profile: geometry `admin_pockets`, anomaly `teleport_cells`, majority `citizens`, danger 3
- Current metrics: rooms 88, doors 279, containers 19, entities 1358, reachable 106215, walls 942345, floors 106231
...
```text
036_z+15_procedural_z15_перескоки_клеток_административные_карманы,_гражданский_этаж.png
интересное макро на надо комнат
```

## Diagnosis

- См. дословный фидбек: закрыть замечание видимой геометрией и проверяемыми метриками, а не только переименованием или лутом.
- Процедурный профиль: geometry=admin_pockets, anomaly=teleport_cells, majority=citizens, danger=3. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 036 --out-dir tmp/floor-maps/genfix_036_after`
