# genfix_016: перескоки клеток: административные карманы, дикий этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/016_z+35_procedural_z35_перескоки_клеток_административные_карманы,_дикий_этаж.png`
- Route: procedural `z35`, z+35, index 016/101
- Procedural profile: geometry `admin_pockets`, anomaly `teleport_cells`, majority `wild`, danger 3
- Current metrics: rooms 91, doors 246, containers 20, entities 1521, reachable 102771, walls 945789, floors 102787
...
```text
016_z+35_procedural_z35_перескоки_клеток_административные_карманы,_дикий_этаж.png
интересно но надо добавить комнат и разнообразия
```

## Diagnosis

- См. дословный фидбек: закрыть замечание видимой геометрией и проверяемыми метриками, а не только переименованием или лутом.
- Процедурный профиль: geometry=admin_pockets, anomaly=teleport_cells, majority=wild, danger=3. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 016 --out-dir tmp/floor-maps/genfix_016_after`
