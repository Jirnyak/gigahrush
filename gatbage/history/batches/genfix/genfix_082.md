# genfix_082: цеховой этаж, дикий этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/082_z-31_procedural_z-31_цеховой_этаж,_дикий_этаж.png`
- Route: procedural `z-31`, z-31, index 082/101
- Procedural profile: geometry `workshops`, anomaly `none`, majority `wild`, danger 4
- Current metrics: rooms 92, doors 225, containers 20, entities 1849, reachable 105366, walls 943194, floors 105382
...
- Target control shares: граждане 17%, ликвидаторы 13%, культисты 12%, учёные 8%, дикие 50%.
- Dominant owner: дикие; reason: procedural majority wild.
...
```text
082_z-31_procedural_z-31_цеховой_этаж,_дикий_этаж.png
потенциал есть но надо заполнять пустые 18_design_registry_morgue_Морг_регистраций
```

## Diagnosis

- Главная проблема: генератор останавливается слишком рано и оставляет внешний/межкоридорный объем без структуры.
- Процедурный профиль: geometry=workshops, anomaly=none, majority=wild, danger=4. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- Faction control starts from HQs and approximately matches target shares: граждане 17%, ликвидаторы 13%, культисты 12%, учёные 8%, дикие 50%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 082 --out-dir tmp/floor-maps/genfix_082_after`
