# genfix_096: bad apple!: затопленные эстакады, дикий этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/096_z-45_procedural_z-45_bad_apple!_затопленные_эстакады,_дикий_этаж.png`
- Route: procedural `z-45`, z-45, index 096/101
- Procedural profile: geometry `sump_causeways`, anomaly `bad_apple_world`, majority `wild`, danger 5
- Current metrics: rooms 96, doors 269, containers 22, entities 2108, reachable 68565, walls 979995, floors 68581
...
```text
096_z-45_procedural_z-45_bad_apple!_затопленные_эстакады,_дикий_этаж.png
плохо
```
...
- Процедурный профиль: geometry=sump_causeways, anomaly=bad_apple_world, majority=wild, danger=5. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 096 --out-dir tmp/floor-maps/genfix_096_after`
