...
- Map: `tmp/floor-maps/all_route_seed_61061/037_z+14_story_story_kvartiry_Квартиры.png`
- Route: story `story_kvartiry`, z+14, index 037/101
- Story route key: `story_kvartiry`
- Current metrics: rooms 13907, doors 79353, containers 28, entities 4662, reachable 823940, walls 224237, floors 824339
...
- Target control shares: граждане 66%, ликвидаторы 12%, культисты 6%, учёные 7%, дикие 9%.
...
```text
037_z+14_story_story_kvartiry_Квартиры.png
отличный оригинальный этаж
самобытен и не нуждается в улучшениях
(но это не значит что другие этажи должны его копироать - он именно крут тем что оригинален)
```
...
- `src/gen/kvartiry/social_macro_graph.ts`
- `src/gen/kvartiry/content_manifest.ts`
- `tests/kvartiry-social-macro-graph.test.ts`
- `factions.md`
...
- Faction control starts from HQs and approximately matches target shares: граждане 66%, ликвидаторы 12%, культисты 6%, учёные 7%, дикие 9%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 037 --out-dir tmp/floor-maps/genfix_037_after`
...
- `./node_modules/.bin/tsx --test tests/kvartiry-social-macro-graph.test.ts`
- `npm run typecheck`
