# genfix_011: Спецприёмник

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/011_z+40_design_spetspriemnik_Спецприёмник.png`
- Route: design `spetspriemnik`, z+40, index 011/101
- Design route id: `spetspriemnik`, role: задержанные, ключи, решетки и бунт, danger 3
- Current metrics: rooms 46, doors 59, containers 5, entities 1685, reachable 188153, walls 860421, floors 188155
...
- Target control shares: граждане 24%, ликвидаторы 44%, культисты 10%, учёные 8%, дикие 14%.
...
```text
011_z+40_design_spetspriemnik_Спецприёмник.png
интересно но нарушены масштабы
надо добавить маленьких комнат
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 24%, ликвидаторы 44%, культисты 10%, учёные 8%, дикие 14%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 011 --out-dir tmp/floor-maps/genfix_011_after`
...
- `./node_modules/.bin/tsx --test tests/spetspriemnik.test.ts`
- `npm run typecheck`
