# genfix_084: игра жизнь: затопленные эстакады, этаж ликвидаторов

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/084_z-33_procedural_z-33_игра_жизнь_затопленные_эстакады,_этаж_ликвидаторов.png`
- Route: procedural `z-33`, z-33, index 084/101
- Procedural profile: geometry `sump_causeways`, anomaly `conway_life`, majority `liquidators`, danger 5
- Current metrics: rooms 17, doors 22, containers 23, entities 2203, reachable 787663, walls 260897, floors 787679
...
```text
084_z-33_procedural_z-33_игра_жизнь_затопленные_эстакады,_этаж_ликвидаторов.png
оригинальный этаж 
не идеален но хорош своим особым смыслом
```

## Diagnosis

- См. дословный фидбек: закрыть замечание видимой геометрией и проверяемыми метриками, а не только переименованием или лутом.
- Процедурный профиль: geometry=sump_causeways, anomaly=conway_life, majority=liquidators, danger=5. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `src/gen/procedural_anomalies/conway_life.ts`
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 084 --out-dir tmp/floor-maps/genfix_084_after`
