# genfix_093: Спектральная часовня

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/093_z-42_design_spectral_chasovnya_Спектральная_часовня.png`
- Route: design `spectral_chasovnya`, z-42, index 093/101
- Design route id: `spectral_chasovnya`, role: звук, культ, слуховая геометрия, danger 5
- Current metrics: rooms 14, doors 26, containers 3, entities 4096, reachable 55948, walls 992626, floors 55950
...
- Target control shares: граждане 10%, ликвидаторы 8%, культисты 46%, учёные 6%, дикие 22%, самосбор 8%.
- Dominant owner: культисты; reason: floor-specific override from new territory brief.
...
```text
093_z-42_design_spectral_chasovnya_Спектральная_часовня.png
выглядит очень интересно и оригинально потенциал есть но не доделано масштабы не учтены и пусто нужно заполнить
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 10%, ликвидаторы 8%, культисты 46%, учёные 6%, дикие 22%, самосбор 8%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 093 --out-dir tmp/floor-maps/genfix_093_after`
...
- `./node_modules/.bin/tsx --test tests/spectral-chasovnya.test.ts`
- `npm run typecheck`
