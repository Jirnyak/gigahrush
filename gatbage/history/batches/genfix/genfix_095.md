# genfix_095: Кантор пустоты

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/095_z-44_design_cantor_pustoty_Кантор_пустоты.png`
- Route: design `cantor_pustoty`, z-44, index 095/101
- Design route id: `cantor_pustoty`, role: рекурсивные разрывы, мосты и пыльные острова, danger 5
- Current metrics: rooms 11, doors 13, containers 4, entities 4097, reachable 393708, walls 27439, floors 393710
...
- Target control shares: граждане 6%, ликвидаторы 8%, культисты 24%, учёные 8%, дикие 34%, самосбор 20%.
- Dominant owner: дикие; reason: floor-specific override from new territory brief.
...
```text
095_z-44_design_cantor_pustoty_Кантор_пустоты.png
идея хорошая очень
но масштабы нарушены полность и не доделано
слишком огромные структуры и поэтому пусто почти везде
надо сделать масштабы норм чтобы этаж был заполнен и тогда будет оригинально и красиво
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 6%, ликвидаторы 8%, культисты 24%, учёные 8%, дикие 34%, самосбор 20%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 095 --out-dir tmp/floor-maps/genfix_095_after`
...
- `./node_modules/.bin/tsx --test tests/cantor-pustoty.test.ts`
- `npm run typecheck`
