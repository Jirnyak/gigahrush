# genfix_014: радио-шахматы: чердачные венткамеры, дикий этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/014_z+37_procedural_z37_радио-шахматы_чердачные_венткамеры,_дикий_этаж.png`
- Route: procedural `z37`, z+37, index 014/101
- Procedural profile: geometry `attic_weatherworks`, anomaly `radio_chess`, majority `wild`, danger 4
- Current metrics: rooms 89, doors 209, containers 21, entities 1714, reachable 84332, walls 964219, floors 84357
...
```text
014_z+37_procedural_z37_радио-шахматы_чердачные_венткамеры,_дикий_этаж.png
ПЛОХО 
аналогично длинные коридоры
```

## Diagnosis

- См. дословный фидбек: закрыть замечание видимой геометрией и проверяемыми метриками, а не только переименованием или лутом.
- Процедурный профиль: geometry=attic_weatherworks, anomaly=radio_chess, majority=wild, danger=4. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `src/gen/procedural_anomalies/radio_chess.ts`
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 014 --out-dir tmp/floor-maps/genfix_014_after`
