# genfix_069: Служебный этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/069_z-18_design_service_floor_Служебный_этаж.png`
- Route: design `service_floor`, z-18, index 069/101
- Design route id: `service_floor`, role: служебный обход и ремонт, danger 3
- Current metrics: rooms 46, doors 49, containers 6, entities 2405, reachable 116284, walls 931666, floors 116286
...
- Target control shares: граждане 16%, ликвидаторы 52%, культисты 6%, учёные 12%, дикие 14%.
...
```text
069_z-18_design_service_floor_Служебный_этаж.png
нарушены масштабы и недоделано и пусто
большие комнаты со стеной надо реализовать лучше мотому что стена не очень понятно её можно обойти мб добавить типа будо по краям и тд
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 16%, ликвидаторы 52%, культисты 6%, учёные 12%, дикие 14%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 069 --out-dir tmp/floor-maps/genfix_069_after`
