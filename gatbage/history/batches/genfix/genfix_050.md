# genfix_050: поезда: жилая нарезка, гражданский этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/050_z+1_procedural_z1_поезда_жилая_нарезка,_гражданский_этаж.png`
- Route: procedural `z1`, z+1, index 050/101
- Procedural profile: geometry `living_blocks`, anomaly `rail_trains`, majority `citizens`, danger 2
- Current metrics: rooms 98, doors 88, containers 13, entities 1072, reachable 29691, walls 1018869, floors 29707
...
```text
050_z+1_procedural_z1_поезда_жилая_нарезка,_гражданский_этаж.png
хорошая задумка но не доделано нужно заполнить весь этаж алгоритмом
```

## Diagnosis

- Главная проблема: генератор останавливается слишком рано и оставляет внешний/межкоридорный объем без структуры.
- Процедурный профиль: geometry=living_blocks, anomaly=rail_trains, majority=citizens, danger=2. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 050 --out-dir tmp/floor-maps/genfix_050_after`
