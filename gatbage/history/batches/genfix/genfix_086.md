# genfix_086: фрактал: затопленные эстакады, гражданский этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/086_z-35_procedural_z-35_фрактал_затопленные_эстакады,_гражданский_этаж.png`
- Route: procedural `z-35`, z-35, index 086/101
- Procedural profile: geometry `sump_causeways`, anomaly `fractal_floor`, majority `citizens`, danger 5
- Current metrics: rooms 91, doors 217, containers 25, entities 2106, reachable 64499, walls 981968, floors 64691
...
```text
086_z-35_procedural_z-35_фрактал_затопленные_эстакады,_гражданский_этаж.png
скорее плохо но есть интересные элементы (как квадрат коридоров снизу слева) именно хорошо что разнообразие
но надо чтобы кроме них было ещё заполнение а не просто коридоры длинные
```

## Diagnosis

- Главная проблема: генератор останавливается слишком рано и оставляет внешний/межкоридорный объем без структуры.
- Процедурный профиль: geometry=sump_causeways, anomaly=fractal_floor, majority=citizens, danger=5. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `src/gen/procedural_anomalies/fractal_floor.ts`
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 086 --out-dir tmp/floor-maps/genfix_086_after`
