# genfix_062: зомби-апокалипсис: коллекторы, дикий этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/062_z-11_procedural_z-11_зомби-апокалипсис_коллекторы,_дикий_этаж.png`
- Route: procedural `z-11`, z-11, index 062/101
- Procedural profile: geometry `collectors`, anomaly `zombie_apocalypse`, majority `wild`, danger 4
- Current metrics: rooms 100, doors 248, containers 25, entities 4096, reachable 202337, walls 846223, floors 202353
...
- Target control shares: граждане 25%, ликвидаторы 12%, культисты 12%, учёные 9%, дикие 42%.
- Dominant owner: дикие; reason: zombie apocalypse should sit on ruined residential/civil structure, not cult circles.
...
```text
_z-11_procedural_z-11_зомби-
апокалипсис_коллекторы,_дикий_этаж.png
Задумка хорошая но плохая архитектура 
зомби апок должен быть в чём то типа квартир или жилого
и эти круги культа плохие (масштабы не учтены)
```
...
- Процедурный профиль: geometry=collectors, anomaly=zombie_apocalypse, majority=wild, danger=4. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `src/gen/procedural_anomalies/zombie_apocalypse.ts`
...
- Zombie-specific hint: move architecture toward residential/apartment density; avoid oversized cult circles as the primary structure.
...
- Faction control starts from HQs and approximately matches target shares: граждане 25%, ликвидаторы 12%, культисты 12%, учёные 9%, дикие 42%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 062 --out-dir tmp/floor-maps/genfix_062_after`
