# genfix_078: сортировочный конвейер: затопленные эстакады, культовый этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/078_z-27_procedural_z-27_сортировочный_конвейер_затопленные_эстакады,_культовый_этаж.png`
- Route: procedural `z-27`, z-27, index 078/101
- Procedural profile: geometry `sump_causeways`, anomaly `conveyor_sorter`, majority `cultists`, danger 5
- Current metrics: rooms 91, doors 211, containers 25, entities 2109, reachable 70063, walls 978497, floors 70079
...
- Target control shares: граждане 16%, ликвидаторы 12%, культисты 40%, учёные 8%, дикие 24%.
- Dominant owner: культисты; reason: procedural majority cultists.
...
```text
078_z-27_procedural_z-27_сортировочный_конвейер_затопленные_эстакады,_культовый_этаж.png
недоделно
возможно будет норм но сейчас пустой длинные коридоры очень нужно много комнат и доделывать
```
...
- Процедурный профиль: geometry=sump_causeways, anomaly=conveyor_sorter, majority=cultists, danger=5. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `src/gen/procedural_anomalies/conveyor_sorter.ts`
...
- Faction control starts from HQs and approximately matches target shares: граждане 16%, ликвидаторы 12%, культисты 40%, учёные 8%, дикие 24%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 078 --out-dir tmp/floor-maps/genfix_078_after`
