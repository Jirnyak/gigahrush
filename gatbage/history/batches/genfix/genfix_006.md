# genfix_006: говнячный смог: административные карманы, этаж ликвидаторов

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/006_z+45_procedural_z45_говнячный_смог_административные_карманы,_этаж_ликвидаторов.png`
- Route: procedural `z45`, z+45, index 006/101
- Procedural profile: geometry `admin_pockets`, anomaly `smog`, majority `liquidators`, danger 3
- Current metrics: rooms 88, doors 288, containers 27, entities 1482, reachable 100524, walls 948020, floors 100556
...
```text
006_z+45_procedural_z45_говнячный_смог_административные_карманы,_этаж_ликвидаторов.png
неплохо 
но наверное нужно добавить комнаты пока только коридоры есть
но к ним добавить наверное комнаты грозьдьями на этих коридорах
```

## Diagnosis

- См. дословный фидбек: закрыть замечание видимой геометрией и проверяемыми метриками, а не только переименованием или лутом.
- Процедурный профиль: geometry=admin_pockets, anomaly=smog, majority=liquidators, danger=3. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 006 --out-dir tmp/floor-maps/genfix_006_after`
