# genfix_097: bad apple!: затопленные эстакады, культовый этаж

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/097_z-46_procedural_z-46_bad_apple!_затопленные_эстакады,_культовый_этаж.png`
- Route: procedural `z-46`, z-46, index 097/101
- Procedural profile: geometry `sump_causeways`, anomaly `bad_apple_world`, majority `cultists`, danger 5
- Current metrics: rooms 92, doors 240, containers 20, entities 2105, reachable 63406, walls 985154, floors 63422
...
```text
097_z-46_procedural_z-46_bad_apple!_затопленные_эстакады,_культовый_этаж плохо
```

## Diagnosis

- См. дословный фидбек: закрыть замечание видимой геометрией и проверяемыми метриками, а не только переименованием или лутом.
- Процедурный профиль: geometry=sump_causeways, anomaly=bad_apple_world, majority=cultists, danger=5. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- `src/gen/procedural_anomalies/bad_apple_world.ts`
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 097 --out-dir tmp/floor-maps/genfix_097_after`
