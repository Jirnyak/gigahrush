# genfix_073: Кремниевый НЕТ-колодец

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/073_z-22_design_silicon_net_well_Кремниевый_НЕТ-колодец.png`
- Route: design `silicon_net_well`, z-22, index 073/101
- Design route id: `silicon_net_well`, role: НЕТ-доступ, кремниевая жизнь, редкое оружие, danger 4
- Current metrics: rooms 45, doors 50, containers 4, entities 2468, reachable 135465, walls 912400, floors 135467
...
- Target control shares: граждане 12%, ликвидаторы 18%, культисты 8%, учёные 48%, дикие 14%.
...
```text
073_z-22_design_silicon_net_well_Кремниевый_НЕТ-колодец.png
недоделано
скорее пока плохо
нужно доделывать алгоритм и идею и разнообразия и оригинальности
в центре чтото интересное но пока недоделано
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 12%, ликвидаторы 18%, культисты 8%, учёные 48%, дикие 14%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 073 --out-dir tmp/floor-maps/genfix_073_after`
