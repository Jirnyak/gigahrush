# genfix_058: цеховой этаж, этаж ликвидаторов

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/058_z-7_procedural_z-7_цеховой_этаж,_этаж_ликвидаторов.png`
- Route: procedural `z-7`, z-7, index 058/101
- Procedural profile: geometry `workshops`, anomaly `none`, majority `liquidators`, danger 2
- Current metrics: rooms 76, doors 187, containers 17, entities 973, reachable 99005, walls 949555, floors 99021
...
- Target control shares: граждане 17%, ликвидаторы 56%, культисты 7%, учёные 8%, дикие 12%.
- Dominant owner: ликвидаторы; reason: procedural majority liquidators.
...
```text
058_z-7_procedural_z-7_цеховой_этаж,_этаж_ликвидаторов.png
макро интересное и видно что у этажа есть потенциал он скорее хороший
но надо добавить больше комнат и заполнить для разнообразия мб ответвления коридоров ещё (островки где есть стркутры хорошие) и маленьких комнат
```

## Diagnosis

- Главная проблема: генератор останавливается слишком рано и оставляет внешний/межкоридорный объем без структуры.
- Процедурный профиль: geometry=workshops, anomaly=none, majority=liquidators, danger=2. Исправление должно усиливать семейство геометрии/аномалии универсально, а не под один PNG.
...
- Faction control starts from HQs and approximately matches target shares: граждане 17%, ликвидаторы 56%, культисты 7%, учёные 8%, дикие 12%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 058 --out-dir tmp/floor-maps/genfix_058_after`
