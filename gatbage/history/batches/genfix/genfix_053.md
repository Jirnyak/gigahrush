# genfix_053: Оранжерея бетона

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/053_z-2_design_oranzhereya_betona_Оранжерея_бетона.png`
- Route: design `oranzhereya_betona`, z-2, index 053/101
- Design route id: `oranzhereya_betona`, role: еда, вода, споры и дефицит, danger 3
- Current metrics: rooms 44, doors 98, containers 8, entities 1914, reachable 156671, walls 891903, floors 156673
...
- Target control shares: граждане 46%, ликвидаторы 10%, культисты 6%, учёные 24%, дикие 14%.
...
```text
053_z-2_design_oranzhereya_betona_Оранжерея_бетона.png
интересное макро но масштабы не учтены и нужно добавить микро склады и переходы и тд
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 46%, ликвидаторы 10%, культисты 6%, учёные 24%, дикие 14%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 053 --out-dir tmp/floor-maps/genfix_053_after`
...
- `./node_modules/.bin/tsx --test tests/oranzhereya-betona.test.ts`
- `npm run typecheck`
