# genfix_015: Бюро Кэли

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/015_z+36_design_cayley_byuro_Бюро_Кэли.png`
- Route: design `cayley_byuro`, z+36, index 015/101
- Design route id: `cayley_byuro`, role: порядок форм, генераторные двери, факторный обход, danger 3
- Current metrics: rooms 24, doors 49, containers 4, entities 1746, reachable 81002, walls 967572, floors 81004
...
- Target control shares: граждане 26%, ликвидаторы 20%, культисты 10%, учёные 34%, дикие 10%.
...
```text
015_z+36_design_cayley_byuro_Бюро_Кэли.png
ПЛОХО
непонятен замысел
почему это бюро кели где тогда алгоритм? просто набор комнат с нарушенным масштабом 
нужно делать генератор
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 26%, ликвидаторы 20%, культисты 10%, учёные 34%, дикие 10%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 015 --out-dir tmp/floor-maps/genfix_015_after`
...
- `./node_modules/.bin/tsx --test tests/cayley-byuro.test.ts`
- `npm run typecheck`
