# genfix_085: Аттракторный двор

## Route target

- Map: `tmp/floor-maps/all_route_seed_61061/085_z-34_design_attractor_dvor_Аттракторный_двор.png`
- Route: design `attractor_dvor`, z-34, index 085/101
- Design route id: `attractor_dvor`, role: потоки, щитки, патрульные петли, danger 4
- Current metrics: rooms 30, doors 22, containers 3, entities 2660, reachable 152102, walls 896472, floors 152104
...
- Target control shares: граждане 14%, ликвидаторы 36%, культисты 10%, учёные 16%, дикие 24%.
...
```text
085_z-34_design_attractor_dvor_Аттракторный_двор.png
потенциал заметен но недоделано
как обычно для недоделаных этажей снаружи пусто в центре чтото есть но надо доделывать и добавлять много и заполнять снаружи
```
...
- Faction control starts from HQs and approximately matches target shares: граждане 14%, ликвидаторы 36%, культисты 10%, учёные 16%, дикие 24%.
...
- `./node_modules/.bin/tsx scripts/render-procedural-floor-map.ts --seed 61061 --all --entry 085 --out-dir tmp/floor-maps/genfix_085_after`
...
- `./node_modules/.bin/tsx --test tests/attractor-dvor.test.ts`
- `npm run typecheck`
