# Items 152: black_concentrate

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `black_concentrate` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:108 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `black_concentrate` |
| Russian name | Чёрный концентрат |
| mode | new |
| intended type | FOOD |
| gameplay role | Worker/liquidator high-calorie ration |
| reachability target | HQ/factory |
| implementation note | Existing liquidator_ration may cover. |

## Worker Outcome 2026-05-25

Merged into existing `liquidator_ration` instead of adding `black_concentrate`. Current source already defines `liquidator_ration` as the closed-issue black nutritious concentrate: it is FOOD, reachable from storage/HQ rooms, mapped to the food economy, edible through the standard high-nutrition `feed(40)` handler, and carries `black_concentrate`, `nutritious_concentrate`, `field_ration`, `closed_issue` and `liquidator` tags.

No source item id was added. Adding `black_concentrate` would duplicate the same HQ/factory ration role and split existing food economy, field-ration and use-event coverage.
...
- src/data/item_packs/black_concentrate.ts
...
- Add or improve only `black_concentrate`.
...
- Whether `black_concentrate` was added, improved, merged into an existing id or rejected.
