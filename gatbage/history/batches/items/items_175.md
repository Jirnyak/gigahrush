# Items 175: wire_bundle

Status: single-item parallel worker plan, not shipped behavior. Created 2026-05-24.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `wire_bundle` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_5.md:88 (Trade, production, contraband, electronics and resident goods)
...
| --- | --- |
| id | `wire_bundle` |
| Russian name | Провода |
| mode | new |
| intended type | MISC |
| gameplay role | Electrical repair input |
| reachability target | Existing wire_coil may cover |
| implementation note | Deduplicate with existing. |

## Worker Outcome 2026-05-25

Merged into existing `wire_coil` instead of adding `wire_bundle`. Current source already defines `wire_coil` as the electrical repair input: it is reachable as room loot, mapped to tools/electronics, used by emergency panel repair costs and carries `repair`, `electronics`, `emergency_panel` and `pneumomail` tags.

No source item id was added. Adding `wire_bundle` would duplicate the same electrical repair input without adding a new buy/repair/spend decision.
...
- src/data/item_packs/wire_bundle.ts
...
- Add or improve only `wire_bundle`.
...
- Whether `wire_bundle` was added, improved, merged into an existing id or rejected.
