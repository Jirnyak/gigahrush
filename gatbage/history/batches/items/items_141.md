...
Status: deferred by worker pass, not shipped behavior. Created 2026-05-24. Reviewed 2026-05-25.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `splint` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:97 (NII sampleware, slime, medicine, food and anomaly loot)
...
| gameplay role | Fracture/limp treatment |
| reachability target | Medical |
| implementation note | Defer if no fracture status. |

## Worker Outcome 2026-05-25

Rejected for this pass. Current source has a bounded timed status system, but no fracture/limp status or movement injury that a splint can cure. The only current `fracture` references are Treskotnik AI/readability labels, not player state. Adding `splint` as generic HP medicine would duplicate bandage/painkiller roles and ignore the candidate's explicit defer note.

No source files were changed. Revisit after a generic fracture/limp status is implemented and validated.
...
- Whether `splint` was added, improved, merged into an existing id or rejected.
