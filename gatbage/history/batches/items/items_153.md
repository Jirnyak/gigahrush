# Items 153: red_concentrate

Status: implemented in source. Created 2026-05-24. Reviewed 2026-05-25.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `red_concentrate` and must not opportunistically implement neighboring items.

## Implementation Review

`red_concentrate` is implemented as `ItemType.FOOD` in `src/data/items.ts` with the player-facing name `Красный концентрат`, premium/bribe/concentrate tags, HQ and office reachability, food economy mapping and the standard bounded food-use handler. It is valuable relative to common concentrate and lower-nutrition than the closed liquidator ration, so the decision is to eat it before a route or keep/sell/trade it as a premium bribe ration.

Coverage: `tests/items_153_red_concentrate.test.ts` verifies item data, tags, food resource mapping, inventory category, use behavior and emitted item-use event tags.
...
| --- | --- |
| id | `red_concentrate` |
| Russian name | Красный концентрат |
| mode | new |
| intended type | FOOD |
| gameplay role | Premium/bribe ration |
| reachability target | HQ/office |
| implementation note | Valuable small food. |
...
- src/data/item_packs/red_concentrate.ts
...
- Add or improve only `red_concentrate`.
...
- Whether `red_concentrate` was added, improved, merged into an existing id or rejected.
