# Items 150: daily_concentrate

Status: merged into `grey_briquette`, not shipped as a separate id. Created 2026-05-24. Reviewed 2026-05-25.

Use this file as the complete brief for one GPT-5.5 subagent. That subagent owns only `daily_concentrate` and must not opportunistically implement neighboring items.
...
- Source plan row(s):
- items_4.md:106 (NII sampleware, slime, medicine, food and anomaly loot)
...
| Field | Value |
| --- | --- |
| id | `daily_concentrate` |
| Russian name | Пищевой концентрат ежедневный |
| mode | new |
| intended type | FOOD |
| gameplay role | Standard vending ration |
| reachability target | Kitchen/vending |
| implementation note | Could be alias to grey_briquette; dedupe first. |

## Worker Outcome 2026-05-25

Merged into existing `grey_briquette` instead of adding `daily_concentrate`. Current source already defines `grey_briquette` as the white daily concentrate ration: it is FOOD, tagged `concentrate` and `daily_ration`, reachable from kitchens/storage/common rooms, mapped to the food economy resource, edible through the standard `feed(18)` handler, redeemable from `concentrate_coupon`, and produced by the concentrate press.

No source item id was added. Adding `daily_concentrate` would duplicate the same kitchen/vending ration role and split existing concentrate press, coupon, food-resource and test coverage without creating a new player decision.
...
- src/data/item_packs/daily_concentrate.ts
...
- Add or improve only `daily_concentrate`.
...
- Whether `daily_concentrate` was added, improved, merged into an existing id or rejected.
