...
- Тень отбрасывается в сторону, противоположную источнику света (пока реализован только фонарик игрока, передаваемый через uniform массивы динамических источников).
...
- **Остаточные проблемы (задача на доработку):**
  1. Легаси блоб-тени (ambient gradient blobs, `uIsShadow == 2`) до сих пор остаются вокруг персонажей, когда они вне зоны фонарика.
  2. Нет теней от фич (features) и декоративной 3D-геометрии (от мешей нет теней, только от NPC/мобов).
  3. Динамические тени отбрасываются только от фонарика игрока (массив `dynamicLights`). От стационарных ламп и свечей динамических теней пока нет.
