...
`Tab` opens and closes the in-game hotkey screen. It lists every registered keyboard action, starts with a dedicated reset row, includes a mouse sensitivity slider, and lets the player add keys to the selected action immediately. A key can be assigned to multiple actions, and an action can have multiple keys; fixed browser/gameplay keys such as `Esc`, `Space` menu close and `Backspace` default reset are not assignable through normal capture. Bindings and sensitivity are stored in browser `localStorage` separately from the game save. The game uses `KeyboardEvent.code`, so defaults follow physical keys and keep working across keyboard layouts; the browser Tab focus action is prevented while the game input handler is active. Inside the hotkey screen itself, `Enter` activates the selected row or starts key capture, `Space` closes/back out of the screen, and `Backspace` restores default key bindings.
...
| `G` | open/close the separate map legend/settings screen; its options control what `M` and the minimap render |
...
| `Backspace` on hotkey screen | restore default key bindings |
...
| `Enter` | open game menu from gameplay; universal accept/confirm in menus; send Net Sphere line and close it |
| `Backspace` in Net Sphere | erase chat character |
| `Delete` in Net Sphere | close Net Sphere |
| `F11` | browser fullscreen |
| `~` | open/close debug menu |

`Esc` has no default gameplay/window binding. In the browser build it belongs to the browser and pointer-lock release path. `Enter` opens the game menu from normal gameplay and confirms the selected row inside canvas menus. `Space` closes or steps back from ordinary open canvas windows, while Net Sphere keeps it as printable chat input. `I`, `M`, `G`, `Q`, `L`, `F`, `Tab`, `U` and `~` close their own top-level menus when those menus are already open. `E` remains reserved for world interaction such as pickups, doors, NPCs and aimed interactables.
