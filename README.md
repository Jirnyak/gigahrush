...
>
> Роль: обязательная инструкция для всех агентов, работающих с кодом, документами, PR, релизами и системными контрактами ГИГАХРУЩА. Этот документ описывает корневую архитектуру и является обязательной инструкцией для агентов; он говорит, как безопасно читать, менять, проверять и не ломать проект.
...
Routine NPC movement deliberately allows corridor traffic but treats gradual corridor attractors as a P0 AI regression: low-pressure routine intents must not keep replacing active paths into A-B-A corridor loops, and the Living-floor regression stays in the normal unit/check gate. NPCs may naturally pause to investigate noise, barter with friends, or pick up items without abandoning their active paths.
...
The vertical game is the persistent `FloorRun`: the player starts on `LIVING` at `z=0`, normal lifts span `z=-50..+50`, and every stop is a keyed floor identity. Base authored floors, routed design floors, procedural stops and numbered lift instances are separate little worlds with their own generator/package, population field, NPC/monster mix, rules, POIs and route role. Even `z` stops are treated as design packages, odd `z` stops are procedural, and shared smooth `abs(z)` population curves provide the baseline that local floor profiles only retune. Danger and pressure generally rise away from the center through `z`, floor data, anomaly pressure and local design overrides rather than one hardcoded table.

The building is inhabited before the player arrives. A-Life creates a seed-sized population around `100_000` procedural NPC identities per run, with `131_072` as the technical identity capacity, and distributes them across base design, routed design and procedural route keys from a run-start population plan before the first floor is generated. Plot/authored/event reserved identities occupy slots inside that same budget; `npc:*` package reservations marked `presence: 'population'` can fill ordinary materialization slots, while `event_only` and legacy plot reservations stay event/authored-owned. Only the current floor is materialized into live AI; off-floor people are persistent records. Current shipped macro changes come from folded live state, deaths, saved overrides, bounded cold A-Life migration, caravan/economy/faction/contract events, authored arrivals and lazy background procedural looting on world rebuilds. Deaths are permanent, ordinary background refill is disabled, and cleared floors stay changed until an explicit arrival or migration changes them.
...
New runs create a compact in-memory pool around `100_000` procedural NPC records on every supported runtime, leaving unused headroom up to the `131_072` technical capacity for explicit arrivals and sanitizer clamps. Every single NPC allocated at run start—including both authored plot/side characters (`isPlotNpc`) and procedural inhabitants—receives a unique `alifeId` within this single unified pool and ID array (`AlifeState.npcs`). A universal population plan assigns every created record to the per-run vertical route stops (`FloorRun`): even-numbered floors (`z % 2 === 0`) as authored design floors and odd-numbered floors (`z % 2 !== 0`) as procedural mix/anomaly floors. Only the current floor is materialized into live `entities`; other floors keep identity, floor assignment, family id, quest affordance, RPG traits, deterministic default loadout, death state and optional last known coordinates without running AI. The macro A-Life pool (`AlifeState` / `Инфосеть Демос`) acts as the authoritative **Source of Truth** (`source of truth`) for all persistent character facts.
...
When a floor is generated (`materializeAlifeFloorPopulation`), ambient generator NPCs (`isAmbientNpcCandidate`) act as placement templates (`шаблоны`) and are consumed and replaced by real A-Life NPC entities with stable `alifeId` / `persistentNpcId` assigned to that floor bucket (`alife.floorIndex[floorKey]`). If the current floor bucket has fewer assigned A-Life records than its required capacity (`floorCaps[floorKey]`), the engine executes a radial surplus-borrowing search across surrounding floors (`+1, -1, +2, -2, ...` vertical distance). If a candidate floor has surplus population above its own capacity (`candidateBucket.length > cap`), surplus records (`recordIndex`) are borrowed and materialized on the active floor. Borrowing surplus records operates with **two-way synchronization**: when a record is moved to the current floor (`moveAlifeNpcRecord`), its macro assignment (`alife.floorIndex`, `record.floorKey`, `lastSeenZ`) is immediately updated in the macro A-Life (`AlifeState` / Demos) database so the character has officially migrated (`типа мигрировали`). Materialized NPCs also carry personal `playerRelation`, initialized from faction attitude plus deterministic individual fluctuation, and `karma` in `[-127, 127]`, with faction-biased distribution. Before floor transitions, samosbor rebuilds and saves, live A-Life NPC state is folded back into the pool. Killed A-Life NPCs and killed `plotNpcId` NPCs do not respawn on later visits. Saves store the A-Life seed, up to `65_536` dead A-Life ids, dead plot ids, changed-record overrides and a capped A-Life mobility queue, not the full live entity array.
...
Normal lifts move through a per-run vertical `FloorRun` route rather than directly through adjacent enum values. The player starts on `LIVING` at `z=0`; down decreases `z`, up increases `z`. The route spans 101 floors from `z=-50` to `z=+50`: `VOID` is the final lowest floor at `z=-50`, and `roof` is the highest floor at `z=+50`. The unified floor generation architecture distinguishes two classes of stops across the vertical span: **Even-numbered floors** (`z % 2 === 0`) are separate, independent authored design floors (`design:*` / `story:*` route keys, each authored as a standalone package without inheriting biomes), and **Odd-numbered floors** (`z % 2 !== 0`) are procedurally assembled (`procedural:*`) by randomly mixing pieces of other floors and introducing procedural anomalies. Until more authored floors exist, any unoccupied even slot also falls back to seeded procedural generation. The current span has 47 unique authored design stops and 54 procedural/fallback floors.
