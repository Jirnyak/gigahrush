...
| Main plot steps | 18 |
| Plot/side NPC ids after production manifests load | 336 |
...
`npm run content:audit` is intentionally conservative and reports static literal registrations: currently 336 plot NPC ids, 399 side quest steps and 135 literal contract entries. The runtime counts above include production manifest imports, dynamic route-floor side-quest registration and spread/composed contract arrays used by the running game.
...
Side quests use `registerSideQuest()` from `src/data/plot.ts`. Content modules register NPC definitions and quest steps at module import time. With production floor and design-floor manifests loaded, the current runtime registry has 336 plot/side NPC ids. `SIDE_QUESTS` has 399 steps across base data and `src/gen/`.
...
Events cover samosbor, zone capture, fog bosses, floor transitions, elevator anomalies, lift arachna, item pickup/drop/use/sale/handoff/destruction, generic interactive surface use, tool breakage, ammo consumption, UV spotlight use, monster bait, quest/system-assignment creation/completion/failure, theft, containers, production, pneumomail capsules, ration coupon audits, shelter tallies, smog, route cues, faction events, monster windups/counterplay and kills.
...
`Tab` opens the in-game hotkey screen. It lists every registered keyboard action, starts with a dedicated reset row, includes a mouse sensitivity slider, and lets the player add keys to the selected action immediately. A key can be assigned to multiple actions, and an action can have multiple keys; `E`, `Enter` and `Backspace` are assignable during key capture. Bindings and sensitivity are stored in browser `localStorage` separately from the game save. The game uses `KeyboardEvent.code`, so defaults follow physical keys and keep working across keyboard layouts; the browser Tab focus action is prevented while the game input handler is active. Inside the hotkey screen itself, `Enter` activates the selected row or starts key capture, while `Backspace`/`Delete` closes/back out of the screen. The top reset row restores default key bindings.

| Key | Action |
| --- | --- |
| `WASD` | movement |
| Mouse | mandatory look / pointer lock gate on desktop |
| `E` | in-world interaction: pickups, doors, NPCs, containers and other aimed world actions |
...
| top row on hotkey screen | restore default key bindings |
...
| `Enter` | open game menu from gameplay; universal accept/confirm in menus; send Net Sphere line |
| `Backspace` / `Delete` | universal close/back for menus and Net Sphere |
| `F11` | browser fullscreen |
| `~` | debug menu |

`Esc` has no default gameplay/window binding. In the browser build it belongs to the browser and pointer-lock release path. `Enter` opens the game menu from normal gameplay and confirms the selected row inside canvas menus. `Backspace`/`Delete` close or step back from open canvas windows. `E` remains reserved for world interaction such as pickups, doors, NPCs and aimed interactables.
