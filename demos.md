...
Смерть человека — единственное событие, где величина не назначается поводом, а **берётся из графа**: каждая прямая связь убитого отвечает убийце ровно своим весом.

```txt
delta(X → убийца) = −relation(X ↔ убитый)
```

Любил на `+127` — столько же отнимется; ненавидел на `−80` — столько же прибавится. Отдельных ставок за родню и друзей нет: близость это и есть величина связи, а не флаг рядом с ней. Убийца может быть и NPC, и игроком — во втором случае дельта идёт в слот игрока.
