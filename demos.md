...
**Шкала одна на весь проект** и определена один раз в `src/data/relations.ts`: `RELATION_MIN/MAX = ±127` (весь знаковый байт минус `RELATION_UNSET = −128`), пороги `RELATION_HOSTILE_THRESHOLD/RELATION_FRIENDLY_THRESHOLD = ∓64` — ровно половина пути в каждую сторону. По ней живут фракционная матрица, личное отношение к игроку и рёбра Демоса; второй шкалы, второго `clampRelation` и конвертации между ними в проекте быть не должно. Раньше их было именно так: `playerRelation` жил в ±100, граф в ±127, и каждая запись через Демос округлялась дважды.

Базовая матрица фракций записана самими порогами (`HOSTILE`, `FRIENDLY`, `KIN`), а не числами: рукописные значения уже разъезжались со шкалой — стоило порогу вражды сдвинуться, как пары, написанные ровно на старом пороге, молча становились нейтральными и фракции переставали воевать.
