...
- Save shape is `25` (`SAVE_SHAPE_VERSION`, `src/systems/save_runtime.ts`); `demosSocial` stores relation overrides, post ring and reaction ring, while generated computer and NET-hack terminal reward/cooldown keys persist as compact route-keyed runtime facts. Demos quest notices remain runtime state outside the save section unless they are represented by posts/relation facts; accepting a notice still happens only through face-to-face NPC talk and normal quest/contract systems.
...
DEMOS_SOCIAL_NPC_SLOTS = 7
DEMOS_SOCIAL_INITIAL_NPC_SLOTS = 4
DEMOS_SOCIAL_PUBLIC_SLOTS = 8 // player slot + 7 NPC slots
```

Почему 7:

- строка человека — степень двойки, и ровно столько же байт (восемь) занимает его отношение к фракциям в колонках A-Life: обе строки индексируются одним `alifeId` и растут вместе;
- стартовый процедурный круг обычно даёт до `5` public relations: player-slot плюс до `4` NPC slots;
- оставшиеся NPC slots свободны для событийного роста круга до `8` public relations total;
- 7 NPC slots дают место для 2-4 друга/родственника, 1-2 недруга, рабочих/долговых связей и новых weak ties без расширения storage;
...
**Связь симметрична: число одно на пару.** Отношение — свойство пары людей, а не двух независимых мнений, поэтому `A → B` и `B → A` всегда несут одно значение. Любая запись — генерация, авторская связь, рантайм-дельта, восстановленный оверрайд — зеркалит встречное ребро через `mirrorEdgeRelation`; если у второй стороны нет свободного слота, вытесняется её слабейшая связь: взаимность важнее знакомства ни о чём.

Направлена только **роль**: родитель ↔ ребёнок, а не «родители друг друга». Роль встречного ребра переворачивается (`reverseAuthoredRole`), число остаётся общим.

Односторонних отношений в графе не бывает, поэтому поля `bidirectional` у авторских связей и у связей пакетов больше нет. «Он её ненавидит, а она не в курсе» описывается не связью, а её отсутствием.

**Поправка к тому, где именно стоит зеркало (проверено прогоном 2026-09-12).** Зеркалит `mirrorEdgeRelation` — это путь ПОЛНОЙ сборки, авторских связей, связей пакетов и рантайм-дельт. Ленивая строка, которой и живёт настоящий прогон, зеркала не зовёт, и правильно делает: родство там собрано ЗВЕЗДОЙ вокруг двух старших семьи, обе стороны считают её одинаково, и ребро выходит взаимным по построению — а зеркало, которое переписывает роль отношением, как раз стёрло бы направленное «родитель/ребёнок». Знакомство же в ленивой строке остаётся односторонним, как и было до правки: слот у человека один, и требовать встречного места у незнакомца значило бы вытеснять у него родню.
...
**Шкала одна на весь проект** и определена один раз в `src/data/relations.ts`: `RELATION_MIN/MAX = ±127` (весь знаковый байт минус `RELATION_UNSET = −128`), пороги `RELATION_HOSTILE_THRESHOLD = −64` и `RELATION_FRIENDLY_THRESHOLD = +64`. Они ЗЕРКАЛЬНЫ, и порог дружбы выводится из порога вражды: пока он стоял на +32, другом становились вдвое дешевле, чем врагом, полосы ярлыков сверху и снизу мерили шкалу разным шагом, а авторские рёбра переворачивались (объявленное «знакомый» с числом 34 читалось как «друг»). Разница сторон живёт в ТАБЛИЦЕ баз, а не в порогах. По ней живут фракционная матрица, личное отношение к игроку и рёбра Демоса; второй шкалы, второго `clampRelation` и конвертации между ними в проекте быть не должно. Раньше их было именно так: `playerRelation` жил в ±100, граф в ±127, и каждая запись через Демос округлялась дважды.

Базовая матрица фракций (`BASE_FACTION_MATRIX`) асимметрична и записана числами: строка — кто смотрит, столбец — на кого. Ликвидатор смотрит на жителя сверху вниз (−24), житель на ликвидатора — с надеждой (+48). Числа в ней не приговор, а СЕРЕДИНА распределения: живое отношение конкретного человека рождается из клетки плюс гауссиана σ=32, и расстояние клетки до порога вражды задаёт долю тех, кто считает фракцию врагом (см. `alife.md`). Строка и столбец игрока живут отдельно: у него свой канал, союз в них записан зеркалом полосы «ненавидит» (+96), то есть другом игрока рождается 84% дружественной фракции. Все положительные базы стоят на +32 выше прежних: пороги стали зеркальными, и числа сдвинуты так, чтобы ДОЛИ остались теми же.
...
Дельта живёт в том же диапазоне, что и само отношение (`±254` по модулю, кламп только чтобы отсечь мусор). Прежний потолок в `±32` резал повод, а не результат, и делал убийство дешевле царапины.

### Смерть: цена связи целиком

Вред человеку — единственное событие, где величина не назначается поводом, а **берётся из графа**: каждая прямая связь пострадавшего отвечает обидчику своим весом, взятым по доле отнятого.

```txt
delta(X → обидчик) = −relation(X ↔ пострадавший) × доля
доля = 1 при смерти, доля снятого здоровья при ударе
```

Любил на `+127` — при смерти столько же и отнимется; ненавидел на `−80` — столько же прибавится. Полполоски стоит половины связи, поэтому слабый и сильный платят одинаково за одинаковое горе. Отдельных ставок за родню и друзей нет: близость это и есть величина связи, а не флаг рядом с ней. Обидчик может быть и NPC, и игроком — во втором случае дельта идёт в слот игрока.

**Смерть доходит всегда, удар — только через очевидца** (решение владельца 2026-09-12). О смерти узнают по телу; об избиении круг узнаёт, лишь если удар кто-то ВИДЕЛ — тот же луч свидетелей, которым платят зрители насилия (`alertWitnesses`). Избить человека наедине по-прежнему ничего не стоит в глазах тех, кто его любит, и это правило, а не недосмотр. Добивающий удар кругу не платит дважды: событие удара несёт пометку `killed`, и вес связи снимает только событие смерти.

Весть не расходится дальше прямых связей жертвы (`propagate: false`): друг друга о смерти не узнаёт. Отвечают **все** связи убитого, а не выборка — кап на событие поднят до числа слотов, потому что у смерти столько последствий, сколько у человека было связей.

Работает это на `player_kill_npc` / `npc_kill_npc`, и оба события обязаны нести `actorAlifeId` и `targetAlifeId`: без них адресовать последствия некому, и весь ответ круга близких молчит. Именно так эта механика и простояла мёртвой — код был написан, а события приходили с пустым `data`.
...
**Что из этого отгружено и где оно живёт (с 2026-09-12).** Полная сборка (`createGraph`) идёт только при населении ≤ `DEMOS_FULL_GRAPH_BUILD_LIMIT` = 2048; в настоящем прогоне записей сто тысяч, поэтому работает ленивая строка (`initializeLazyRow`), и до 2026-09-12 она была обеднённой копией: пункты 1 и 4 в ней не были реализованы вовсе — ни семейного индекса, ни `floorIndex`, кандидат брался равномерно по всему пулу. Замер на жилом этаже: из 1600 рёбер своих 92, родственных 0. Теперь у ленивой строки есть перепись своего этажа (`floorRoster`: список плюс семьи внутри него, кэш на три этажа, строится из `currentAlifeFloorRecordIds`), родство ставится первым, а знакомые берутся по этажу — **кроме последней из начальных ячеек (`DEMOS_SOCIAL_DISTANT_SLOT`), намеренно оставленной чужому этажу**: это пункт «cross-floor acquaintance», и без него обнулились бы социальные поездки (`requestOneSocialJourney` ищет связанного именно на ДРУГОМ этаже). Пункты «same occupation/faction» и «rival» по-прежнему живут только в полной сборке — там они выбор БАКЕТА, а не рода связи, и на ленивой строке их отсутствие ничего не ломает: род всё равно решает `describeCandidateEdge`.
