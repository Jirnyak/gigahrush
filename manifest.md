...
- **Такты**: `fly` (пролёт камеры, `wait` держит такт до прибытия), `orbit` (облёт точки), `say` (бабл над головой + строка в лог; без `seconds` длительность выводится из длины строки, как у обычных барков), `pause`, `awaitDeath` (ЖДЁТ смерти роли с верхней границей `timeout`, а не устраивает её), `defect` (смена фракции на ходу + сброс кэша боевой цели), `materialize` (воплотить отложенных актёров), `release` (снять поводок), `depart` (увести живых на другой этаж без записи смерти), `log`.
- **Актёры**: `packageId` зовёт уже живущего на этаже авторского NPC (`extractNpcForScene`), `source: 'alife'` черпает из пула A-Life (личностей не создаёт), иначе массовка спавнится с `role: NpcRole.CINEMATIC_ACTOR`. Расстановка идёт по расходящейся спирали свободных клеток, занятое запоминается — толпа растекается, а не встаёт штабелем.
...
Режиссёрская камера (`systems/camera.ts`, режим `cinematic`) поддерживает точку внимания (`lookAt`), орбиту вокруг неё, удержание кадра (`hold`), плавную целевую высоту с обрезкой по потолку клетки (`world.ceilHeight`, под открытым небом обрезки нет) и маршрут **по проходимым клеткам** (`routeCinematicCamera` через `bfsPath`). Кадровое перемещение режется на субшаги не длиннее допуска на узел, иначе на скорости камера перескакивает узел и идёт по хорде — то есть сквозь простенок.
