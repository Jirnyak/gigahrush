...
- **Reported cells** (via `markNavigationCellsDirty(cells)`) are **advisory only**. `patchNavigationRegions` re-syncs `_navCellVersion`/`_navPathBlockerVersion` and clears the report set — it performs **no region-graph rebuild** ("accept stale"), because the earlier O(R²) local-reflood revision froze large floors for 10 s+. A stale graph only costs a briefly sub-optimal path.
- **Removed with the local-reflood revision (do not look for them):** `NAV_COMPACT_GROWTH`, `NAV_PATCH_MAX_CLUSTERS`, `_bakeBaseRegions` — 0 occurrences repo-wide. Only `NAV_PATCH_MAX_CELLS` (4096) survives, and it merely bounds the size of the advisory report set.
...
- A `baseHash` (FNV-1a) drift guard makes a stale/drifted base decode to `null` → the loader regenerates the floor fresh instead of corrupting the grid. `SAVE_SHAPE_VERSION` was bumped to 24 by that change; the current value is 25.
...
## Закон владельца: никаких полных переборов в горячем пути

Date: 2026-08-22.

**Перебор всей коллекции (`world.containers`, `entities`, `world.rooms`) на каждого
актора — на снос, а не на оптимизацию.** Дозволенный минимум — обход фиксированно малого
числа клеток тора вокруг точки, запечённые поля и индексы. «Ближайший X по всему этажу» —
не запрос, а ошибка проектирования: у актора есть радиус, за которым для него ничего не
существует.

Цена такой строки не видна в коде и умножается на население. Диагноз, с которого закон
объявлен: министерство падало до 30 fps при тысяче человек, жилой этаж с бо́льшим
населением держал 60–70. Обе горячие цены зависели не от числа людей, а от размера этажа
и открытости планировки:

- `supplyErrandRoomId` (28% кадра) считался БЕЗУСЛОВНО каждый кадр у каждого работника, а
  использовался раз в 14–18 секунд, и внутри шёл по всем ящикам этажа.
- Сглаживание пути (27% кадра) первой пробой трассировало луч до КОНЦА маршрута. Цена
  трассировки равна пройденному открытому месту: в квартирах луч гибнет о стену через пару
  подклеток, в министерских залах пробегает полсотни клеток — каждый кадр на каждого идущего.

Инструменты вместо перебора:

- **Ящики** — `src/world/container_index.ts`: `containersInRoom` (проекция по комнате) и
  `forEachContainerNear` / `nearestContainer` (бакеты тора шага 16, как у сетки сущностей).
  Пересборка ленивая, по трём признакам самого мира: ссылка на массив (удаления идут
  `filter`), длина (добавления `push`), `cellVersion` (самосбор двигает комнаты под
  ящиками). Отдельный флаг грязи не нужен.
- **Сущности** — `getEntityIndex().byId` вместо `entities.find(e => e.id === …)`,
  `queryRadiusCapped` вместо перебора по радиусу.
- **Ловушка:** индекс сущностей держит ТОЛЬКО живых (`if (!e.alive) continue` во всех
  путях заполнения). Поиск трупов через него невозможен. Падальщик поэтому ищет не тело, а
  МЕСТО: `blood_fx` уже кладёт импульс в `world.dangerField`, и `findBloodTrailCell`
  (`systems/danger_field.ts`) читает это запечённое поле. Оставшийся такой перебор —
  ротационный скан трупов у головного слизня (`parasite*`).

Законно: запекание при загрузке этажа, кэши по `cellVersion`, один проход-построение
индекса раз в несколько секунд. Не законно: тот же проход на каждого актора. Перебор в
самосборе, A-Life-фолдах и сериализации — событийный, не горячий; трогать его = churn.

**Перебор БЕЗ актора — тот же закон (2026-08-25).** Строка в кадре стоит population-независимо,
но всё так же умножается на размер этажа: на жилом 10255 комнат и 2837 дверей. Ревизия
всех одиннадцати обходов `world.rooms` в рантайме: десять уже кешируют по местной идиоме
(`npc_feud.arenaRoomOf` — по тождеству мира, `floorHasFungusRooms` — по числу комнат,
`hladon` — по `cellVersion` плюс сигнатура) либо сидят на редких путях (нажатие `E`,
выдача квеста, прибытие, отладочные сводки). Некешированным в кадре был ровно один —
`arena_ladder.findArenaRoom`, и он же имел готового правильного близнеца в `npc_feud`.
...
401 мкс на кадр. Лечится не ускорением обхода, а тем, что смерть перестаёт быть тайной: единый путь
`killEntity` двигает эпоху, обход ждёт её сдвига (`Entity Death Contract` в `architecture.md`).
Замерено: 401 → 227 мкс на кадр, 24.1 → 13.6 мс/с. Остаток — динамический проход, он законен.
Общее правило из трёх случаев подряд (створки, песок арены, смерть): **если система ИЩЕТ изменение
чужого состояния, чинить надо не поиск, а молчание источника.**

**Двери получили свою версию.** `world.doorVersion` + `world.markDoorsDirty()` стоят в
общем ряду версий рядом с `cellVersion`. Нужны потому, что для НАВИГАЦИИ обычные открытая
и закрытая створки — одно и то же (топологию меняют только `LOCKED` и `HERMETIC_CLOSED`),
и `cellVersion` на обычном открывании не двигается, а картинка меняется. Без своей версии
`syncDoorStates` в `render/webgl.ts` перебирал все створки этажа каждый кадр и был листом
профиля. Гейт в рендере составной — `doorVersion` (вид), `doors.size` (створку создали или
...
**Date:** 2026-08-22
**Target:** Фоновая живность — CPU-цикл по пулу особей в каждом кадре.
**Changes:**

1. **Особей больше не существует.** `CRITTERS_POOL` (256 объектов), `updateCritters`
   с попыткой спавна, деспавном по дальности и порулёжкой на каждую особь, а также
   CPU-сборка инстанс-буфера в `drawCritters` — сняты целиком. Вместо них один
   `drawArraysInstanced` с фиксированным числом инстансов (`block² × perCell`: 24×24×3
   на десктопе, 14×14×3 на тач-устройстве), где каждый инстанс сам читает поля мира в
   вершинном шейдере (`src/render/critters_pass.ts`) и решает, кто он и есть ли он
   вообще. Не совпало с весами вида — вырожденный треугольник. CPU не платит ни за
   одну особь; бюджет живности стал константой, не зависящей от их количества на
   экране.
2. **Ни одной новой выборки в кадре.** Проход берёт уже загруженные `uCells`,
   `uFeatures`, `uLight` и одну новую текстуру `uDanger` (`world.dangerField`).
   Заливка поля запаха версионирована (`dangerFieldVersion()` в
   `systems/danger_field.ts`): поле меняется максимум 2 раза в секунду и только пока в
   нём что-то есть, поэтому на чистом этаже выгрузок ноль.
3. **Тач-устройства расконсервированы.** Запрет живности при `maxTouchPoints > 0`
   снят: он платил за CPU-симуляцию, которой больше нет. Осталась ручка в настройках
   и сторож по FPS (`< 30` — выключить).

**Правило на будущее:** фоновая мелочь описывается весами по полям мира и разворачивается
на GPU, а не пулом объектов на CPU. Пул нужен только тому, с чем игрок физически
взаимодействует.

**Date:** 2026-08-24
**Target:** Стратегический ярус полей восприятия раздавал цели В БЕТОН.
**Changes:**

1. **Ячейка яруса отдаёт своего ПРЕДСТАВИТЕЛЯ.** `macroTargetCell`
   (`src/systems/fields/macro.ts`) возвращал геометрический центр ячейки 16×16, проверив
   лишь «в ячейке есть хоть одна не-стена». Центр попадает в стену на **61% ячеек жилого
   этажа и на 99.7% квартир** (там шаг сетки квартир совпал с шагом яруса, и ярус был мёртв
   целиком). Комментарий обещал, что «точку внутри уточнит маршрут», но поиск пути на
   непроходимой цели возвращает пустой путь, и драйв молча падал. Теперь ячейка отдаёт
   ближайшую к своему центру проходимую клетку мира, `-1` у глухой.
2. **Своей цены у этого нет.** Представитель считается **тем же проходом по миру**, что и
   доля проходимых клеток ячейки, внутри `bakeMacroFields` — то есть на пути загрузки этажа
   и после сшивки самосбора, как требует Iron Law; в кадре не считается ничего. Хранение —
   один `Int32Array` на `MACRO_PLANE` = 4096 записей, **16 КиБ на этаж**, рядом с уже
   существующей плоскостью проходимости. Своей ручки тюнинга не заведено: шаг ячейки
   по-прежнему `MACRO_SHIFT`, общий с бродфейзом сущностей и индексами комнат и ящиков.
