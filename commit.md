...
## 0. Корпус `marx_N.md` выведен из репозитория (2026-08-06)

Планы «100 агентов» (`marx_0.md`…`marx_110.md` и мастер-чеклист `100.md`) **больше не лежат в репозитории** и **не являются действующей инструкцией**. Они перенесены в архив `../gatbage/gigahrush1_root_cleanup_2026-08-06/plans_marx/`.

Почему их нельзя исполнять как есть:

- все 111 файлов опираются на удалённый тип `FloorLevel` — код по ним не соберётся;
- 80 файлов требуют «обязательно делайте `git commit` и создавайте Pull Request» — это прямо противоречит действующему правилу «коммитить и пушить только по прямой просьбе владельца»;
- 80 файлов требуют «НЕ задавайте пользователю вопросы» — противоречит рабочему контракту;
- 13 файлов раздают готовые сниппеты с `Math.random()` в геймплейной логике при жёстком запрете в `CLAUDE.md`.

Действующие контракты для любого агента: `CLAUDE.md` / `AGENTS.md`, затем профильный системный документ из таблицы обязательного чтения. Тег `[marx_N]` в commit message больше не требуется.
...
Также критично: с выносом музыки из single-file (см. `keepMusicExternalPlugin` в `vite.config.ts`) архив обязан содержать рядом с `index.html` 10 `*.ogg` треков и воркер `loading_worker-*.js`. Загрузка одного `index.html` больше не несет музыку. Воркера `nav_worker-*.js` в архиве быть НЕ должно: он удалён вместе с матрицей маршрутов (`723ff08a`), и его отсутствие — не дефект сборки. Это относится и к MyIndie (тот же ZIP), и к Pikabu ZIP.
...
**Обязательный шаг каждого релиза, наравне с Cloudflare.** Отдельной просьбы не требуется: `комить`/`деплой` означает обе площадки.

```bash
npm run gh-pages:deploy
```

Это соберет проект с правильным `base` URL и отправит папку `dist` в отдельную организацию GIGAHRUSH. Игра будет доступна по адресу `https://gigahrush.github.io/`.

Порядок важен: `gh-pages:deploy` пересобирает `dist/` со своим `base`, поэтому запускай его **после** `cf:deploy`, иначе на Cloudflare уедет сборка с чужим базовым URL.

CDN GitHub Pages отдает старую версию еще какое-то время после публикации. Не объявляй деплой готовым по факту `Published` — проверь дату сборки в ответе:

```bash
curl -fsSI "https://gigahrush.github.io/?v=$(git rev-parse --short HEAD)" | grep -i last-modified
```
