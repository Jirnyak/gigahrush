...
- **Шкала отношений одна на проект** — `RELATION_MIN/MAX = ±127`, `RELATION_UNSET = −128`, пороги `∓64`, всё в `src/data/relations.ts`. Личное отношение к игроку, фракционная матрица и рёбра Демоса живут в ней без переводов. Накопитель один: дельта уходит через `applyDemosRelationDelta`, живая сущность зеркалит результат. Две независимые прибавки к одному числу давали разный итог в зависимости от того, когда игрок ушёл с этажа.
