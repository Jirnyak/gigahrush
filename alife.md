...
## Расстановка контекстна, а не изотропна (с 2026-08-23)

Этаж генерирует геометрию, зоны, владение клетками и МЕСТА под людей; личности приходят из
A-Life при входе на этаж. Раздача слотов идёт **по хозяину земли под слотом**: личность
встаёт на слот своей стороны, на чужой — только если своих не осталось. Ключом служит
именно земля, а не поле `faction` шаблона: у большинства шаблонов его нет вовсе.

- Мёртвая личность ЗАБИРАЕТ свой слот и не отдаёт никому: смерть — постоянный факт, дыра
  остаётся дырой. Пропуск мёртвых означал бы запрещённый рантайм-рефилл.
- Замер жилого этажа: на своей земле стояли 11.7% диких и 0% культистов, стало 95.9% и
  72.7%; смертность за 90 с упала 471 → 34, у диких 75% → 6.6%.

### Связанных сажают рядом (с 2026-09-12)

Второй вход в ту же раздачу: место выбирает не только земля, но и уже объявленный граф
связей. Человек садится к первому уже посаженному родственнику или другу — `pool.takeNear`
берёт ближайшее свободное место **на его собственной земле** в пределах
`DEMOS_SOCIAL_NEARBY_RADIUS`, того же радиуса, на котором AI вообще замечает связанного.
Уже посаженный враг это место ветирует. Не нашлось — обычная раздача, без исключений из
правила земли.

- **Земля выше родни.** Семья смешанных фракций рядом не сядет, и это не дефект: зоны
  контроля решают раньше. Замерено — к якорю удаётся посадить 664 из 1312 на жилом
  и 845 из 2237 на квартирах, остальные идут обычной раздачей и часто оказываются рядом
  всё равно, потоком шаблонов по квартирам.
- **Якорь предпочитается на своей земле:** родня на чужой территории якорем быть может, но
  место рядом с ней всё равно занять нельзя, и поиск закончился бы ничем (+9.8 п.п. на
  квартирах).
- Граф приходит сюда **инъекцией** (`registerAlifeSeatingBondSource`), а не импортом:
  `demos_social` уже импортирует A-Life, и обратное ребро завело бы новый рантайм-цикл
  систем. Цикл как был 4, так и остался.
- Замер: доля людей, у которых связанный ближе 24 клеток, жилой 0.16 % → 60.6…63.4 %
  (медиана расстояния 400 → 2…3 клетки), квартиры 0.00…0.07 % → 47.7 % (396 → 109).
  Цена раздачи мест: жилой в шуме, квартиры +2.5 %. Стенд —
  `npx tsx scripts/social_proximity_bench.ts <этаж> <сид...>`, замок —
  `tests/alife-social-seating.test.ts`.

## Личное отношение и память о комнатах

- Отношение к ИГРОКУ рождается как «живая база матрицы + гауссиана σ=32» — тем же законом,
  что отношение к фракциям (`attitudeSpread`). Прежний равномерный джиттер ±12 давал у всей
  фракции практически одно число; ручка `NPC_PLAYER_RELATION_FLUCTUATION` удалена.
- Кольцо последних шести комнат — колонка личности (`ALIFE_ROOM_VISIT_MEMORY`), доступ
  через `systems/room_visits.ts`. Переезд на другой этаж кольцо стирает (номера комнат
  этажные), сейв хранит его компактным оверрайдом даже у нетронутой личности.
- Высота этажа при переезде берётся из маршрута и спецификации процедурного этажа;
  прежняя таблица 30/60/100/140/180/200 (и `z = 100` для любого дизайн-этажа) выжжена.
...
- **Каждый живой NPC обязан числиться в макропопуляции.** Человек без записи A-Life не умирает по-настоящему, не имеет отношений, не виден Демосу, не мигрирует и не сворачивается при уходе с этажа — он остаётся на экране, но выпадает из мира. Усыновление на материализации этажа берёт любого живого NPC без `alifeId` и без авторской личности (`plotNpcId` / `npcPackageId` / `persistentNpcId`): предикат `needsAlifeAdoption`. Прежнее условие требовало ещё и `questId === -1`, поэтому всякий, кому генератор поля не проставил вовсе, молча оставался вне мира. Замок — `tests/alife-macropopulation.test.ts`.
...
- The numeric `plotNpcId` is a **slot number**, not an identity: it indexes the dense record array (`npcs[id - 1]`) and its typed columns, drives the range predicate `isPlotNpc` (`1 <= alifeId <= getPlotNpcCount()`), and is written into the save (`deadPlotNpcIds`, `Quest.giverId`, A-Life overrides). Since 2026-08-15 the slot order is frozen in `src/data/npc_plot_ids.ts`, so it no longer depends on which modules were imported; `getPlotNpcCount()` is a constant 473, not the count of loaded packages. A new authored NPC is appended to the END of that list. Reservations still bind by string: `reservedIdentityId = npc:<package>` wins over any numeric `plotNpcId` passed from outside (`reservedPlotSlot`).
...
- **Шкала отношений одна на проект** — `RELATION_MIN/MAX = ±127`, `RELATION_UNSET = −128`, порог вражды `−64`, порог дружбы `+64` (зеркало вражды: выводится из неё, второй ручки нет), всё в `src/data/relations.ts`. Личное отношение к игроку, фракционная матрица и рёбра Демоса живут в ней без переводов. Накопитель один: дельта уходит через `applyDemosRelationDelta`, живая сущность зеркалит результат. Две независимые прибавки к одному числу давали разный итог в зависимости от того, когда игрок ушёл с этажа.
- **Отношение к фракциям — личное, восемь байт на личность.** Колонка `factionAttitude` — плоский `Int8Array`, `(id − 1) * 8 + faction`: по ячейке на каждую из пяти фракций, три в запасе, столбцы сверх реальных фракций держат `RELATION_UNSET`. Ячейки рождаются ОДИН раз, при записи собственной фракции личности: база из `BASE_FACTION_MATRIX` плюс гауссиана σ=32, детерминированно от `alifeSeed` и `id` (сумма двенадцати равномерных из двух слов `hash32`, `Math.random()` не участвует). Ширина разброса — не «характер», а ДОЛЯ: доля тех, кто считает фракцию врагом, равна Φ((−64 − база)/32). База +40 даёт 0.06%, −24 — 11%, −64 — ровно половину, −120 — 96%; замерено по всему пулу, расхождение с Φ не превышает 1.2 процентного пункта (`tmp/faction-attitude-bench.ts`, замок — `tests/npc-faction-attitude.test.ts`).
- **Враждебность решается этим числом, а не матрицей.** `isHostile` (`systems/factions.ts`) читает ячейку смотрящего через `isSideHostileToFaction`; колонку подсовывают раз за кадр тем же `setFactionsSocialContext`, что и граф Демоса, поэтому предикат остаётся одним чтением байта. У кого личности нет — монстры, процедурные NPC, игрок — за тех отвечает база матрицы. Вражда стала НАПРАВЛЕННОЙ: житель может считать врагом ликвидатора, которому сам врагом не кажется. Ответная агрессия (`areSameSide`) и отношение к игроку этим не затронуты — у обоих свои каналы.
- **Двигают ячейку события, а не политика.** Всякий сдвиг личного отношения к ЧЕЛОВЕКУ (`applyDemosRelationDelta`, целевой вид `alife`) переносит на его фракцию ровно один шаг `QUEST_FACTION_RELATION_DELTA`: удар роняет, помощь и поручение поднимают. Отголоски по кругу знакомых (`propagate: false`) фракцию не двигают, к своей фракции отношение не двигается вовсе. Глобальную матрицу по-прежнему двигает только игрок.
...
**Роль сцены — состояние кадра, а не свойство личности.** `captureEntityToRecord` пишет в запись не `NpcRole.CINEMATIC_ACTOR`, а роль, которой человек жил до сцены (`entity.cinematicState.originalRole`, при её отсутствии — `WANDERER`), и обнуляет `record.cinematicState`. Причина жёсткая: цикл AI пропускает актёров сцены целиком (`systems/ai/index.ts`), поэтому записанная роль — это приговор. Человек возвращался бы из сейва живым снаружи и выключенным из симуляции внутри: не сканирует, не ходит, не стреляет, не бежит. Попасть туда было проще простого — автосейв срабатывает, когда игрок сворачивает вкладку, а сцена идёт минутами и ничего от игрока не требует. Материализация страхует то же самое с другой стороны: запись, в которой роль сцены всё-таки оказалась, поднимается как `WANDERER`. Замок — `tests/scene-role-not-persisted.test.ts` (и прямая запись, и круг сейв → загрузка).
