...
### 5.1 Предвзятость массивов (Array Prefix Bias / Fairness)
Многие системы (например, `src/systems/demos_social.ts`, `demos.ts`) до сих пор используют жесткий срез `slice(0, N)`. Это заставляет порядок элементов в памяти влиять на симуляцию (кто лежит первым, тот и получает приоритет). Нужно перевести логику на случайный выбор (shuffle) или честный скоринг перед срезом.

### 5.2 Хрупкие квестовые цели (Quest Target Truth)
В `src/systems/quests.ts` и `npc_interaction_options.ts` используется хардкодное сравнение по строкам (например, `room.name === target.roomName`, `npc.name === 'Мастер Арены'`). Это ломает квесты при изменении описаний или локализации. Необходимо перевести проверки на уникальные `id` или теги.

### 5.3 Mesh pop-in и просадки ФПС (Render vs Entity Query)
В рендеринге `src/render/mesh/scene_collect.ts` до сих пор используется тяжелый вызов `queryRadiusCapped()`. Отрисовка 3D-мешей зависит от дорогих запросов сущностей. Нужно развязать радиус отрисовки мешей от логики сущностей, опираясь на туман войны/тайлы.

### 5.4 Легаси ключи `story:*`
Несмотря на переход к единой Z-координате, строковый префикс `story:` все еще пронизывает кодовую базу (например, в `src/data/floor_keys.ts`, `alife.ts`, `lift_arachna.ts`). Это порождает дублирование (особенно в A-Life бакетах) и ломает UI Инфосети Демос. Необходимо полностью искоренить строковые префиксы для этажей в пользу прямой координатной привязки `Z`.
